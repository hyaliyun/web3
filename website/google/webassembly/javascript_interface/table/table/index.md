...
We can retrieve the index contents using [`Table.prototype.get()`](/WebAssembly/JavaScript_interface/Table/get):
...
We instantiate `table2.wasm` using the [`WebAssembly.instantiateStreaming()`](/WebAssembly/JavaScript_interface/instantiateStreaming_static) method:
...
The items at indexes 0 and 1 of the table are now callable [Exported WebAssembly Functions](/WebAssembly/Exported_functions). To call them, note that we must add the function invocation operator `()` after the `get()` call:
