...
If you have some Rust code, you can compile it into [WebAssembly](/WebAssembly) (Wasm). This tutorial will show you how to compile a Rust project into WebAssembly and use it in an existing web app.
...
As you might suspect, this is [the `alert` function provided by JavaScript](/Web/API/Window/alert). We call this function in the next section.
...
Serve the root directory of the project with a local web server, (e.g. `python3 -m http.server`). If you're not sure how to do that, refer to [Running a simple local HTTP server](/Learn/Common_questions/Tools_and_setup/set_up_a_local_testing_server#running_a_simple_local_http_server).
