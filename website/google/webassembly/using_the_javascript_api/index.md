...
If you have already [compiled a module from another language using tools like Emscripten](/WebAssembly/C_to_Wasm), or [loaded and run the code yourself](/WebAssembly/Loading_and_running), the next step is to learn more about using the other features of the WebAssembly JavaScript API. This article teaches you what you'll need to know.

> [!NOTE]
> If you are unfamiliar with the basic concepts mentioned in this article and need more explanation, read [WebAssembly concepts](/WebAssembly/Concepts) first, then come back.
...
3. Now, to help us understand what is going on here, let's look at the text representation of our Wasm module (which we also meet in [Converting WebAssembly format to Wasm](/WebAssembly/Text_format_to_Wasm#a_first_look_at_the_text_format)):
...
If you can't or don't want to use the streaming methods as described above, you can use the non-streaming methods [`WebAssembly.compile()`](/WebAssembly/JavaScript_interface/compile_static) / [`WebAssembly.instantiate()`](/WebAssembly/JavaScript_interface/instantiate_static) instead.
...
A memory instance can be grown by calls to [`Memory.prototype.grow()`](/WebAssembly/JavaScript_interface/Memory/grow), where again the argument is specified in units of WebAssembly pages:
...
Note: Since an {{jsxref("ArrayBuffer")}}'s byteLength is immutable, after a successful [`Memory.prototype.grow()`](/WebAssembly/JavaScript_interface/Memory/grow) operation the buffer getter will return a new ArrayBuffer object (with the new byteLength) and any previous ArrayBuffer objects become "detached", or disconnected from the underlying memory they previously pointed to.

Just like functions, linear memories can be defined inside a module or imported. Similarly, a module may also optionally export its memory. This means that JavaScript can get access to the memory of a WebAssembly instance either by creating a new `WebAssembly.Memory` and passing it in as an import or by receiving a Memory export (via [`Instance.prototype.exports`](/WebAssembly/JavaScript_interface/Instance/exports)).
...
Note how we create the {{jsxref("DataView")}} view on the Memory object's buffer ([`Memory.prototype.buffer`](/WebAssembly/JavaScript_interface/Memory/buffer)), not on the Memory itself.
...
Tables can be mutated via [`Table.prototype.set()`](/WebAssembly/JavaScript_interface/Table/set), which updates one of the values in a table, and [`Table.prototype.grow()`](/WebAssembly/JavaScript_interface/Table/grow), which increases the number of values that can be stored in a table. This allows the indirectly-callable set of functions to change over time, which is necessary for [dynamic linking techniques](https://github.com/WebAssembly/tool-conventions/blob/main/DynamicLinking.md). The mutations are immediately accessible via [`Table.prototype.get()`](/WebAssembly/JavaScript_interface/Table/get) in JavaScript, and to Wasm modules.
...
To create a WebAssembly global instance from inside your JavaScript, you use the [`WebAssembly.Global()`](/WebAssembly/JavaScript_interface/Global) constructor, which looks like this:
...
You can see multiplicity in action in our Understanding text format article — see the [Mutating tables and dynamic linking section](/WebAssembly/Understanding_the_text_format#mutating_tables_and_dynamic_linking).
