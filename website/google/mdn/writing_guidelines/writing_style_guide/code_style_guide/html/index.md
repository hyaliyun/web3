...
> [!NOTE]
> The guidelines in this section apply only when you need to show a complete HTML document. A snippet is usually enough to demonstrate a feature. When using the [EmbedLiveSample macro](/MDN/Writing_guidelines/Page_structures/Code_examples#traditional_live_samples), just include the HTML snippet; it will automatically be inserted into a full HTML document when displayed.
...
Set the document language using the [`lang`](/Web/HTML/Global_attributes#lang) attribute on your {{htmlelement("html")}} element:
...
See [Using the viewport meta tag to control layout on mobile browsers](/Web/HTML/Viewport_meta_tag) for further details.
