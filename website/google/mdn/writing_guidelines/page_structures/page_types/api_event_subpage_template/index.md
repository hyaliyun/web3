...
>     For example, the [animationcancel](/Web/API/Element/animationcancel_event) event of the [Window](/Web/API/Window) interface has a _title_ of `Window: animationcancel event`.
> - **slug**
...
> For example, [Element: wheel event](/Web/API/Element/wheel_event) is linked from [`Element` Events](/Web/API/Element#events).
>
> If the parent object does not have an _Events_ section, then add one.
> If this is a new "class" of event, then you should add a link to this section of the parent from the [Event reference](/Web/Events).
>
