...
>
> - **title**
>   - : Title heading displayed at the top of the page. Format as _NameOfTheHeader_. For example, the [Cache-Control](/Web/HTTP/Headers/Cache-Control) header has a _title_ of `Cache-Control`.
> - **slug**
>   - : The end of the URL path after `https://developer.mozilla.org/`. This will be formatted like `Web/HTTP/Headers/NameOfTheHeader`. For example, the [Cache-Control](/Web/HTTP/Headers/Cache-Control) slug is `Web/HTTP/Headers/Cache-Control`.
> - **page-type**
>   - : For HTTP headers, must be `http-header`. For other HTTP `page-type` values, see the [HTTP section](/MDN/Writing_guidelines/Page_structures/Page_types/Page_type_key#http_page_types) of the documentation for the `page-type` front matter key.
> - **status**
...
>
> - `\{{SeeCompatTable}}` — this generates a **This is an experimental technology** banner that indicates the header is [experimental](/MDN/Writing_guidelines/Experimental_deprecated_obsolete#experimental).
...
> - `\` — this generates a **Deprecated** banner that indicates that use of the header is [discouraged](/MDN/Writing_guidelines/Experimental_deprecated_obsolete#deprecated).
...
>
> - `\` — this generates the HTTP sidebar that must appear on every HTTP reference page.
...
Fill in a syntax box, like the one below, according to the guidance in our [syntax sections](/MDN/Writing_guidelines/Page_structures/Syntax_sections) article.
...
Include links to reference pages and guides related to the current HTTP header. For more guidelines, see the [See also section](/MDN/Writing_guidelines/Writing_style_guide#see_also_section) in the _Writing style guide_.
