...
An ARIA role page should have a `title` and `slug` of `ARIA: Name Of The Role`. For example, the [button role](/Web/Accessibility/ARIA/Roles/button_role) has a `title` and `slug` of `ARIA/NameOfTheRole_role` and the [aria-labelledby](/Web/Accessibility/ARIA/Attributes/aria-labelledby) attribute has a `title` of `aria-labelledby`.
...
To include the (appropriate) feature status key — [**experimental**](/MDN/Writing_guidelines/Experimental_deprecated_obsolete#experimental), [**deprecated**](/MDN/Writing_guidelines/Experimental_deprecated_obsolete#deprecated), or **non-standard** — see the section ["How to add or update feature statuses"](/MDN/Writing_guidelines/Page_structures/Feature_status#how_to_add_or_update_feature_statuses).
...
Include links to reference pages and guides related to the current role or attribute. For more guidelines, see the [See also section](/MDN/Writing_guidelines/Writing_style_guide#see_also_section) in the _Writing style guide_.
