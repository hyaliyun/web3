...
>     For example, the [`:hover`](/Web/CSS/:hover) selector has a title of _:hover_.
> - **slug**
>   - : The end of the URL path after `https://developer.mozilla.org/`). This will be formatted like `Web/CSS/:NameOfTheSelector`.
>     For example, the [`:hover`](/Web/CSS/:hover) selector slug is `Web/CSS/:hover`.
> - **page-type**
>   - : The `page-type` key for CSS properties is one of `css-selector`, `css-pseudo-class`, or `css-pseudo-element`, depending on whether the selector is a [pseudo-class](/Web/CSS/Pseudo-classes), a [pseudo-element](/Web/CSS/Pseudo-elements), a [combinator](/Web/CSS/CSS_selectors/Selectors_and_combinators#combinators), or a [simple selector](/Web/CSS/CSS_selectors/Selector_structure#simple_selector).
> - **status**
...
Include links to reference pages and guides related to the current selector. For more guidelines, see the [See also section](/MDN/Writing_guidelines/Writing_style_guide#see_also_section) in the _Writing style guide_.
