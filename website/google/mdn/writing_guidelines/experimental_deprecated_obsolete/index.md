...
On the other hand, some technologies might have a stable specification, but do not have native support in browsers. [IMSC](/Related/IMSC) for example is used via a JavaScript polyfill. <!-- need to revisit link -->
...
When in doubt, feel free to ask for advice on the [MDN Web Docs chat rooms](/MDN/Community/Communication_channels#chat_rooms).
...
For example, as of Jan 2019, the [`inputmode`](/Web/HTML/Global_attributes/inputmode) global attribute has a conflict, which was summarized like so: <!--this warning example for spec conflict does not exist anymore on that page. couldn't find any other examples as well -->
