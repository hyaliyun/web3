...
- Templates
  ...
  - `pug` - [Pug templates](https://pugjs.org/api/getting-started.html) (which may be used by [Express](/Learn/Server-side/Express_Nodejs/Displaying_data/Template_primer))
- Other
  - `plain` - Plain text
  - `diff` - Diff file
  - `http` - HTTP headers
...
The _page summary_ is the first "content" paragraph in a page—the first text that appears after the page front matter and any [sidebar](/MDN/Writing_guidelines/Page_structures/Macros/Commonly_used_macros#sidebar_generation) or [page banner](/MDN/Writing_guidelines/Page_structures/Macros/Commonly_used_macros#page_or_section_header_indicators) macros.
...
```md
The **`margin`** [CSS](/Web/CSS) property
...
See [Using macros](/MDN/Writing_guidelines/Page_structures/Macros) for more information on macros.
