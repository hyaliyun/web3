...
The example we'll refer to in this article is the [Fetch API](/Web/API/Fetch_API).
...
For example, look at the [Fetch API](/Web/API/Fetch_API) page on MDN.
...
```html
<li><a href="/Web/API/Response">Response</a></li>
```
...
1. `"overview"` — the value is an array, inside of which you include the slug of the API overview page, if there is one.
   "Fetch API" results in a link being made to [https://developer.mozilla.org/Web/API/Fetch_API](/Web/API/Fetch_API).
2. `"interfaces"` — the value is an array in which you should list all of the interfaces that form part of that API.
   "Response" results in a link being made to [https://developer.mozilla.org/Web/API/Response](/Web/API/Response).
3. `"methods"` — the value is an array that should contain any methods the spec adds to interfaces associated with other APIs, such as instantiation methods created on {{domxref("Navigator")}} or {{domxref("Window")}}.
   ...
   "fetch()" results in a link being made to [https://developer.mozilla.org/Web/API/fetch](/Web/API/Window/fetch).
...
4. `"properties"` — the value is an array that should contain all of the properties associated with the API.
   ...
   "Headers.append" results in a link being made to [https://developer.mozilla.org/Web/API/Headers/append](/Web/API/Headers/append).
5. `"events"` — the value is an array that should contain the _title_ of events that are part of the API bit are defined in interfaces that are _not_ part of the API (events belonging to interfaces in the API (`interfaces`) are documented by default).
   ...
   For example, `"Document: selectionchange"` is part of the [Selection API](/Web/API/Selection_API) but `Document` is not, so we add the event to the array and it will be linked from the [Selection API](/Web/API/Selection_API) topic.
6. `"guides"` — the value is an array of strings, each that addresses a guide topic that explain how to use the API.
   ...
   For example, to link to the topic "Using Fetch" at `https://developer.mozilla.org/Web/API/Fetch_API/Using_Fetch`, the guide array would contain "/docs/Web/API/Fetch_API/Using_Fetch".
7. `"dictionaries"` — an array of strings listing all of the dictionaries which are part of the API.
   ...
   "CryptoKeyPair" results in a link to [https://developer.mozilla.org/Web/API/CryptoKeyPair](/Web/API/CryptoKeyPair).
   > [!NOTE]
...
As an example, the [WebVR API](/Web/API/WebVR_API)'s sidebar is included in its pages with the following:
