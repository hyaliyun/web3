...
- The CSS property [`transform-box`](/Web/CSS/transform-box) now supports the values `content-box` and `stroke-box`. For the reference box, the `content-box` value uses the [content box](/Learn/CSS/Building_blocks/The_box_model#parts_of_a_box) and the `stroke-box` value uses the stroke bounding box that contains an SVG's shape ([Firefox bug 1868374](https://bugzil.la/1868374)).
- The [`content-visibility`](/Web/CSS/content-visibility) CSS property value `auto` is now enabled by default. This allows content to skip rendering if it is not [relevant to the user](/Web/CSS/CSS_containment#relevant_to_the_user). ([Firefox bug 1874874](https://bugzil.la/1874874)).
...
- The [Popover API](/Web/API/Popover_API) is now fully supported, allowing the creation of top level "popover" UI elements that might be used for action menus, custom "toast" notifications, form element suggestions, content pickers, and so on.
  ...
  - [`HTMLButtonElement`](/Web/API/HTMLButtonElement) properties [`popoverTargetElement`](/Web/API/HTMLButtonElement/popoverTargetElement) and [`popoverTargetAction`](/Web/API/HTMLButtonElement/popoverTargetAction).
  - [`HTMLInputElement`](/Web/API/HTMLInputElement) properties [`popoverTargetElement`](/Web/API/HTMLInputElement/popoverTargetElement) and [`popoverTargetAction`](/Web/API/HTMLInputElement/popoverTargetAction).
  - [`HTMLElement`](/Web/API/HTMLElement) property [`popover`](/Web/API/HTMLElement/popover), [`hidePopover()`](/Web/API/HTMLElement/hidePopover), [`showPopover()`](/Web/API/HTMLElement/showPopover), and [`togglePopover()`](/Web/API/HTMLElement/togglePopover) methods, and [`beforetoggle`](/Web/API/HTMLElement/beforetoggle_event) and [`toggle_event`](/Web/API/HTMLElement/toggle_event) events (of type [`ToggleEvent`](/Web/API/ToggleEvent)).

  The following CSS pseudo class and element are now supported for use with popovers:

  - [`:popover-open`](/Web/CSS/:popover-open)
  - [`::backdrop`](/Web/CSS/::backdrop) has been extended to support popovers

  The following HTML global attributes are supported:

  - [`popovertarget`](/Web/HTML/Element/button#popovertarget)
  - [`popovertargetaction`](/Web/HTML/Element/button#popovertargetaction)
...
- {{domxref("Element.ariaBrailleLabel")}} and {{domxref("Element.ariaBrailleRoleDescription")}} are now supported, respectively reflecting the global ARIA HTML attributes [`aria-braillelabel`](/Web/Accessibility/ARIA/Attributes/aria-braillelabel) and [`aria-brailleroledescription`](/Web/Accessibility/ARIA/Attributes/aria-brailleroledescription). ([Firefox bug 1861201](https://bugzil.la/1861201)).
...
- Added support to allow web applications to gracefully recover if a canvas temporarily loses its 2D context, which might happen if the canvas is running hardware-accelerated on a GPU, and its driver crashes ([Firefox bug 1887729](https://bugzil.la/1887729)).
  ...
  - Applications can monitor for [`contextlost`](/Web/API/HTMLCanvasElement/contextlost_event) and [`contextrestored`](/Web/API/HTMLCanvasElement/contextrestored_event) events, which are fired on at [`HTMLCanvasElement`](/Web/API/HTMLCanvasElement) when the context is lost and recovered, respectively, and can also check the context using [`CanvasRenderingContext2D.isContextLost()`](/Web/API/CanvasRenderingContext2D/isContextLost).
  ...
  - Offscreen canvases can be monitored in the same way, but using [`OffScreenCanvas`](/Web/API/OffscreenCanvas) events [`contextlost`](/Web/API/OffscreenCanvas/contextlost_event) and [`contextrestored`](/Web/API/OffscreenCanvas/contextrestored_event), along with [`OffscreenCanvasRenderingContext2D.isContextLost()`](/Web/API/OffscreenCanvasRenderingContext2D#context).

- Added support for the [`shadowrootclonable`](/Web/HTML/Element/template#shadowrootclonable) attribute of the `<template>` element, and the [`shadowRootClonable`](/Web/API/HTMLTemplateElement/shadowRootClonable) property of the `HTMLTemplateElement` interface that reflects it.
  These set the [`clonable`](/Web/API/ShadowRoot/clonable) property of a [`ShadowRoot`](/Web/API/ShadowRoot) that has been created declaratively using the [`<template>`](/Web/HTML/Element/template) element.
  ([Firefox bug 1880188](https://bugzil.la/1880188)).

- The [`readText()`](/Web/API/Clipboard/readText) method of the [`Clipboard`](/Web/API/Clipboard) interface is now supported for asynchronously reading text from the system clipboard.
...
- The [AV1](/Web/Media/Formats/Video_codecs#av1) codec is now supported for [Encrypted Media Extensions](/Web/API/Encrypted_Media_Extensions_API), enabling higher-quality playback from video streaming providers. ([Firefox bug 1601817](https://bugzil.la/1601817)).

#### Removals

- The [`SVGAElement.text`](/Web/API/SVGAElement#svgaelement.text) property has been removed. The {{domxref("Node.textContent", "textContent")}} property (inherited from `Node`) is broadly supported and should be used instead. ([Firefox bug 1880689](https://bugzil.la/1880689)).
...
- Support has been added for Wasm modules to use multiple independent linear memories.
  ...
  WebAssembly [memory instructions](/WebAssembly/Reference/Memory) use the index to reference the memory on which they are operating, defaulting to the first memory defined if no index is specified.
  For more information, see [WebAssembly Memory](/WebAssembly/Understanding_the_text_format#webassembly_memory) in _Understanding WebAssembly text format_.
...
- The content of the {{WebExtAPIRef("proxy.ProxyInfo")}} property `proxyAuthorization` is now passed to the {{httpheader("Proxy-Authorization")}} request header sent to HTTP proxies (in addition to the existing support for HTTPS proxies) as part of a [CONNECT](/Web/HTTP/Methods/CONNECT) request ([Firefox bug 1794464](https://bugzil.la/1794464)).

## Experimental web features

These features are shipping in Firefox 125 but only in developer releases or behind a preference. To try these out, you can search for the related preference on the `about:config` page to see if they're enabled or disabled. To learn more about these features, see the [Experimental features](/Mozilla/Firefox/Experimental_features) page.
...
- **CSS `transition-behavior`:** `layout.css.transition-behavior.enabled`.
  ...
  Authors can use this property to control whether to apply CSS transitions to properties with a [discrete animation type](/Web/CSS/CSS_animated_properties#discrete) ([Firefox bug 1882408](https://bugzil.la/1882408), [Firefox bug 1805727](https://bugzil.la/1805727)).
