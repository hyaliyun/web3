...
- The ECMAScript 2021 `export * as namespace` syntax for the [`export`](/Web/JavaScript/Reference/Statements/export) statement is now supported ([Firefox bug 1496852](https://bugzil.la/1496852)).

### HTTP

- Previously, when the [fullscreen](/Web/HTTP/Headers/Permissions-Policy/fullscreen) directive was applied to an [`<iframe>`](/Web/HTML/Element/iframe) (i.e. via the `allow` attribute), it didn't work unless the `allowfullscreen` attribute was also present This has now been fixed ([Firefox bug 1608358](https://bugzil.la/1608358)).

### APIs

#### DOM

- Web Animations API compositing operations are now enabled — see [`KeyframeEffect.composite`](/Web/API/KeyframeEffect/composite) and [`KeyframeEffect.iterationComposite`](/Web/API/KeyframeEffect/iterationComposite) ([Firefox bug 1652676](https://bugzil.la/1652676)).

#### Removals

- The `outerHeight` and `outerWidth` features of [`Window.open()`](/Web/API/Window/open) are no longer exposed to web content ([Firefox bug 1623826](https://bugzil.la/1623826)).
