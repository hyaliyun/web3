...
- [Using audio and video](/Learn/HTML/Multimedia_and_embedding/Video_and_audio_content)
  - : Firefox 3.5 adds support for the HTML 5 [`audio`](/Web/HTML/Element/audio) and [`video`](/Web/HTML/Element/video) elements.
- Offline resources in Firefox
  - : Firefox 3.5 now fully supports the HTML 5 offline resource specification.
- [Drag and drop](/Web/API/HTML_Drag_and_Drop_API)
...
- [Downloadable fonts support](/Web/CSS/@font-face)
...
- [CSS media queries](/Web/CSS/CSS_media_queries/Using_media_queries)
...
- `ch` units for length
  - : The `ch` unit can now be used anywhere that accepts a [unit of length](/Web/CSS/length#units). `1ch` is the width of the "0" (zero) character.
- {{ cssxref("opacity") }}
...
- [localStorage](/Web/API/Web_Storage_API#localstorage)
...
- [Using geolocation](/Web/API/Geolocation_API)
...
- [Mouse gesture events](https://web.archive.org/web/20210724051233/https://developer.mozilla.org/Web/Events/Mouse_gesture_events)
  - : Firefox 3.5 supports mouse gesture events such as trackpad swipes.
- [The `NodeIterator` object](/Web/API/NodeIterator)
...
- [New in JavaScript 1.8.1](https://web.archive.org/web/20200805002842/https://developer.mozilla.org/Archive/Web/JavaScript/New_in_JavaScript/1.8.1)
  - : An overview of all the changes in JavaScript 1.8.1.
- [`Object.getPrototypeOf()`](/Web/JavaScript/Reference/Global_Objects/Object/getPrototypeOf)
  - : This new method returns the prototype of a specified object.
- Using native JSON
  - : Firefox 3.5 has native support for [JSON](/Glossary/JSON).
- New trim methods on the String object
  - : The [`String`](/Web/JavaScript/Reference/Global_Objects/String) object now has [`trim()`](/Web/JavaScript/Reference/Global_Objects/String/trim), [`trimLeft()`](/Web/JavaScript/Reference/Global_Objects/String/trimStart), and [`trimRight()`](/Web/JavaScript/Reference/Global_Objects/String/trimEnd) methods.

#### Networking

- Cross-site access controls for HTTP
  - : In Firefox 3.5, it's now possible for HTTP requests, including those made by [`XMLHttpRequest`](/Web/API/XMLHttpRequest), to work across domains if the server supports it.
- [Progress events for `XMLHttpRequest`](/Web/API/XMLHttpRequest_API/Using_XMLHttpRequest#monitoring_progress)
...
- [HTML 5 text API for `canvas` elements](/Web/API/Canvas_API/Tutorial/Drawing_text)
  - : Canvas elements now support the HTML 5 text API.
- [Shadow effects in a `canvas`](/Web/API/Canvas_API/Tutorial/Applying_styles_and_colors#shadows)
  - : Canvas shadow effects are now supported.
- [`createImageData()`](/Web/API/Canvas_API/Tutorial/Pixel_manipulation_with_canvas#creating_an_imagedata_object)
...
- `moz-opaque` attribute
  - : Added the [`moz-opaque`](/Web/HTML/Element/canvas#moz-opaque) attribute, which lets the canvas know whether or not translucency will be a factor. If the canvas knows there's no translucency, painting performance can be optimized. See also {{domxref("HTMLCanvasElement.mozOpaque")}}.
...
- [ICC color correction in Firefox](/Mozilla/Firefox/Releases/3.5/ICC_color_correction_in_Firefox)
  - : Firefox 3.5 now supports ICC color correction for tagged images.
- The `defer` attribute is now supported on [`script`](/Web/HTML/Element/script) elements
...
- The Text node's [`wholeText`](/Web/API/Text/wholeText) property and `Text.replaceWholeText()` method have been implemented.
- The property [`element.children`](/Web/API/Element/children) has been added. It returns a _collection_ of child elements of the given element.
- The property {{domxref("HTMLElement.contentEditable")}} is now supported, to support editable elements.
- The Element Traversal API is now supported by the DOM [Element](/Web/API/Element) object.
- HTML document nodes may now be cloned using [`cloneNode()`](/Web/API/Node/cloneNode).
- The non-standard `getBoxObjectFor()` DOM method has been removed. You should be using [`getBoundingClientRect()`](/Web/API/Element/getBoundingClientRect) instead.
...
- In non-chrome scope, caught objects in exceptions are now the actual thrown object instead of an [`XPConnect`](https://web.archive.org/web/20210423135742/https://developer.mozilla.org/Mozilla/Tech/XPCOM/Language_bindings/XPConnect) wrapper containing the thrown object.
- SVG ID references are now live.
- SVG filters now work for `foreignObject`.
- The `GetSVGDocument()` method has been added to [`object`](/Web/HTML/Element/object) and [`iframe`](/Web/HTML/Element/iframe) elements for compatibility.
...
If you're an extension developer, you should start by reading [Updating extensions for Firefox 3.5](/Mozilla/Firefox/Releases/3.5/Updating_extensions), which offers a helpful overview of what changes may affect your extension.

#### New components and functionality

- [Supporting private browsing mode](https://web.archive.org/web/20191029190431/https://developer.mozilla.org/Archive/Mozilla/Supporting_private_browsing_mode)
...
- [Security changes in Firefox 3.5](/Mozilla/Firefox/Releases/3.5/Security_changes)
  - : This article covers security-related changes in Firefox 3.5.
- [Theme changes in Firefox 3.5](https://web.archive.org/web/20191004004454/https://developer.mozilla.org/Archive/Themes/Theme_changes_in_Firefox_3.5)
...
- In addition, `keyset`s can now be removed using the node's [`removeChild()`](/Web/API/Node/removeChild) method.
