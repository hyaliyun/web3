...
- Events are now dispatched on disabled HTML elements, i.e. {{htmlelement("button")}}, {{htmlelement("fieldset")}}, , {{htmlelement("select")}}, and {{htmlelement("textarea")}} elements with `disabled` attributes set on them ([Firefox bug 329509](https://bugzil.la/329509)).
...
- We have added support for the [`referrerpolicy`](/Web/HTML/Element/script#referrerpolicy) attribute on {{htmlelement("script")}} elements ([Firefox bug 1460920](https://bugzil.la/1460920)).
...
- The [`keyup`](/Web/API/Element/keyup_event) and [`keydown`](/Web/API/Element/keydown_event) events are now fired during IME composition, to improve cross-browser compatibility for CJKT users ([Firefox bug 354358](https://bugzil.la/354358).
...
- The {{domxref("ServiceWorkerContainer.register()")}} and {{domxref("WorkerGlobalScope.importScripts()")}} (when used by a service worker) methods will now accept any files with a valid [JavaScript MIME type](/Web/HTTP/Basics_of_HTTP/MIME_types#textjavascript) ([Firefox bug 1354577](https://bugzil.la/1354577)).
...
- [Mutation events](/Web/API/MutationEvent) have been disabled in shadow trees ([Firefox bug 1489858](https://bugzil.la/1489858)).
...
- Support for [WebP](/Glossary/WebP) images has been added ([Firefox bug 1294490](https://bugzil.la/1294490)).
...
- The `headerURL`/`theme_frame` properties for [WebExtension themes](/Mozilla/Add-ons/WebExtensions/manifest.json/theme) are now supported on Firefox for Android ([Firefox bug 1429488](https://bugzil.la/1429488)).
