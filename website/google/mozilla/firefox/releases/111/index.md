...
- The [`autocapitalize`](/Web/HTML/Global_attributes/autocapitalize) global attribute is now supported by default. The default value for the attribute is `none`, so no capitalization occurs ([Firefox bug 1692007](https://bugzil.la/1692007)).
- The [`translate`](/Web/HTML/Global_attributes/translate) global attribute is now supported ([Firefox bug 1418449](https://bugzil.la/1418449)).
...
- CSS color functions `color()`, `lab()`, `lch()`, `oklab()`, and `oklch()` are now supported.
  ...
  For more information, see the [CSS color value](/Web/CSS/color_value) documentation ([Firefox bug 1352757](https://bugzil.la/1352757) and [Firefox bug 1128204](https://bugzil.la/1128204)).
...
- The `context-stroke` and `context-fill` values are now supported inside `<marker>` elements.
  For more information on using these values with `fill` and `stroke` properties, see the [`<marker>`](/Web/SVG/Element/marker) documentation ([Firefox bug 752638](https://bugzil.la/752638)).

### HTTP

- The HTTP [`Authorization`](/Web/HTTP/Headers/Authorization) header is removed from cross origin redirects.
  See [Firefox bug 1802086](https://bugzil.la/1802086) for more details.

### APIs

- [Origin private file system (OPFS)](/Web/API/File_System_API/Origin_private_file_system) is now supported when using the [File System API](/Web/API/File_System_API).
...
- The HTTP [`Authorization`](/Web/HTTP/Headers/Authorization) header is removed from [`fetch()`](/Web/API/Window/fetch) and [`XMLHttpRequest`](/Web/API/XMLHttpRequest) requests that are redirected cross-origin (`fetch()` headers may be added by developers using the [`option.headers`](/Web/API/Window/fetch#headers) argument).
  See [Firefox bug 1802086](https://bugzil.la/1802086) for more details.
- The `none` value of the `options.imageOrientation` parameter passed to [`createImageBitmap()`](/Web/API/createImageBitmap) has been renamed to [`from-image`](/Web/API/createImageBitmap#from-image).
  This is to better match the meaning of the equivalent CSS [`image-orientation`](/Web/CSS/image-orientation) property. ([Firefox bug 1809740](https://bugzil.la/1809740)).
...
- [`RTCInboundRtpStreamStats.trackIdentifier`](/Web/API/RTCInboundRtpStreamStats#trackidentifier) is now supported.
