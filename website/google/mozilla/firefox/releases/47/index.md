...
- [User-agent spoofing](/Web/HTTP/Browser_detection_using_the_user_agent) from the [Responsive mode](https://firefox-source-docs.mozilla.org/devtools-user/responsive_design_mode/index.html)
...
- [Service workers](/Web/API/ServiceWorker) and [Push API](/Web/API/Push_API) debugging
  ...
  - Support for [cache storage](/Web/API/Cache) in [Storage Inspector](https://firefox-source-docs.mozilla.org/devtools-user/storage_inspector/index.html)
...
- The case-insensitive modifier `i` (like in `[foo=bar i]`) for [attribute selectors](/Web/CSS/Attribute_selectors) has been implemented ([Firefox bug 888190](https://bugzil.la/888190)).
...
- As per the new ES2016 specification, the {{jsxref("Proxy")}} enumerate trap for [`for...in`](/Web/JavaScript/Reference/Statements/for...in) statements has been removed ([Firefox bug 1246318](https://bugzil.la/1246318)).
...
- [Service workers](/Web/API/Service_Worker_API) and [Push](/Web/API/Push_API) have been disabled in the [Firefox 45 Extended Support Release](https://www.mozilla.org/en-US/firefox/enterprise/) (ESR) ([Firefox bug 1232029](https://bugzil.la/1232029)).

#### WebRTC

- Support for the `RTCIceServer` dictionary has been updated in keeping with revisions to the WebGL 1.0 specification by adding support for the [credentialType](/Web/API/RTCPeerConnection/RTCPeerConnection#credentialtype) property. This property is a string that specifies whether the credential is a password or a token. Currently, Firefox only supports `"password"`.
...
- [Web Crypto API](/Web/API/Web_Crypto_API) methods can now use the RSA-PSS cryptographic algorithm ([Firefox bug 1191936](https://bugzil.la/1191936)).
- The [Permissions API](/Web/API/Permissions_API) has had the {{domxref("Permissions.revoke()")}} method added ([Firefox bug 1197461](https://bugzil.la/1197461)).
...
The FUEL JavaScript library, introduced back in Firefox 3, **has been removed**. This library was designed to aid in add-on development and with the introduction of the Add-on SDK and, now, by [WebExtensions](/Mozilla/Add-ons/WebExtensions) support, is no longer useful. ([Firefox bug 1090880](https://bugzil.la/1090880))
