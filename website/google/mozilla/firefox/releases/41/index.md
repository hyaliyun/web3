...
- The [`crossorigin`](/Web/HTML/Element/link#crossorigin) attribute is now supported for [\<link rel='preconnect'>](/Web/HTML/Element/link) ([Firefox bug 1174152](https://bugzil.la/1174152)).
- The picture element does not react to resize/viewport changes ([Firefox bug 1135812](https://bugzil.la/1135812)).

### JavaScript

- `Date.prototype` is now an ordinary object, not a  instance anymore ([Firefox bug 861219](https://bugzil.la/861219)).
...
- Per ES2015, curly braces are required for [method definitions](/Web/JavaScript/Reference/Functions/Method_definitions). Syntax without them will fail from now on ([Firefox bug 1150855](https://bugzil.la/1150855)).
- [Method definitions](/Web/JavaScript/Reference/Functions/Method_definitions) (except for generator methods) are not constructable anymore ([Firefox bug 1059908](https://bugzil.la/1059908) and [Firefox bug 1166950](https://bugzil.la/1166950)).
...
- The [`new.target`](/Web/JavaScript/Reference/Operators/new.target) syntax has been added ([Firefox bug 1141865](https://bugzil.la/1141865)).
...
- Improvement to our experimental [Service Worker](/Web/API/Service_Worker_API) implementation:
...
- {{domxref("MessagePort")}} and {{domxref("MessageChannel")}} now available in [Web workers](/Web/API/Web_Workers_API), and are enabled by default in all contexts ([Firefox bug 952139](https://bugzil.la/952139)) and ([Firefox bug 911972](https://bugzil.la/911972)).
- The User Timing API is now available in [Web workers](/Web/API/Web_Workers_API) ([Firefox bug 1155761](https://bugzil.la/1155761)).
- The [Notifications API](/Web/API/Notifications_API) is now available in [Web workers](/Web/API/Web_Workers_API) ([Firefox bug 916893](https://bugzil.la/916893)).
- `DOMRequest` and `DOMCursor` are now available in [Web workers](/Web/API/Web_Workers_API) ([Firefox bug 1167650](https://bugzil.la/1167650)).
- The [CSS Font Loading API](/Web/API/CSS_Font_Loading_API) has been completely implemented and is now enabled by default ([Firefox bug 1149381](https://bugzil.la/1149381)).
...
- The [CSP](/Web/HTTP/CSP) 1.1 `manifest-src` [directive](/Web/HTTP/Headers/Content-Security-Policy) is now supported ([Firefox bug 1089255](https://bugzil.la/1089255)).
- Previous versions of Firefox incorrectly expected the [Content Security Policy](/Web/HTTP/CSP) [referrer](/Web/HTTP/Headers/Content-Security-Policy#referrer) directive's value `origin-when-cross-origin` to be spelled `origin-when-crossorigin`. This has been corrected to include the missing dash character.
