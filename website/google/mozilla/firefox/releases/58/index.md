...
- Budget-based background timeout throttling has been implemented — see [Policies in place to aid background page performance](/Web/API/Page_Visibility_API#policies_in_place_to_aid_background_page_performance) for more details ([Firefox bug 1377766](https://bugzil.la/1377766)).
...
- The [WebVTT](/Web/API/WebVTT_API) {{domxref("VTTRegion")}} interface has always been created when interpreting WebVTT files, but the resulting regions were not previously utilized. Starting in Firefox 58, they are, if you enable the preference `media.webvtt.regions.enabled` by setting its value to `true`.
...
- [`frame-ancestors`](/Web/HTTP/Headers/Content-Security-Policy/frame-ancestors) is no longer ignored in {{httpheader("Content-Security-Policy-Report-Only")}} ([Firefox bug 1380755](https://bugzil.la/1380755)).
...
- The [`worker-src`](/Web/HTTP/Headers/Content-Security-Policy/worker-src) CSP directive has been implemented ([Firefox bug 1302667](https://bugzil.la/1302667)).
- The [425: Too Early](/Web/HTTP/Status/425) status code and related {{httpheader("Early-Data")}} request header are now supported ([Firefox bug 1406908](https://bugzil.la/1406908)).
...
- "Add to home screen" is now supported in Firefox for Android, part of the [Progressive Web Apps](/Web/Progressive_web_apps) effort ([Firefox bug 1212648](https://bugzil.la/1212648)).
- [WebAssembly](/WebAssembly) now has a tiered compiler providing load time optimizations ([Firefox bug 1277562](https://bugzil.la/1277562)), and new streaming APIs — [`WebAssembly.compileStreaming()`](/WebAssembly/JavaScript_interface/compileStreaming_static) and [`WebAssembly.installStreaming()`](/WebAssembly/JavaScript_interface/installStreaming) [Firefox bug 1347644](https://bugzil.la/1347644).
...
- The non-standard [`Date.prototype.toLocaleFormat()`](/Web/JavaScript/Reference/Deprecated_and_obsolete_features#date_2) method has been removed ([Firefox bug 818634](https://bugzil.la/818634)).
- The non-standard and deprecated {{jsxref("Object.prototype.watch()")}} and {{jsxref("Object.prototype.unwatch", "unwatch()")}} methods have been removed and will no longer work ([Firefox bug 638054](https://bugzil.la/638054)). Consider using [setters and getters](/Web/JavaScript/Guide/Working_with_objects#defining_getters_and_setters) or [proxies](/Web/JavaScript/Reference/Global_Objects/Proxy) instead.
- The [legacy Iterator protocol](/Web/JavaScript/Reference/Deprecated_and_obsolete_features#legacy_generator_and_iterator), the [`StopIteration`](/Archive/Web/StopIteration) object, the [legacy generator functions](/Web/JavaScript/Reference/Deprecated_and_obsolete_features) and the non-standard {{jsxref("Function.prototype.isGenerator()")}} method have been removed. Use the ES2015 [iteration protocols](/Web/JavaScript/Reference/Iteration_protocols) and standards-compliant [iterators and generators](/Web/JavaScript/Guide/Iterators_and_generators) instead ([Firefox bug 1083482](https://bugzil.la/1083482), [Firefox bug 1413867](https://bugzil.la/1413867), [Firefox bug 1119777](https://bugzil.la/1119777)).
- The non-standard [Array comprehensions](/Web/JavaScript/Reference/Deprecated_and_obsolete_features) and [Generator comprehensions](/Web/JavaScript/Reference/Deprecated_and_obsolete_features) have been removed ([Firefox bug 1414340](https://bugzil.la/1414340)).
...
- The `dom.abortController.enabled` and `dom.abortController.fetch.enabled` prefs that controlled exposure of the [Abort API functionality](/Web/API/Fetch_API#aborting_a_fetch) have now been removed, since those features are now enabled by default ([Firefox bug 1402317](https://bugzil.la/1402317)).
...
- [browserSettings](/Mozilla/Add-ons/WebExtensions/API/browserSettings)
  ...
  - [browserSettings.webNotificationsDisabled](/Mozilla/Add-ons/WebExtensions/API/browserSettings/webNotificationsDisabled) has been implemented (bug 1364942)

- [browsingData](/Mozilla/Add-ons/WebExtensions/API/browsingData)

  - [browsingData.localStorage](/Mozilla/Add-ons/WebExtensions/API/browsingData/removeLocalStorage) now supports deleting localStorage by host (bug 1388428)

- [pkcs11](/Mozilla/Add-ons/WebExtensions/API/pkcs11) API to manage security devices (Bug 1357391)
- privacy
...
- tabs
  ...
  - [tabs.discard](/Mozilla/Add-ons/WebExtensions/API/tabs/discard) has been implemented (Bug 1322485)
  - isArticle, isInReaderMode properties of Tab implemented (Bug 1381992)
  - [toggleReaderMode](/Mozilla/Add-ons/WebExtensions/API/tabs/toggleReaderMode)() method implemented (Bug 1381992)
...
- theme
  ...
  - [getCurrent](/Mozilla/Add-ons/WebExtensions/API/theme/getCurrent)() method to obtain current theme properties (Bug [1349944](https://bugzil.la/1349944))
...
- webRequest
  ...
  - [webRequest.onBeforeRequest](/Mozilla/Add-ons/WebExtensions/API/webRequest/onBeforeRequest) now includes a "frameAncestors" parameter
