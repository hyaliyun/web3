...
- The [`dirname`](/Web/HTML/Element/input#dirname) attribute is now supported on [`input`](/Web/HTML/Element/input#dirname) and [`textarea`](/Web/HTML/Element/textarea#dirname) elements.
...
- [`Intl.NumberFormat`](/Web/JavaScript/Reference/Global_Objects/Intl/NumberFormat) supports new [constructor options](/Web/JavaScript/Reference/Global_Objects/Intl/NumberFormat/NumberFormat) that control how numbers are rounded (`roundingIncrement`, `roundingMode`, `roundingPriority`), the strategy for displaying trailing zeros on whole numbers (`trailingZeroDisplay`), and whether to use grouping separators to indicate thousands, millions, and so on (`useGrouping`).
  It also supports new methods [`formatRange()`](/Web/JavaScript/Reference/Global_Objects/Intl/NumberFormat/formatRange) amd [`formatRangeToParts()`](/Web/JavaScript/Reference/Global_Objects/Intl/NumberFormat/formatRangeToParts) for formatting ranges of numbers.
  ([Firefox bug 1795756](https://bugzil.la/1795756)).
- [`Intl.PluralRules`](/Web/JavaScript/Reference/Global_Objects/Intl/PluralRules) was updated (as part of the same set of changes as `Intl.NumberFormat`) to support [constructor options](/Web/JavaScript/Reference/Global_Objects/Intl/PluralRules/PluralRules) `roundingIncrement`, `roundingMode`, `roundingPriority` and `trailingZeroDisplay`, and the [`selectRange()`](/Web/JavaScript/Reference/Global_Objects/Intl/PluralRules/selectRange) method.
  ([Firefox bug 1795756](https://bugzil.la/1795756)).

### SVG

- The `q` [length unit](/Web/SVG/Content_type#length) (`1q = 1/40th of 1cm`) is now supported ([Firefox bug 1836995](https://bugzil.la/1836995)).

### HTTP

- Configuring a [Content-Security-Policy](/Web/HTTP/CSP) now supports specifying [external JavaScript files to be allowlisted using hashes](/Web/HTTP/Headers/Content-Security-Policy/script-src#allowlisting_external_scripts_using_hashes), where previously only inline scripts could be allowlisted using a hash ([Firefox bug 1409200](https://bugzil.la/1409200)).
...
- The [Audio Output Devices API](/Web/API/Audio_Output_Devices_API) is now supported on all platforms except for Android.
  ...
  Affected APIs include {{domxref("MediaDevices.selectAudioOutput()")}}, {{domxref("MediaDevices.enumerateDevices()")}}, [`HTMLMediaElement.setSinkId()`](/Web/API/HTMLMediaElement/setSinkId), [`HTMLMediaElement.sinkId`](/Web/API/HTMLMediaElement/sinkId), and the permission policy [`Permissions-Policy: speaker-selection`](/Web/HTTP/Headers/Permissions-Policy/speaker-selection) ([Firefox bug 1498512](https://bugzil.la/1498512)).
...
- Added support for [capability matching](/Web/WebDriver/Capabilities) for the `session.new` command. It allows to define expectations about the target browser, such as browser name, platform name, etc. It can also be used to configure the session, for example, to specify if insecure certificates should be accepted ([Firefox bug 1731730](https://bugzil.la/1731730)).
