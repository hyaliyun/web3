...
- You can now [specify the start and stop time of media](/Learn/HTML/Multimedia_and_embedding/Video_and_audio_content#specifying_playback_range) in the URI of the media when using {{ HTMLElement("audio") }} and {{ HTMLElement("video") }} elements.
- {{ HTMLElement("input") }} and {{ HTMLElement("textarea") }} elements [now respect the value of the `lang` attribute](/Web/HTML/Global_attributes/spellcheck#controlling_the_spellchecker_language) when invoking the spell checker.
...
- Drawing images that use the [`crossorigin`](/Web/HTML/Attributes/crossorigin) attribute to request CORS access no longer incorrectly [taint the canvas](/Web/HTML/CORS_enabled_image#what_is_a_.22tainted.22_canvas.3f) when CORS is granted.
- The value of the [`rowspan`](/Web/HTML/Element/td#rowspan) attribute may now be as large as 65,534, up from 8190.
...
- When a stylesheet included using the {{ HTMLElement("link") }} element has been fully loaded and parsed (but not yet applied to the document), a [`load` event](/Web/HTML/Element/link#stylesheet_load_events) is now fired. Also, if an error occurs processing a style sheet, an `error` event is fired.
...
- [Using fullscreen mode](/Web/API/Fullscreen_API)
...
- DOM Level 3 [composition events](/Web/API/CompositionEvent) are now supported.
...
- The set of events that can be listened for on {{ HTMLElement("body") }} elements has been revised to match the latest draft of the HTML5 specification. The list of events in the [DOM event reference](/Web/Events) reflects which events can be listened for on {{ HTMLElement("body") }}.
...
- A new response type, "`moz-json`", has been added to `XMLHttpRequest`, letting `XMLHttpRequest` automatically parse [JSON](/Glossary/JSON) strings for you; when you request this type, a returned JSON string is parsed, so that the value of the `response` property is the resulting JavaScript object.
- [`XMLHttpRequest` "progress" events](/Web/API/XMLHttpRequest_API/Using_XMLHttpRequest#monitoring_progress) are now reliably sent for every chunk of data received; in the past it was possible for the last chunk of data received to not fire a "progress" event. Now you can track progress by following only "progress" events, instead of also having to monitor "load" events to detect the receipt of the last chunk of data.
- In the past, calling [`addEventListener()`](/Web/API/EventTarget/addEventListener) with a `null` listener would throw an exception. Now it returns without error and with no effect.
...
- The value of {{ domxref("Node.ownerDocument") }} for doctype nodes is now the document on which [`createDocumentType()`](/Web/API/DOMImplementation/createDocumentType) was called to create the node, instead of `null`.
...
- The [WebGL](/Web/API/WebGL_API) context `drawingBufferWidth` and `drawingBufferHeight` attributes are now supported.
...
- You can now send the contents of [JavaScript typed arrays](/Web/JavaScript/Guide/Typed_arrays) (that is, the contents of an [`ArrayBuffer`](/Web/JavaScript/Reference/Global_Objects/ArrayBuffer) object) [using XMLHttpRequest](/Web/API/XMLHttpRequest_API/Using_XMLHttpRequest#sending_typed_arrays_as_binary_data).
...
- Attempts by a server to use the `301 Moved Permanently` or `307 Temporary Redirect` response codes to redirect the user to a `javascript:` URI now [result in a "bad connection" error](/Web/HTTP#more_on_redirection_responses) instead of actually redirecting. This prevents certain types of cross-site scripting attacks.
...
See [Updating add-ons for Firefox 9](/Mozilla/Firefox/Releases/9/Updating_add-ons) for an overview of the changes you may need to make to get your add-ons working in Firefox 9.
...
- You can now use a `<xul:panel>` as a drag image for DOM drag and drop operations. This lets you use the standard drag & drop API for [drag and drop of XUL content](/Web/API/HTML_Drag_and_Drop_API/Drag_operations#using_xul_panels_as_drag_images).
