...
- The {{HTMLElement("textarea")}} element's [`autocomplete`](/Web/HTML/Element/textarea#autocomplete) attribute has been implemented. This lets you enable or disable form auto-fill for the element.
...
- {{cssxref("position")}} `sticky` is now supported on appropriate [HTML table](/Learn/HTML/Tables) parts (e.g. {{htmlelement("th")}} elements) ([Firefox bug 975644](https://bugzil.la/975644)).
- {{cssxref("calc", "calc()")}} is now supported in  values — `rgb()`, `rgba()`, `hsl()`, and `hsla()` ([Firefox bug 984021](https://bugzil.la/984021)).
- {{cssxref("calc", "calc()")}} in [media query](/Web/CSS/CSS_media_queries) values is now supported [Firefox bug 1396057](https://bugzil.la/1396057).
...
- The service worker [Clients API](/Web/API/Clients) can now find and communicate with windows in a separate browser process ([Firefox bug 1293277](https://bugzil.la/1293277)).
...
- Support for [WebVTT](/Web/API/WebVTT_API) regions was implemented in Firefox 58 but disabled by default. They're now available by default ([Firefox bug 1415805](https://bugzil.la/1415805)).
...
The non-standard `version` parameter of the {{htmlelement("script")}} element's [`type`](/Web/HTML/Element/script#type) attribute (e.g. `type="application/javascript;version=1.8"`) has been removed ([Firefox bug 1428745](https://bugzil.la/1428745)).
...
- Non-standard [conditional catch clauses](/Web/JavaScript/Reference/Statements/try...catch#conditional_catch_clauses) have been removed ([Firefox bug 1228841](https://bugzil.la/1228841)).
...
- The proprietary [`Navigator.mozNotification`](/Archive/API/Navigator/mozNotification) property and `DesktopNotification` interface have been removed, in favor of the standard [Notifications API](/Web/API/Notifications_API) ([Firefox bug 952453](https://bugzil.la/952453)).
...
- [Theme](/Mozilla/Add-ons/WebExtensions/manifest.json/theme) updates:
...
- New [browser settings](/Mozilla/Add-ons/WebExtensions/API/browserSettings):

  - [`contextMenuShowEvent`](/Mozilla/Add-ons/WebExtensions/API/browserSettings/contextMenuShowEvent)
  - [`openBookmarksInNewTabs`](/Mozilla/Add-ons/WebExtensions/API/browserSettings/openBookmarksInNewTabs)
  - [`openSearchResultsInNewTabs`](/Mozilla/Add-ons/WebExtensions/API/browserSettings/openSearchResultsInNewTabs)
  - [`proxyConfig`](/Mozilla/Add-ons/WebExtensions/API/proxy/settings)

- New [`tabs`](/Mozilla/Add-ons/WebExtensions/API/tabs) APIs:

  - [`tabs.captureTab()`](/Mozilla/Add-ons/WebExtensions/API/tabs/captureTab)
  - [`tabs.hide()`](/Mozilla/Add-ons/WebExtensions/API/tabs/hide)
  - [`tabs.show()`](/Mozilla/Add-ons/WebExtensions/API/tabs/show)

- The [`contextMenus`](/Archive/Add-ons/Legacy_Firefox_for_Android/API/NativeWindow/contextmenus) API now supports a ["bookmark" context](/Mozilla/Add-ons/WebExtensions/API/menus/ContextType).
- New [`contentScripts`](/Mozilla/Add-ons/WebExtensions/API/contentScripts) API enables runtime registration of content scripts.
- New [`pageAction`](/Mozilla/Add-ons/WebExtensions/API/pageAction), [`browserAction`](/Mozilla/Add-ons/WebExtensions/API/browserAction), [`SidebarAction`](/Mozilla/Add-ons/WebExtensions/API/sidebarAction) APIs:

  - `browserAction/pageAction/sidebarAction.set*` functions now accept `null` to undo changes.
  - [`browserAction.isEnabled()`](/Mozilla/Add-ons/WebExtensions/API/browserAction/isEnabled), [`pageAction.isShown()`](/Mozilla/Add-ons/WebExtensions/API/pageAction/isShown), [`sidebarAction.isOpen()`](/Mozilla/Add-ons/WebExtensions/API/sidebarAction/isOpen) functions.

- New option in [`page_action`](/Mozilla/Add-ons/WebExtensions/manifest.json/page_action) to show page actions by default.
...
- New [`privacy.websites`](/Mozilla/Add-ons/WebExtensions/API/privacy/websites) setting "cookieConfig".
- Support in [`cookies`](/Mozilla/Add-ons/WebExtensions/API/cookies) API for [first-party isolation](/Mozilla/Add-ons/WebExtensions/API/cookies#first-party_isolation).
- New option `upgradeToSecure` in [`webRequest`](/Mozilla/Add-ons/WebExtensions/API/webRequest).
