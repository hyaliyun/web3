...
>
> The best way to request resources over HTTP is to use [`fetch()`](/Web/API/Window/fetch), which allows you to specify the priority in [`Request.priority`](/Web/API/Request/priority).
> You can also set the HTTP priority on [`HTMLLinkElement`](/Web/API/HTMLLinkElement/fetchPriority), [`HTMLIFrameElement`](/Web/API/HTMLIFrameElement), and [`HTMLImageElement`](/Web/API/HTMLImageElement/fetchPriority) elements (and associated tags) using the `fetchpriority` attribute.

In [Firefox 1.5](/Mozilla/Firefox/Releases/1.5), an API was added to support changing the priority of [HTTP](/Web/HTTP) requests. Prior to this, there was no way to directly indicate that a request was of a different priority. The API is defined in `nsISupportsPriority`, but is defined in very generic terms so that any object can implement this interface to enable the concept of priority. This article deals specifically with using that interface to change the priority of HTTP requests.
...
The examples in this document are all written in [JavaScript](/Web/JavaScript) using XPCOM.
...
> [!NOTE]
> This example uses a synchronous [XMLHttpRequest](/Web/API/XMLHttpRequest), which you should not use in practice.
