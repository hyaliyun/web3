...
The first step — and, for most extensions, the only one that will be needed — is to update the [install manifest](/Install_Manifests) file, [`install.rdf`](/Creating_a_Skin_for_Firefox/install.rdf), to indicate compatibility with Firefox 3.
...
Nodes from external documents should be cloned using [`document.importNode()`](/Web/API/Document/importNode) (or adopted using [`document.adoptNode()`](/Web/API/Document/adoptNode)) before they can be inserted into the current document. For more on the [`Node.ownerDocument`](/Web/API/Node/ownerDocument) issues, see the [W3C DOM FAQ](https://www.w3.org/DOM/faq.html#ownerdoc).

Firefox doesn't currently enforce this rule (it did for a while during the development of Firefox 3, but too many sites break when this rule is enforced). We encourage Web developers to fix their code to follow this rule for improved future compatibility.

### Bookmarks & History

If your extension accesses bookmark or history data in any way, it will need substantial work to be compatible with Firefox 3. The old APIs for accessing this information have been replaced by the new [Places](/Places) architecture. See the [Places migration guide](/Places_Developer_Guide) for details on updating your existing extension to use the Places API.

### Download Manager

The Download Manager API has changed slightly due to the transition from an RDF data store to using the [Storage](/Storage) API. This should be a pretty easy transition to make. In addition, the API for monitoring download progress has changed to support multiple download manager listeners. See `nsIDownloadManager`, `nsIDownloadProgressListener`, and [Monitoring downloads](/Monitoring_downloads) for more information.
...
- The article [Using nsILoginManager](/XPCOM_Interface_Reference/Using_nsILoginManager) includes examples, including a demonstration of how to write your extension to work with both the Password Manager and the Login Manager, so it will work with both Firefox 3 and earlier versions.
- `nsILoginInfo`
- `nsILoginManager`

You can also override the built-in password manager storage if you want to provide your own password storage implementation in your extensions. See [Creating a Login Manager storage module](/Creating_a_Login_Manager_storage_module) for details.

### Popups (Menus, Context Menus, Tooltips and Panels)

The XUL Popup system was heavily modified in Firefox 3. The Popup system includes main menus, context menus and popup panels. A guide to [using Popups](/XUL/PopupGuide) has been created, detailing how the system works. One thing to note is that `popup.showPopup` has been deprecated in favor of new `popup.openPopup` and `popup.openPopupAtScreen`.
...
- [Mozilla external string guide](/Mozilla_external_string_guide)
- [XPCOM Glue](/XPCOM_Glue)
- [Migrating from Internal Linkage to Frozen Linkage](/Migrating_from_Internal_Linkage_to_Frozen_Linkage)
...
Extensions that attempt to overlay onto the "appcontent" box try to float chrome over document content will no longer display that material. You should update your extension to use the new [`<xul:panel>`](/Mozilla/Tech/XUL/panel) XUL element. If you wish to keep the panel from automatically disappearing after a delay, you can set the `noautohide` attribute to `true`.
...
- The [`<xul:tabbrowser>`](/Mozilla/Tech/XUL/tabbrowser) element is no longer part of "toolkit" ([Firefox bug 339964](https://bugzil.la/339964)). This means this element is no longer available to XUL applications and extensions. It continues to be used in the main Firefox window (browser.xul).
...
- `event.preventBubble()` was deprecated in Firefox 2 and has been removed in Firefox 3. Use [`event.stopPropagation()`](/Web/API/Event/stopPropagation), which works in Firefox 2 as well.
...
- If your extension needs to allow an untrusted source (e.g., a website) to access the extension's chrome, then you must use the new [`contentaccessible` flag](/Chrome_Registration#contentaccessible).
