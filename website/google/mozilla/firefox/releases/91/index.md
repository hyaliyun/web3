...
- The [Gamepad API](/Web/API/Gamepad_API) now requires a [secure context](/Web/Security/Secure_Contexts) ([Firefox bug 1704005](https://bugzil.la/1704005)).

### APIs

#### DOM

- The [Visual Viewport API](/Web/API/Visual_Viewport_API) is now enabled by default on Firefox desktop releases (it has been enabled on Firefox for Android since version 68).
...
- The [Gamepad API](/Web/API/Gamepad_API) is now protected by {{httpheader('Feature-Policy/gamepad','Feature-Policy: gamepad')}}.
  If disallowed by the [Permission Policy](/Web/HTTP/Permissions_Policy), calls to {{domxref('Navigator.getGamepads()')}} will throw a `SecurityError` {{domxref('DOMException')}},
...
- Changing the playback speed of a media element ([`<video>`](/Web/HTML/Element/video) or [`<audio>`](/Web/HTML/Element/audio)) using the [`playbackRate`](/Web/API/HTMLMediaElement/playbackRate) attribute now works when the media element is captured to a [`MediaStream`](/Web/API/MediaStream) or via [`AudioContext.createMediaElementSource`](/Web/API/AudioContext/createMediaElementSource) ([Firefox bug 1517199](https://bugzil.la/1517199)).
