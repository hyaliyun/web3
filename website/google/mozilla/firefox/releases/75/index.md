...
- In the [inspector](https://firefox-source-docs.mozilla.org/devtools-user/page_inspector/index.html), you can now use [XPath](/Web/XPath) expressions to locate elements, in addition to locating elements using CSS selectors as before ([Firefox bug 963933](https://bugzil.la/963933)).
- You can now filter [WebSocket](/Web/API/WebSockets_API) messages using [regular expressions](/Web/JavaScript/Guide/Regular_expressions), in addition to plain text search, by writing the regular expression in slashes ([Firefox bug 1593837](https://bugzil.la/1593837)).

### HTML

- The [`loading`](/Web/HTML/Element/img#loading) attribute of the {{HTMLElement("img")}} element has now been implemented. This string value can be used to specify that the image should be [lazily loaded](/Web/Performance/Lazy_loading), by setting its value to `lazy` ([Firefox bug 1542784](https://bugzil.la/1542784)).
- The value of the [`<style>`](/Web/HTML/Element/style) element's `type` attribute is now restricted to `text/css` only, as per the spec ([Firefox bug 1614329](https://bugzil.la/1614329)).
...
New [ARIA](/Web/Accessibility/ARIA) roles and attributes are now exposed in Firefox, on Windows and Linux (bear in mind that these still won't be usable until screen readers start to support them):

- `aria-description` ([Firefox bug 1608961](https://bugzil.la/1608961)).
- [`role="mark"`](/Web/Accessibility/ARIA/Roles/mark_role) and [`role="suggestion"`](/Web/Accessibility/ARIA/Roles/suggestion_role) ([Firefox bug 1608965](https://bugzil.la/1608965)).
- [`role="comment"`](/Web/Accessibility/ARIA/Roles/comment_role) ([Firefox bug 1608969](https://bugzil.la/1608969)).
...
- [Public static class fields](/Web/JavaScript/Reference/Classes/static) are now supported ([Firefox bug 1535804](https://bugzil.la/1535804)).
- The [`Intl.Locale`](/Web/JavaScript/Reference/Global_Objects/Intl/Locale) class is now supported ([Firefox bug 1613713](https://bugzil.la/1613713)).
- The [`Function.caller`](/Web/JavaScript/Reference/Global_Objects/Function/caller) property has been updated to be aligned with the latest [ECMAScript spec proposal](https://github.com/claudepache/es-legacy-function-reflection). Previously throwing a `TypeError`, it now returns `null` if the caller is a strict, async, or generator function ([Firefox bug 1610206](https://bugzil.la/1610206)).
...
Firefox 75 sees numerous additions to the [Web Animations API](/Web/API/Web_Animations_API):

- [Implicit to/from keyframes](/Web/API/Web_Animations_API/Keyframe_Formats#implicit_tofrom_keyframes) are now supported, as is automatically removing filling animations that have been replaced by other indefinitely filling animations ([Firefox bug 1618773](https://bugzil.la/1618773)). This includes enabling of support for:

  - [`Animation.commitStyles()`](/Web/API/Animation/commitStyles)
  - [`Animation.onremove`](/Web/API/Animation/remove_event)
  - [`Animation.persist()`](/Web/API/Animation/persist)
  - [`Animation.replaceState`](/Web/API/Animation/replaceState)

- The [`Animation.timeline`](/Web/API/Animation/timeline) getter, [`Document.timeline`](/Web/API/Document/timeline), [`DocumentTimeline`](/Web/API/DocumentTimeline), and [`AnimationTimeline`](/Web/API/AnimationTimeline) features are now enabled by default ([Firefox bug 1619178](https://bugzil.la/1619178)).
- The [`Document.getAnimations()`](/Web/API/Document/getAnimations) and [`Element.getAnimations()`](/Web/API/Element/getAnimations) methods are now enabled by default ([Firefox bug 1619821](https://bugzil.la/1619821)).

#### Media, Web Audio, and WebRTC

- The [`RTCPeerConnection.setLocalDescription()`](/Web/API/RTCPeerConnection/setLocalDescription) method can now be called without arguments, in which case the WebRTC runtime will try to create the new local session description itself ([Firefox bug 1568292](https://bugzil.la/1568292)).
...
- [CSP](/Web/HTTP/CSP) nonces from non-script sources, such as CSS selectors, and `.getAttribute("nonce")` calls, are now hidden. Instead, check the [`.nonce`](/Web/API/HTMLElement/nonce) property to access nonces from scripts ([Firefox bug 1374612](https://bugzil.la/1374612)).
