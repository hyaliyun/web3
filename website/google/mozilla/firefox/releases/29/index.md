...
- Added the [console API](/Web/API/Console_API) to Web Workers ([bug 620935](https://bugzil.la/620935)). Now you can log messages to the Web Console from Web Workers.
...
- [CSS variables](/Web/CSS/Using_CSS_custom_properties) have been implemented ([Firefox bug 773296](https://bugzil.la/773296)). Mozilla Hacks article can be found [here](https://hacks.mozilla.org/2013/12/css-variables-in-firefox-nightly/). They are enabled by default only for non-release builds (on release builds flip the pref `layout.css.variables.enabled` to `true` if you want to play with them).
...
- The [ECMAScript Internationalization API (ECMA-402)](https://402.ecma-international.org/1.0/) has been implemented and is now enabled by default in Firefox Desktop ([Firefox bug 853301](https://bugzil.la/853301)):
  ...
  - The following methods of {{jsxref("String")}}, {{jsxref("Number")}} and  have been updated to include the `locales` and `options` arguments per ECMA-402:
...
- Completed [generators](/Web/JavaScript/Reference/Statements/function*) now return an `IteratorResult` object instead of throwing ([Firefox bug 958951](https://bugzil.la/958951)).
...
- The [Gamepad API](/Web/API/Gamepad_API/Using_the_Gamepad_API) is enabled by default ([Firefox bug 878828](https://bugzil.la/878828)).
