...
- Firefox now displays the [label value](/Web/HTML/Element/option#browser_compatibility) on an {{HTMLElement("option")}} element when the element's content is empty. ([Firefox bug 40545](https://bugzil.la/40545))
...
- The following permissions are now optional, they can be specified in the [`optional_permissions`](/Mozilla/Add-ons/WebExtensions/manifest.json/optional_permissions) manifest key and requested using the {{WebExtAPIRef("permissions")}} API:
...
- Changes related to [`SameSite` cookies](/Web/HTTP/Headers/Set-Cookie#samesitesamesite-value) mean that when setting [host permissions](/Mozilla/Add-ons/WebExtensions/manifest.json/permissions#host_permissions) to bypass tracking protection for extension pages hosts must be specified as a full domain or with wildcards. However, for Content scripts tracking protection can only be bypassed for hosts specified with a full domain.
