...
- The {{ HTMLElement("body") }} element's [`background`](/Web/HTML/Element/body#background) attribute is no longer resolved as a URI; this is in compliance with the current HTML specification.
- The {{ HTMLElement("option") }} element's [`label`](/Web/HTML/Element/option#label) attribute now reflects the value of the element's text content if the attribute isn't specified.
...
- The [`isPointInPath()`](/Web/API/CanvasRenderingContext2D/isPointInPath) method now correctly considers the transformation matrix when comparing the specified point to the current path.
- Calling `strokeRect()` with a zero width and height now correctly does nothing.
- Calling [`drawImage()`](/Web/API/CanvasRenderingContext2D/drawImage) with a zero width or height {{ HTMLElement("canvas") }} now throws `INVALID_STATE_ERR`.
- Calling [`drawImage()`](/Web/API/CanvasRenderingContext2D/drawImage) with non-finite coordinates no longer throws an exception.
...
- [Shadows](/Web/API/Canvas_API/Tutorial/Applying_styles_and_colors#shadows) are now only drawn for `source-over` compositing operations.
- You can now configure the fill rule used by canvas by setting the `mozFillRule` [attribute](/Web/API/CanvasRenderingContext2D#attributes) on the context.
...
- [Names for negative spaces](</Web/MathML/Values#constants_(namedspaces)>) are now supported.
...
- The `Function.arity` property has been removed; use [`Function.length`](/Web/JavaScript/Reference/Global_Objects/Function/length) instead.
...
- Initial implementation of the [Navigation Timing](/Web/API/Performance_API/Navigation_timing) specification which provides data that can be used to measure the performance of a website.
...
These changes affect add-on developers as well as developers working on or with Mozilla code itself. Add-on developers should see [Updating extensions for Firefox 7](/Mozilla/Firefox/Releases/7/Updating_extensions) for additional information.
