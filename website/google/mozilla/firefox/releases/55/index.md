...
- Elements on which [`contenteditable`](/Web/HTML/Global_attributes#contenteditable) has been set to `true` now use {{htmlelement("div")}} elements to separate different lines of text, to give Firefox parity with other modern browsers ([Firefox bug 1297414](https://bugzil.la/1297414)). See [Differences in markup generation](/Web/HTML/Global_attributes/contenteditable#differences_in_markup_generation) for more details.
...
- The `layout.css.variables.enabled` preference has been removed completely meaning that the [CSS variables](/Web/CSS/Using_CSS_custom_properties) feature is enabled all the time and can no longer be disabled ([Firefox bug 1312328](https://bugzil.la/1312328)).
...
- The rest operator (`...`) is now supported in [object destructuring](/Web/JavaScript/Reference/Operators/Destructuring_assignment) and the spread operator (`...`) now works in [object literals](/Web/JavaScript/Reference/Operators/Spread_syntax#spread_in_object_literals) (Stage 3 ECMAScript proposal: [Object Rest/Spread Properties](https://github.com/tc39/proposal-object-rest-spread), [Firefox bug 1339395](https://bugzil.la/1339395)).
- [Async generator methods](/Web/JavaScript/Reference/Functions/Method_definitions#async_generator_methods) are now supported ([Firefox bug 1353693](https://bugzil.la/1353693)).
...
- The [Intl API](/Web/JavaScript/Reference/Global_Objects/Intl) now uses the browser's default locale instead of the operating system's default locale when no locale setting is provided ([Firefox bug 1346674](https://bugzil.la/1346674)).
- [Template call sites objects](/Web/JavaScript/Reference/Template_literals) are now canonicalized per realm, based upon their list of raw strings ([Firefox bug 1108941](https://bugzil.la/1108941)).
...
- Methods of the {{domxref("DOMTokenList")}} that modify the list value now automatically trim whitespace and remove duplicate tokens ([Firefox bug 869788](https://bugzil.la/869788), also see [Trimming of whitespace and removal of duplicates](/Web/API/DOMTokenList#trimming_of_whitespace_and_removal_of_duplicates)).
...
- The {{domxref("StorageManager.persist()")}} and {{domxref("StorageManager.persisted()")}} methods of the [Storage API](/Web/API/Storage_API) have been implemented and exposed to `Window` contexts ([Firefox bug 1286717](https://bugzil.la/1286717)).
...
- {{domxref("setTimeout()")}} and {{domxref("setInterval()")}} are now subject to minimum interval throttling for tracking scripts in background tabs — see [Throttling of tracking timeout scripts](/Web/API/setTimeout#throttling_of_tracking_timeout_scripts) ([Firefox bug 1355311](https://bugzil.la/1355311)).
...
- {{domxref("mediaDevices.getUserMedia", "getUserMedia()")}} now provides a stereo audio stream by default if the source device provides stereo sound; support to specifically request mono input will come in [Firefox 56](/Mozilla/Firefox/Releases/56). This only works on desktop at this time; mobile Firefox does not currently support stereo audio input sources ([Firefox bug 971528](https://bugzil.la/971528)).
- The `getUserMedia()` [media capabilities, constraints, and settings](/Web/API/Media_Capture_and_Streams_API/Constraints) `autoGainControl` and `noiseSuppression` now match the spec; formerly they were `moz`-prefixed ([Firefox bug 1366415](https://bugzil.la/1366415)).
...
- On Android, there's now a [pref](https://bugzil.la/1265755#c36) to turn on hardware video encoding to improve video call performance and save on battery. To be enabled by default in [Firefox 56](/Mozilla/Firefox/Releases/56) ([Firefox bug 1265755](https://bugzil.la/1265755)).
...
- The {{domxref("WEBGL_compressed_texture_s3tc_srgb")}} extension is now available to [WebGL](/Web/API/WebGL_API) and [WebGL2](/Web/API/WebGL2RenderingContext) contexts ([Firefox bug 1325113](https://bugzil.la/1325113)).

### Security

- The [Geolocation API](/Web/API/Geolocation_API) is now available only to [secure contexts](/Web/Security/Secure_Contexts) ([Firefox bug 1072859](https://bugzil.la/1072859)).
- The [Storage API](/Web/API/Storage_API) is now available only to [secure contexts](/Web/Security/Secure_Contexts) ([Firefox bug 1268804](https://bugzil.la/1268804)).
...
- The `xml:base` attribute can no longer be used to set the base URL for paths appearing in the [`style`](/Web/HTML/Global_attributes#style) attribute, for example —

  `<div xml:base="https://example.com/" style="background:url(picture.jpg)"></div>` ([Firefox bug 1350521](https://bugzil.la/1350521)).

- The {{htmlelement("style")}} element's [`scoped`](/Web/HTML/Element/style#scoped) attribute has been hidden behind a pref (`layout.css.scoped-style.enabled`) in content documents in Firefox 55+, as no other browsers support it.
- Support for the obscure `MSThemeCompatible` value of the {{htmlelement("meta")}} element's [`http-equiv`](/Web/HTML/Element/meta#http-equiv) attribute has been removed from Gecko. No other modern browsers support it, and it was causing compatibility problems ([Firefox bug 966240](https://bugzil.la/966240)).
...
- [contextMenus.create()'s command property enables you to open browser action popups, page action popups, and sidebars from the context menu.](/Mozilla/Add-ons/WebExtensions/API/menus/create)
- [proxy API](/Mozilla/Add-ons/WebExtensions/API/proxy)
- [chrome_settings_overrides key enables you to override the browser's homepage.](/Mozilla/Add-ons/WebExtensions/manifest.json/chrome_settings_overrides)
- browser_style property enables you to have browser-like styling for [browser action popups](/Mozilla/Add-ons/WebExtensions/manifest.json/browser_action), [sidebars](/Mozilla/Add-ons/WebExtensions/manifest.json/sidebar_action), and [options pages](/Mozilla/Add-ons/WebExtensions/manifest.json/options_ui).
- [permissions API](/Mozilla/Add-ons/WebExtensions/API/permissions)
