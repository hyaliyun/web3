...
- CSS media features for [`dynamic-range`](/Web/CSS/@media/dynamic-range) and [`video-dynamic-range`](/Web/CSS/@media/video-dynamic-range) are now supported. You can now test whether a user agent or an output device supports the combination of brightness, contrast ratio, and color depth by using `dynamic-range` and in the video plane by using `video-dynamic-range` ([Firefox bug 1751217](https://bugzil.la/1751217)).
...
- [`WritableStream`](/Web/API/WritableStream), [`WritableStreamDefaultWriter`](/Web/API/WritableStreamDefaultWriter), [`WritableStreamDefaultController`](/Web/API/WritableStreamDefaultController), and [`ReadableStream.pipeTo()`](/Web/API/ReadableStream/pipeTo) are now supported ([Firefox bug 1759597](https://bugzil.la/1759597)).

#### DOM

- Code can now use the static method [`AbortSignal.timeout()`](/Web/API/AbortSignal/timeout_static).
...
- WebAssembly now supports exceptions that can be thrown and caught in either WebAssembly or JavaScript (or some other runtime), crossing between the environment boundaries if not handled.
  The JavaScript representations of WebAssembly exceptions are [WebAssembly.Exception](/WebAssembly/JavaScript_interface/Exception) and [WebAssembly.Tag](/WebAssembly/JavaScript_interface/Tag) ([Firefox bug 1759217](https://bugzil.la/1759217)).
