...
- [`<link rel="prefetch">`](/Glossary/Prefetch) now obeys the [`crossorigin`](/Web/HTML/Element/link#crossorigin) attribute ([Firefox bug 1214819](https://bugzil.la/1214819)).
...
- Support for the non-standard [`let` blocks](/Web/JavaScript/Reference/Deprecated_and_obsolete_features#statements_2) has been dropped ([Firefox bug 1167029](https://bugzil.la/1167029).
...
- Support for the [`window.sidebar.addSearchEngine()`](/Web/OpenSearch#installing_sherlock_plugins) method, which allowed Web pages to invoke an installation of a Sherlock plugin, has been dropped and now it just logs a warning in the Web Console ([Firefox bug 862148](https://bugzil.la/862148)).
...
- When AppCache is used to provide offline support for a page, a warning message is now displayed in the console advising developers to use [Service workers](/Web/API/Service_Worker_API/Using_Service_Workers) instead ([Firefox bug 1204581](https://bugzil.la/1204581).)
...
- Speech synthesis (text-to-speech) has been implemented in Firefox Desktop for Mac and Linux, hidden behind the `media.webspeech.synth.enabled` flag in `about:config` ([Firefox bug 1003452](https://bugzil.la/1003452), [Firefox bug 1003464](https://bugzil.la/1003464).) See [Web Speech API](/Web/API/Web_Speech_API) for more information.
...
- Added [Snackbars.jsm](/Mozilla/Add-ons/Firefox_for_Android/API/Snackbars.jsm) module to [Firefox for Android](/Mozilla/Firefox_for_Android) ([Firefox bug 1215026](https://bugzil.la/1215026))
...
- Due to breaking changes in Firefox 44 ([bug 1202902](https://bugzil.la/1202902)), add-ons packed with [cfx](/Mozilla/Add-ons/SDK/Tools/cfx) will not work any longer. To make your add-on compatible again, please use [jpm](/Mozilla/Add-ons/SDK/Tools/jpm). You can find steps to migrate from _cfx_ to _jpm_ [here](/Mozilla/Add-ons/SDK/Tools/cfx_to_jpm).
