...
- The [`localName`](/Web/API/Element/localName) DOM property now returns the name of HTML element nodes in lower case. Previously, in HTML documents, it returned it in upper case. (DOM Level 1 [`tagName`](/DOM/node.tagName) continues to return in upper case in HTML documents.)
- The [`namespaceURI`](/Web/API/Element/namespaceURI) DOM property now returns `"http://www.w3.org/1999/xhtml"` on HTML element nodes. Previously, in HTML documents, it returned `null`.
...
- The [`name`](/Web/XPath/Functions/name) and the [`local-name`](/Web/XPath/Functions/local-name) functions in XPath returns the name of HTML elements in lower case. Previously, in HTML documents, they returned it in upper case.
...
Make sure you include a [chrome.manifest](/Chrome_Registration) in your XPI.
