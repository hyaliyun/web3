...
- The `mask-*` longhand properties (see [CSS Masks](/Web/CSS/CSS_masking)) are all supported and available by default (see [Firefox bug 1251161](https://bugzil.la/1251161)).
...
- ECMAScript 2015 semantics for closing iterators have been implemented. This affects the [`for...of`](/Web/JavaScript/Reference/Statements/for...of) loop, for example ([Firefox bug 1147371](https://bugzil.la/1147371)).
- The [Template Literal Revision proposal](https://tc39.es/proposal-template-literal-revision/) that [lifts escape sequence restrictions on tagged template literals](/Web/JavaScript/Reference/Template_literals#tagged_templates_and_escape_sequences) has been implemented ([Firefox bug 1317375](https://bugzil.la/1317375)).
...
- The {{domxref("Element/transitioncancel_event", "transitioncancel")}} event is now fired after a [transition](/Web/CSS/CSS_transitions) is cancelled.
...
- The {{domxref("Selection.setBaseAndExtent()")}} method of the [Selection API](/Web/API/Selection) is now implemented (see [Firefox bug 1321623](https://bugzil.la/1321623)).
- The ["fakepath"](https://html.spec.whatwg.org/multipage/forms.html#fakepath-srsly) addition to `file` type  `values` has been implemented in Gecko, giving it parity with other browsers (see [Firefox bug 1274596](https://bugzil.la/1274596)).
...
- The [Network Information API](/Web/API/Network_Information_API) is now available in workers (see [Firefox bug 1323172](https://bugzil.la/1323172)).
- [Server-sent events](/Web/API/Server-sent_events) can now be used in workers (see [Firefox bug 1267903](https://bugzil.la/1267903)).
...
- Similarly, the {{domxref("RTCPeerConnection")}} methods {{domxref("RTCPeerConnection.setLocalDescription", "setLocalDescription()")}} and {{domxref("RTCPeerConnection.setRemoteDescription", "setRemoteDescription()")}} now accept as input an object conforming to the dictionary `RTCSessionDescriptionInit` dictionary. Existing code continues to work, but [can be simplified](/Web/API/RTCPeerConnection/setLocalDescription#about_the_session_description_parameter).
...
- {{Glossary("DTMF")}} support is now enabled by default using {{domxref("RTCDTMFSender")}}. See [Using DTMF with WebRTC](/Web/API/WebRTC_API/Using_DTMF) for more information on how this works.
...
- [`identity`](/Mozilla/Add-ons/WebExtensions/API/identity)
- [`contextualIdentities`](/Mozilla/Add-ons/WebExtensions/API/contextualIdentities)

Enhanced APIs:

- [`storage.sync`](/Mozilla/Add-ons/WebExtensions/API/storage/sync)
- `page_action`, `browser_action`, `password`, `tab` [context types](/Mozilla/Add-ons/WebExtensions/API/menus/ContextType) in [`contextMenus`](/Mozilla/Add-ons/WebExtensions/API/menus)
- [`webRequest.onBeforeRequest`](/Mozilla/Add-ons/WebExtensions/API/webRequest/onBeforeRequest) now supports `requestBody`
- [`tabs.insertCSS`](/Mozilla/Add-ons/WebExtensions/API/tabs/insertCSS) now supports `cssOrigin`, enabling you to insert user style sheets.

### JavaScript code modules

- The asynchronous [AddonManager APIs](/Mozilla/Add-ons/Add-on_Manager/AddonManager) now support {{jsxref("Promise", "Promises")}} as well as callbacks ([Firefox bug 987512](https://bugzil.la/987512).
