...
- The `rel="noopener"` [Link type](/Web/HTML/Attributes/rel) has been implemented (see [Firefox bug 1222516](https://bugzil.la/1222516)).
...
- Implemented interpolation between numeric color and [currentcolor](/Web/CSS/color_value#currentcolor_keyword) ([Firefox bug 1299741](https://bugzil.la/1299741)).
...
- [CSS Animations](/Web/CSS/CSS_animations) can now be used to animate non-interpolated properties (see [Firefox bug 1064937](https://bugzil.la/1064937)).
...
- There have been a number of changes to CSS  values (see [Firefox bug 1295456](https://bugzil.la/1295456)):
...
- [CSS Grids](/Web/CSS/CSS_grid_layout) are implemented.
...
- The proprietary `-moz-images-in-menus` and `-moz-images-in-buttons` [media types](/Web/CSS/@media#media_features) have been removed (see [Firefox bug 1302157](https://bugzil.la/1302157)).
- Removed `-moz-use-text-color` value from color properties; use [`currentcolor`](/Web/CSS/color_value#currentcolor_keyword) instead ([Firefox bug 1306214](https://bugzil.la/1306214)).
...
- Implemented ES2017 [trailing commas](/Web/JavaScript/Reference/Trailing_commas) in functions ([Firefox bug 1303788](https://bugzil.la/1303788)).
...
- [Array destructuring](/Web/JavaScript/Reference/Operators/Destructuring_assignment#array_destructuring) now throws a {{jsxref("SyntaxError")}} when using destructuring rest with trailing comma ([Firefox bug 1041341](https://bugzil.la/1041341)).
- Duplicate `__proto__` properties are now allowed in [object destructuring](/Web/JavaScript/Reference/Operators/Destructuring_assignment) ([Firefox bug 1204024](https://bugzil.la/1204024)).
...
- {{jsxref("TypedArray")}} constructors now accept [iterables](/Web/JavaScript/Reference/Iteration_protocols) to create new typed arrays ([Firefox bug 1232266](https://bugzil.la/1232266)).
...
- [Unicode code point escapes](/Web/JavaScript/Reference/Lexical_grammar#unicode_code_point_escapes) can now also be used as identifiers (e.g. "`let \u{61} = 123`", see [Firefox bug 1314037](https://bugzil.la/1314037)).
...
- Support for [WebAssembly](/WebAssembly) has been added to Gecko.

### DOM

- The [Selection API](/Web/API/Selection) has fully shipped, including the new {{domxref("Node/selectstart_event", "selectstart")}} and {{domxref("Document/selectionchange_event", "selectionchange")}} events ([Firefox bug 1309612](https://bugzil.la/1309612)).
...
- [Touch events](/Web/API/Touch_events) have been re-enabled on Windows desktop platforms — see [Firefox bug 1244402](https://bugzil.la/1244402). (They were disabled in Firefox 24 because they broke a number of major sites; see [Firefox bug 888304](https://bugzil.la/888304).)
...
- The [Web App Manifest](/Web/Manifest) install event has been renamed {{domxref("Window.appinstalled_event", "appinstalled")}} to avoid confusion with the service worker install event (see {{domxref("ServiceWorkerGlobalScope.install_event", "oninstall")}}). See [Firefox bug 1309099](https://bugzil.la/1309099) for more details about this update.
- The {{domxref("DataTransfer.types")}} property of the [Drag and drop API](/Web/API/HTML_Drag_and_Drop_API) now returns a frozen array of strings rather than a {{domxref("DOMStringList")}} (see [Firefox bug 1298243](https://bugzil.la/1298243)).
...
- The {{domxref("Notification.requireInteraction")}} of the [Notifications API](/Web/API/Notifications_API) has been implemented (see [Firefox bug 862395](https://bugzil.la/862395).)
- The {{domxref("Window.open()")}} method now has a `noopener` [window feature](/Web/API/Window/open#window_functionality_features) available (see [Firefox bug 1267339](https://bugzil.la/1267339)), which mirrors the functionality of the `rel="noopener"` [Link type](/Web/HTML/Attributes/rel).
- The {{domxref("CustomElementRegistry.get()")}} method of the [Web Components API](/Web/API/Web_components) has been implemented (see [Firefox bug 1275838](https://bugzil.la/1275838)).
- [Pointer Event](/Web/API/Pointer_events) {{domxref("PointerEvent.width","width")}} and {{domxref("PointerEvent.height","height")}} properties now default to a value of 1 (see [Firefox bug 1304315](https://bugzil.la/1304315)).
- The [File and Directory Entries API](/Web/API/File_and_Directory_Entries_API) has been updated to include changes in the [latest spec](https://wicg.github.io/entries-api/) (see [Firefox bug 1284987](https://bugzil.la/1284987) for the exact details).
...
- Prior to Firefox 52, the {{domxref("MediaStreamTrack.stop()")}} method could only stop local tracks (that is, tracks obtained through {{domxref("MediaDevices.getUserMedia", "getUserMedia()")}}). Now a variety of tracks can be stopped, including those on a {{domxref("MediaStream")}} associated with a [WebRTC](/Glossary/WebRTC) connection, [Web Audio API](/Web/API/Web_Audio_API) stream, or {{domxref("CanvasCaptureMediaStreamTrack", "CanvasCaptureMediaStream")}}.
...
- The [Battery Status API](/Web/API/Battery_Status_API) is now available only to chrome/privileged code (see [Firefox bug 1313580](https://bugzil.la/1313580)).
...
- The [`'strict-dynamic'` source expression](/Web/HTTP/Headers/Content-Security-Policy/script-src#strict-dynamic) is now supported for {{HTTPHeader("Content-Security-Policy")}} directives, such as {{CSP("script-src")}} ([Firefox bug 1299483](https://bugzil.la/1299483)).
- Insecure sites (`http:`) can't [set cookies](/Web/HTTP/Cookies) with the "secure" directive anymore as per the [Strict Secure Cookies specification](https://datatracker.ietf.org/doc/html/draft-ietf-httpbis-cookie-alone-01) ([Firefox bug 976073](https://bugzil.la/976073)).
...
- When login pages (i.e., those containing an [`<input type="password">`](/Web/HTML/Element/input/password) field) are created so that they would be submitted insecurely, Firefox displays an in-context warning message below the password field to warn users ([Firefox bug 1319119](https://bugzil.la/1319119)). Autofill is also disabled on insecure login forms ([Firefox bug 1217152](https://bugzil.la/1217152)). See [Insecure passwords](/Web/Security/Insecure_passwords) for more details.
...
- [`sessions` API](/Mozilla/Add-ons/WebExtensions/API/sessions)
- [`topSites` API](/Mozilla/Add-ons/WebExtensions/API/topSites)
- [`omnibox` API](/Mozilla/Add-ons/WebExtensions/API/omnibox)
- [`runtime.onInstalled`](/Mozilla/Add-ons/WebExtensions/API/runtime/onInstalled) and [`runtime.onStartup`](/Mozilla/Add-ons/WebExtensions/API/runtime/onStartup) events
- [asynchronous event listeners in webRequest](/Mozilla/Add-ons/WebExtensions/API/webRequest#modifying_requests)
- [`bookmarks.onMoved`](/Mozilla/Add-ons/WebExtensions/API/bookmarks/onMoved), [`bookmarks.onCreated`](/Mozilla/Add-ons/WebExtensions/API/bookmarks/onCreated), [`bookmarks.onChanged`](/Mozilla/Add-ons/WebExtensions/API/bookmarks/onChanged) events
- `_execute_browser_action` and `_execute_page_action` in the [commands manifest key](/Mozilla/Add-ons/WebExtensions/manifest.json/commands)
- [`match_about_blank`](/Mozilla/Add-ons/WebExtensions/manifest.json/content_scripts#match_about_blank) in the content_scripts manifest key
