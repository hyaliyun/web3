...
- The HTML {{HTMLElement("dialog")}} element is now available by default. This element and its associated [DOM APIs](/Web/API/Document_Object_Model) provide support for HTML-based modal dialog boxes ([Firefox bug 1733536](https://bugzil.la/1733536)).
...
- The deprecated [WebVR API](/Web/API/WebVR_API) is now disabled by default on all builds (previously it was enabled on Windows, macOS, and all nightly/dev builds).
