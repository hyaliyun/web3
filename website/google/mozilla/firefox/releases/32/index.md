...
- Experimentally implemented, behind a pref, {{HTMLElement("img")}} [`srcset`](/Web/HTML/Element/img#srcset) property, To activate it set `dom.image.srcset.enable` to `true` ([Firefox bug 870021](https://bugzil.la/870021)).
- [**id**](/Web/HTML/Global_attributes/id) and [**class**](/Web/HTML/Global_attributes/class) are now true [global attributes](/Web/HTML/Global_attributes) and also apply to XML elements, in a namespace or not ([Firefox bug 741295](https://bugzil.la/741295)).
...
- The [Data Store API](/Web/API/Data_Store_API) has been made available to [Web Workers](/Web/API/Web_Workers_API/Using_web_workers) ([Firefox bug 949325](https://bugzil.la/949325)). It still is only activated for certified applications.
- The [ServiceWorker](/Web/API/Service_Worker_API) {{domxref("InstallPhaseEvent")}} and {{domxref("InstallEvent")}} interfaces have been implemented ([Firefox bug 967264](https://bugzil.la/967264)).
- The [MSISDN Verification API](/Web/API/MSISDN_Verification_API), only activated for privileged apps, has been added ([Firefox bug 988469](https://bugzil.la/988469)).
- The [Gamepad API](/Web/API/Gamepad_API) is now supported on Firefox for Android ([Firefox bug 852935](https://bugzil.la/852935)).
...
- [XMLHttpRequest.responseURL](/Web/API/XMLHttpRequest/responseURL) was implemented ([Firefox bug 998076](https://bugzil.la/998076))..
...
- New constraints for [WebRTC](/Glossary/WebRTC)'s {{domxref("NavigatorUserMedia.getUserMedia", "getUserMedia()")}}, `width`, `height`, and `framerate`, have been added, to limit stream dimensions and frame rate ([Firefox bug 907352](https://bugzil.la/907352)):
...
- WebRTC methods which previously used callback functions as input parameters are now also available using JavaScript [promises](/Web/JavaScript/Reference/Global_Objects/Promise).
...
A `getDataDirectory()` method has been added to [`Addon`](/Mozilla/Add-ons/Add-on_Manager/Addon) instances. This method returns the preferred location, within the current profile, for add-ons to store data.

### Add-on SDK

#### Highlights

- Added [`exclude`](/Mozilla/Add-ons/SDK/High-Level_APIs/page-mod#pagemod%28options%29) option to `PageMod`.
- Added [`anonymous`](/Mozilla/Add-ons/SDK/High-Level_APIs/request#request%28options%29) option to `Request`.
- [Add-on Debugger](/Mozilla/Add-ons/Add-on_Debugger) now includes a Console and a Scratchpad.
