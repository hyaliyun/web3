...
- Deprecated `lquote` and `rquote` attributes of the [`<ms>`](/Web/MathML/Element/ms) MathML element for custom opening and closing quotes are now disabled.
...
- The [`contain-intrinsic-size`](/Web/CSS/contain-intrinsic-size) shorthand CSS property can now be applied to specify the size of a UI element that is subject to [size containment](/Web/CSS/CSS_containment#size_containment).
  This allows a user agent to determine the size of an element without needing to render its child elements.
  The shorthand properties [`contain-intrinsic-width`](/Web/CSS/contain-intrinsic-width) and [`contain-intrinsic-height`](/Web/CSS/contain-intrinsic-height) are also supported, along with the [logical properties](/Web/CSS/CSS_logical_properties_and_values) [`contain-intrinsic-block-size`](/Web/CSS/contain-intrinsic-block-size) and [`contain-intrinsic-inline-size`](/Web/CSS/contain-intrinsic-inline-size).
  ([Firefox bug 1597529](https://bugzil.la/1597529)).
- Color font is now supported via the [font-palette](/Web/CSS/font-palette) property ([Firefox bug 1791558](https://bugzil.la/1791558)). Support has also been added for the [@font-palette-values](/Web/CSS/@font-palette-values) CSS at-rule and its descriptors [font-family](/Web/CSS/@font-palette-values/font-family), [base-palette](/Web/CSS/@font-palette-values/base-palette), and [override-colors](/Web/CSS/@font-palette-values/override-colors). Together they help to define the color palette ([Firefox bug 1791558](https://bugzil.la/1791558)).
...
- The non-standard and deprecated [`SVGSVGElement.useCurrentView`](/Web/API/SVGSVGElement#svgsvgelement.usecurrentview) property has been removed.
