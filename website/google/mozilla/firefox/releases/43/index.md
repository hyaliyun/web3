...
- To match the ES2015 specification, [arrow functions](/Web/JavaScript/Reference/Functions/Arrow_functions) no longer have their own [`arguments`](/Web/JavaScript/Reference/Functions/arguments) object. The `arguments` object is now lexically bound (inherited from the outer function). In most cases, [rest parameters](/Web/JavaScript/Reference/Functions/rest_parameters) are a good alternative `(...args) => args[i]`, see [Firefox bug 889158](https://bugzil.la/889158).
- The [arguments](/Web/JavaScript/Reference/Functions/arguments) object is now allowed in conjunction with [rest parameters](/Web/JavaScript/Reference/Functions/rest_parameters) ([Firefox bug 1133298](https://bugzil.la/1133298)).
- From now on, a mapped [`arguments`](/Web/JavaScript/Reference/Functions/arguments) object in non-strict functions is only provided if the function does **not** contain any [rest parameters](/Web/JavaScript/Reference/Functions/rest_parameters), any [default parameters](/Web/JavaScript/Reference/Functions/Default_parameters) or any [destructured parameters](/Web/JavaScript/Reference/Operators/Destructuring_assignment) ([Firefox bug 1175394](https://bugzil.la/1175394)).

#### Other changes

- [Generators](/Web/JavaScript/Reference/Statements/function*) and [generator methods](/Web/JavaScript/Reference/Functions/Method_definitions) are no longer constructable as per ES2016 ([Firefox bug 1191486](https://bugzil.la/1191486)).
...
- The [Battery Status API](/Web/API/Battery_Status_API) now uses the new promise syntax for {{domxref("Navigator.getBattery()")}}, as specified in the recent evolution of the specification ([Firefox bug 1050749](https://bugzil.la/1050749)).
- The `User-Agent` header is no longer in the list of {{Glossary("Forbidden_header_name", "forbidden header names")}} so it can now be set in a [Fetch](/Web/API/Fetch_API) {{domxref("Headers")}} object, via XHR {{domxref("XMLHttpRequest.setRequestHeader()")}},… ([Firefox bug 1188932](https://bugzil.la/1188932)).
...
- The {{domxref("PerformanceObserver")}} interface, belonging to the [Performance APIs](/Web/API/Performance_API) has been implemented ([Firefox bug 1165796](https://bugzil.la/1165796)).
...
- In preparation for future releases to switch over to multi-process content, [NPAPI](/Glossary/Plugin) plugins can no longer be run in the same process as the page content. The preferences starting with `dom.ipc.plugins` are no longer used.
