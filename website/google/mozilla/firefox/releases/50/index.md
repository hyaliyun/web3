...
- Setting the {{HTMLElement("track")}} element's [`src`](/Web/HTML/Element/track#src) attribute now works correctly ([Firefox bug 1281418](https://bugzil.la/1281418)).
...
- The [`ping`](/Web/HTML/Element/a#ping) attribute of {{htmlelement("a")}} element now abides by the [`connect-src`](/Web/HTTP/Headers/Content-Security-Policy#connect-src) [CSP 1.1 policy directive](/Web/HTTP/Headers/Content-Security-Policy) ([Firefox bug 1100181](https://bugzil.la/1100181)).
- Support for the [`sandbox`](/Web/HTTP/Headers/Content-Security-Policy#sandbox) [CSP](/Web/HTTP/CSP) directive has been added ([Firefox bug 671389](https://bugzil.la/671389)).
- It's now possible to set a [content security policy for workers](/Web/API/Web_Workers_API/Using_web_workers#content_security_policy) ([Firefox bug 959388](https://bugzil.la/959388)).
- The {{domxref("Navigator.sendBeacon()")}} method no longer throws an exception if the beacon data couldn't be sent due to a [Content Security Policy](/Web/HTTP/CSP) restriction; instead, it returns `false` as expected ([Firefox bug 1234813](https://bugzil.la/1234813)).
...
- The `only-if-cached` option has been added to [`Request.cache`](/Web/API/Request/cache) ([Firefox bug 1272436](https://bugzil.la/1272436)).
...
- The methods {{domxref("Document.createElement()")}} and {{domxref("Document.createElementNS()")}} now have an optional `options` parameter for creating [custom elements](/Web/API/Web_components/Using_custom_elements) ([Firefox bug 1276579](https://bugzil.la/1276579)).
...
- The old, obsolete Firefox specific drag and drop API events `dragdrop` and `draggesture` are no longer supported. Be sure to update any code still using them to use the [HTML drag and drop API](/Web/API/HTML_Drag_and_Drop_API) ([Firefox bug 1162050](https://bugzil.la/1162050).

### Pointer Lock API

- The [Pointer Lock API](/Web/API/Pointer_Lock_API) is now unprefixed ([Firefox bug 991899](https://bugzil.la/991899)).
- Before Firefox 50, [`requestPointerLock()`](/Web/API/Element/requestPointerLock) asked for permission using a doorhanger, and pointer lock would not be enabled until the user granted permission. From Firefox 50, pointer lock is like the [fullscreen API](/Web/API/Fullscreen_API): it's granted immediately, but a notification is displayed explaining to the user how to exit ([Firefox bug 1273351](https://bugzil.la/1273351)).
...
- The texImage methods have been updated for [WebGL 2](/Web/API/WebGL2RenderingContext) to implement PBOs (`PIXEL_UNPACK_BUFFER`) ([Firefox bug 1280499](https://bugzil.la/1280499)).
...
- The `RTCDataChannel.stream` property has been removed. This was replaced with {{domxref("RTCDataChannel.id")}} in [Firefox 24](/Mozilla/Firefox/Releases/24), but was supported for backward compatibility. Please be sure to update your code to use the `id` property if you haven't done so yet.
...
- A subset of the [File and Directory Entries API](/Web/API/File_and_Directory_Entries_API) has been implemented, to improve compatibility with sites that were previously only compatible with Google Chrome ([Firefox bug 1265767](https://bugzil.la/1265767)).
  ...
  - {{domxref("HTMLInputElement.webkitdirectory")}} as well as the [`webkitdirectory`](/Web/HTML/Element/input#webkitdirectory) attribute of the  element have been implemented; this lets you configure a file input to accept directories instead of files ([Firefox bug 1258489](https://bugzil.la/1258489)).
  ...
  - See [File and Directory Entries API support in Firefox](/Web/API/File_and_Directory_Entries_API/Firefox_support) for details about what we do and do not support in this API.
...
- We've implemented {{domxref("DataTransferItem.webkitGetAsEntry()")}} as part of the [File and Directory Entries API](/Web/API/File_and_Directory_Entries_API); this lets you obtain a {{domxref("FileSystemEntry")}} representing a dropped file ([Firefox bug 1289255](https://bugzil.la/1289255)). This is enabled by default.
