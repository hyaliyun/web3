...
- [Browser Console](https://firefox-source-docs.mozilla.org/devtools-user/browser_console/index.html) messages from [tracking protection errors](/Web/Privacy/Firefox_tracking_protection), [CSP errors](/Web/HTTP/CSP), and [CORS errors](/Web/HTTP/CORS/Errors) are grouped automatically to reduce noise from repeated blocked resources and storage access ([Firefox bug 1522396](https://bugzil.la/1522396)).
...
- Resources that got blocked because of [CSP](/Web/HTTP/CSP) or [Mixed Content](/Web/Security/Mixed_content) are now shown in the Network panel, with details of the reason ([Firefox bug 1556451](https://bugzil.la/1556451)).
...
- The {{cssxref("::cue")}} selector — used to style the captions ("cues") displayed by [WebVTT](/Web/API/WebVTT_API) — now enforces the limitations on which CSS properties may be used within cues, as per the specification ([Firefox bug 1321488](https://bugzil.la/1321488)).
...
- We implemented local-specific casing behavior for Lithuanian ([Firefox bug 1322992](https://bugzil.la/1322992)), [as seen in this example](/Web/CSS/text-transform#lowercase_lithuanian).
...
- [Public class fields](/Web/JavaScript/Reference/Classes#field_declarations) are enabled by default ([Firefox bug 1555464](https://bugzil.la/1555464)). See also [Class fields](/Web/JavaScript/Reference/Classes/Public_class_fields) for more information.
- The promise rejection events [`unhandledrejection`](/Web/API/Window/unhandledrejection_event) and [`rejectionhandled`](/Web/API/Window/rejectionhandled_event) are now enabled by default ([Firefox bug 1362272](https://bugzil.la/1362272)). To learn more about how these work, see [Promise rejection events](/Web/JavaScript/Guide/Using_promises#promise_rejection_events).
...
- The [Resize Observer API](/Web/API/Resize_Observer_API) is supported by default ([Firefox bug 1543839](https://bugzil.la/1543839)).
...
- The [UserScripts API](/Mozilla/Add-ons/WebExtensions/API/userScripts) is now enabled by default.
- The [`topSites.get()`](/Mozilla/Add-ons/WebExtensions/API/topSites/get) method now has new options available — `includePinned` and `includeSearchShortcuts` ([Firefox bug 1547669](https://bugzil.la/1547669)).
