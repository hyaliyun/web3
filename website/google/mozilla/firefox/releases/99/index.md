...
- The [`RTCPeerConnection.setConfiguration()`](/Web/API/RTCPeerConnection/setConfiguration) method is now supported.
...
- The [Network Information API](/Web/API/Network_Information_API) was previously enabled on Android (only), but is now disabled by default on all platforms.
