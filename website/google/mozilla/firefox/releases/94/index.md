...
- Developers can now provide a hint for the enter key label/icon used on virtual keyboards, using either [`HTMLElement.enterkeyhint`](/Web/API/HTMLElement/enterKeyHint) or the global attribute [`enterkeyhint`](/Web/HTML/Global_attributes/enterkeyhint) ([Firefox bug 1648332](https://bugzil.la/1648332)).
...
- The {{domxref("ShadowRoot.delegatesFocus")}} property is now supported, allowing code to check whether the `delegatesFocus` property was set when the [shadow DOM was attached](/Web/API/Element/attachShadow) ([Firefox bug 1413836](https://bugzil.la/1413836)).
...
- The `cache` directive of the [`Clear-Site-Data`](/Web/HTTP/Headers/Clear-Site-Data) response header has been disabled by default.
