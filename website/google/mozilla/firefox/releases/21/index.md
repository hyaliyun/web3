...
- [parseInt](/Web/JavaScript/Reference/Global_Objects/parseInt) no longer treats strings with leading "0" as octal ([Firefox bug 786135](https://bugzil.la/786135)).
...
- The media query [`-moz-windows-glass`](/Web/CSS/CSS_media_queries/Using_media_queries#-moz-windows-glass) has been added on Windows 7 and earlier Windows system ([Firefox bug 816803](https://bugzil.la/816803)).
...
- The function [`window.crypto.getRandomValues`](/Web/API/Crypto/getRandomValues) has been implemented ([Firefox bug 440046](https://bugzil.la/440046)).
...
- Until now, when the same headers were repeatedly set with [`XMLHttpRequest.setRequestHeader`](/Web/API/XMLHttpRequest#setrequestheader), the last-specified value was used. This behavior has been changed to comply with the spec, so those values will be properly combined ([Firefox bug 819051](https://bugzil.la/819051)).

### SVG

- The [paint-order](/Web/SVG/Attribute/paint-order) attribute has been implemented ([Firefox bug 828805](https://bugzil.la/828805)).
...
- The functions {{domxref("URL.createObjectURL_static", "URL.createObjectURL()")}} and {{domxref("URL.revokeObjectURL_static", "URL.revokeObjectURL()")}} are now included in the set of [functions available to workers](/Web/API/Web_Workers_API/Functions_and_classes_available_to_workers).
