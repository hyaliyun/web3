...
- The default modifier keys for the [`accesskey`](/Web/HTML/Global_attributes) of HTML contents on Mac are changed to Control+Option. This is the same as WebKit-based browsers on Mac.

### DOM

- [input](/Web/API/Element/input_event) events are fired also on editing host element of [contenteditable](/Web/API/HTMLElement/contentEditable) editor and root element of [designMode](/Web/API/Document/designMode) editor.
...
- Support for the [Mutation Observers](/Web/API/MutationObserver) has been landed. It is designed as a replacement for the Mutation Events in DOM3, which has a number of issues regarding performance.
...
- The `GeoPositionAddress` interface, an obsolete part of the [Geolocation](/Web/API/Geolocation_API) API, has been removed.
...
- Gecko now supports the new [HTTP](/Web/HTTP) [`308 Permanent Redirect`](/Web/HTTP/Status#308) status code. As Gecko doesn't make a distinction between permanent and temporary redirects, it behaves like the [`307 Temporary Redirect`](/Web/HTTP/Status#307) status code, as it forbids the user agent from changing the HTTP method used between the two requests (`POST` will stay `POST`, `GET` will stay `GET`).
...
- Added the new [`getLineStart()`](/JavaScript_code_modules/source-editor.jsm#getLineStart%28%29) and [`getLineEnd()`](/JavaScript_code_modules/source-editor.jsm#getLineEnd%28%29) methods.
...
- The methods in `nsIPlacesImportExportService` for importing bookmarks have all been removed in favor of the [`BookmarkHTMLUtils.jsm`](/JavaScript_code_modules/BookmarkHTMLUtils.jsm) JavaScript code module.
