...
- Support for the [`download`](/Web/HTML/Element/a#download) attribute on the {{HTMLElement("a")}} and {{HTMLElement("area")}} element has been added ([Firefox bug 676619](https://bugzil.la/676619)).
- The value `auto` for the [global attribute](/Web/HTML/Global_attributes) [`dir`](/Web/HTML/Global_attributes#dir) has been implemented ([Firefox bug 548206](https://bugzil.la/548206)).
- The [global attribute](/Web/HTML/Global_attributes) `contextmenu` now works on Firefox for Android ([Firefox bug 736321](https://bugzil.la/736321)).
...
- Support for the [`Math.imul()`](/Web/JavaScript/Reference/Global_Objects/Math/imul) method, a C-style 32-bit multiplication function. Though proposed for Harmony (ECMAScript 2015) it has not yet been accepted and still is non-standard ([Firefox bug 808148](https://bugzil.la/808148)).
...
- The [`for each...in`](/Web/JavaScript/Reference/Deprecated_and_obsolete_features#statements_2) statement has been deprecated and should not be used. Consider using the new [`for...of`](/Web/JavaScript/Reference/Statements/for...of) statement ([Firefox bug 804834](https://bugzil.la/804834)).
...
- [CSS Flexbox](/Web/CSS/CSS_flexible_box_layout/Basic_concepts_of_flexbox) is now available by default in pre-release builds only (excluding beta versions). It can be enabled in release and beta versions by setting the `layout.css.flexbox.enabled` about:config preference to `true`.
...
- The [`XMLSerializer`](/Web/API/XMLSerializer) `serializeToStream` method is no longer available from web content ([Firefox bug 816410](https://bugzil.la/816410)).
...
- The third, optional, `transfer` argument of {{domxref("Window.postMessage")}} is now supported. It allows to transfer a sequence of [transferable objects](/Web/API/Web_Workers_API/Transferable_objects) to the destination ([Firefox bug 822094](https://bugzil.la/822094)).
