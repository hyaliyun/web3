...
`isSupportedProtocol()` determines if the URL displayed in the active tab is one that can be bookmarked. To extract the protocol from the tab's URL, the extension takes advantage of the [HTMLAnchorElement](/Web/API/HTMLAnchorElement) by adding the tab's URL to an `<a>` element and then getting the protocol using the `protocol` property.
...
If you want to learn more, check out the [Bookmarks API reference](/Mozilla/Add-ons/WebExtensions/API/bookmarks).
