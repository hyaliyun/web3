...
> [!NOTE]
> There are some Tab API features covered elsewhere. These are the methods you can use to manipulate tab content with scripts ({{WebExtAPIRef("tabs.connect")}}, {{WebExtAPIRef("tabs.sendMessage")}}, and {{WebExtAPIRef("tabs.executeScript")}}). If you want more information on these methods, see the Concepts article [Content scripts](/Mozilla/Add-ons/WebExtensions/Content_scripts) and the how-to guide [Modify a web page](/Mozilla/Add-ons/WebExtensions/Modify_a_web_page).
...
- `"tabs"` permission is needed to access the `Tab.url`, `Tab.title`, and `Tab.favIconUrl` properties of the Tab object. In Firefox, you also need `"tabs"` to perform a [query](/Mozilla/Add-ons/WebExtensions/API/tabs/query) by URL.
- [Host permission](/Mozilla/Add-ons/WebExtensions/manifest.json/permissions#host_permissions) is needed for {{WebExtAPIRef("tabs.executeScript()")}} or {{WebExtAPIRef("tabs.insertCSS()")}}.
...
- manifest.json
  ...
  - : The [`manifest.json`](https://github.com/mdn/webextensions-examples/blob/main/apply-css/manifest.json) requests permissions required to use the CSS features. You need either:
    ...
    - `"tabs"` permission and [host permission](/Mozilla/Add-ons/WebExtensions/manifest.json/permissions#host_permissions); or,
...
- [Tabs API reference](/Mozilla/Add-ons/WebExtensions/API/tabs)
- [Example extensions](/Mozilla/Add-ons/WebExtensions/Examples) (many of which use the Tabs API)
