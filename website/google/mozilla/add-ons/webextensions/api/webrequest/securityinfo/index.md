...
If the request is not secured using [TLS](/Glossary/TLS), then this object will contain only the property `state`, whose value will be `"insecure"`.
...
- `hsts` {{optional_inline}}
  - : `Boolean`. `true` if the host uses [Strict Transport Security](/Web/HTTP/Headers/Strict-Transport-Security), `false` otherwise.
