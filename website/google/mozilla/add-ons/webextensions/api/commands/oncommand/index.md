...
The listener is passed the command's name. This matches the name given to the command in its [manifest.json entry](/Mozilla/Add-ons/WebExtensions/manifest.json/commands).
...
- `listener`
  ...
  - : The function called when a user enters the command's shortcut. The function is passed these arguments:
    ...
    - `name`
      - : `string`. Name of the command. This matches the name given to the command in its [manifest.json entry](/Mozilla/Add-ons/WebExtensions/manifest.json/commands).
    - `tab`
