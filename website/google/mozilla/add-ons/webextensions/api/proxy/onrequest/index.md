...
This event is closely modeled on the events defined in the [`webRequest`](/Mozilla/Add-ons/WebExtensions/API/webRequest) API. Like those events, its `addListener()` function takes three arguments:

- the listener that is called when the event is fired.
- a [`RequestFilter`](/Mozilla/Add-ons/WebExtensions/API/webRequest/RequestFilter) object controlling which requests cause the event to fire.
...
To use `proxy.onRequest`, an extension must have the "proxy" [API permission](/Mozilla/Add-ons/WebExtensions/manifest.json/permissions#api_permissions) and the [host permission](/Mozilla/Add-ons/WebExtensions/manifest.json/permissions#host_permissions) for the URLs of the requests that it intercepts, which means that the match patterns in the `filter` argument must be a subset of the extension's host permissions.
