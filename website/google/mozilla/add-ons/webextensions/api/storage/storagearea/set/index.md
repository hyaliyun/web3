...
Note that when storing items in the [`sync`](/Mozilla/Add-ons/WebExtensions/API/storage/sync) storage area, the browser enforces quotas on the amount of data each extension can store. See [Storage quotas for sync data](/Mozilla/Add-ons/WebExtensions/API/storage/sync#storage_quotas_for_sync_data).
...
- `keys`
  ...
  - : An object containing one or more key/value pairs to be stored. If an item is in storage, its value is updated.
    ...
    Values can be [primitive](/Glossary/Primitive) (such as a number, boolean, or string), {{jsxref("Array")}}, or {{jsxref("Object")}} types.
