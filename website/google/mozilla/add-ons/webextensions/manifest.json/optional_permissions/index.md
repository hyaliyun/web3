...
Starting with Firefox 84, users can manage optional permissions from the Firefox Add-ons Manager. Extensions that use optional permissions can listen for [browser.permissions.onAdded](/Mozilla/Add-ons/WebExtensions/API/permissions/onAdded) and [browser.permissions.onRemoved](/Mozilla/Add-ons/WebExtensions/API/permissions/onRemoved) API events to know when a user grants or revokes these permissions.

The key can contain two kinds of permissions: host permissions and API permissions.

## Host permissions

These are the same as the host permissions you can specify in the [`permissions`](/Mozilla/Add-ons/WebExtensions/manifest.json/permissions#host_permissions) key.
...
Note that this is a subset of the API permissions allowed in [`permissions`](/Mozilla/Add-ons/WebExtensions/manifest.json/permissions#api_permissions).
