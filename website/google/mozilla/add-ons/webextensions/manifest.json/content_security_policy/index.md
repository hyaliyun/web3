...
Extensions have a content security policy (CSP) applied to them by default. The default policy restricts the sources from which extensions can load code (such as [\<script>](/Web/HTML/Element/script) resources) and disallows potentially unsafe practices such as the use of [`eval()`](/Web/JavaScript/Reference/Global_Objects/eval). See [Default content security policy](/Mozilla/Add-ons/WebExtensions/Content_Security_Policy#default_content_security_policy) to learn more about the implications of this.

You can use the `"content_security_policy"` manifest key to loosen or tighten the default policy. This key is specified in the same way as the Content-Security-Policy HTTP header. See [Using Content Security Policy](/Web/HTTP/CSP) for a general description of CSP syntax.

For example, you can use this key to:

- Restrict permitted sources for other types of content, such as images and stylesheets, using the appropriate [policy directive](/Web/HTTP/Headers/Content-Security-Policy).
- Allow the extension to take advantage of [WebAssembly](/WebAssembly) by including the `'wasm-unsafe-eval'` source in the `script-src` directive.
- Loosen the default {{CSP("script-src")}} policies (Manifest V2 only):
  ...
  - Allow the extension to execute inline scripts by [supplying the hash of the script in the `script-src` directive](/Web/HTTP/Headers/Content-Security-Policy/script-src#unsafe_inline_script).
...
- The only permitted [keywords](/Web/HTTP/Headers/Content-Security-Policy/default-src#sources) are: `'none'`, `'self'`, `'unsafe-eval'`, and `'wasm-unsafe-eval'`.

## object-src directive

The `{{CSP("object-src")}}` directive may be required in some browsers that support obsolete [plugins](/Glossary/Plugin) and should be set to a secure source such as `'none'` if needed. This may be necessary for browsers up until 2022.
...
Allow [`eval()` and friends](/Mozilla/Add-ons/WebExtensions/Content_Security_Policy#eval%28%29_and_friends):
...
Enable the use of [WebAssembly](/WebAssembly):

- Manifest V2

  For backward compatibility, Manifest V2 extensions in Firefox can use WebAssembly without the use of `'wasm-unsafe-eval'`. However, this behavior isn't guaranteed. See [Firefox bug 1770909](https://bugzil.la/1770909). Extensions using WebAssembly are therefore encouraged to declare `'wasm-unsafe-eval'` in their CSP. See [WebAssembly](/Mozilla/Add-ons/WebExtensions/Content_Security_Policy#webassembly) on the Content Security Policy page for more information.
...
However, this is only invalid in browsers that support obsolete [plugins](/Glossary/Plugin). See [object-src directive](#object-src_directive) for more details..
