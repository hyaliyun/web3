...
You can open the page programmatically by calling [`runtime.openOptionsPage()`](/Mozilla/Add-ons/WebExtensions/API/runtime/openOptionsPage).

Options pages have a Content Security Policy that restricts the sources from which they can load resources, and disallows some unsafe practices such as the use of [`eval()`](/Web/JavaScript/Reference/Global_Objects/eval). See [Content Security Policy](/Mozilla/Add-ons/WebExtensions/Content_Security_Policy) for more details.
...
JavaScript running in the page can use all the [WebExtension APIs](/Mozilla/Add-ons/WebExtensions/API) that the add-on has [permissions](/Mozilla/Add-ons/WebExtensions/manifest.json/permissions) for. In particular, you can use the [`storage`](/Mozilla/Add-ons/WebExtensions/API/storage) API to persist preferences.

Package the page's files in your extension.

You also need to include the [`options_ui`](/Mozilla/Add-ons/WebExtensions/manifest.json/options_ui) key in your manifest.json file, giving it the URL to the page.
...
See the [`options_ui`](/Mozilla/Add-ons/WebExtensions/manifest.json/options_ui) page for **sharing options** between your options page and background or content scripts.
