...
With WebExtension APIs, settings are generally stored using the [`storage`](/Mozilla/Add-ons/WebExtensions/API/storage) API.
...
- Set the path to the HTML file as the [`options_ui`](/Mozilla/Add-ons/WebExtensions/manifest.json/options_ui) key in `manifest.json`. By doing this, the HTML document will be shown in the browser's add-on manager, alongside the extension's name and description.

> [!NOTE]
> You can also open this page programmatically using the [`runtime.openOptionsPage()`](/Mozilla/Add-ons/WebExtensions/API/runtime/openOptionsPage) function.
...
- [`permissions`](/Mozilla/Add-ons/WebExtensions/manifest.json/permissions)
  - : We'll use the [`storage`](/Mozilla/Add-ons/WebExtensions/API/storage) API to store the settings, and we need to ask permission to use this API.
...
This defines a {{htmlelement("form")}} with a labeled text  and a submit {{htmlelement("button")}}. It also includes a script called `options.js`.
...
- When the document has loaded, it fetches the value of `"color"` from storage using [`storage.sync.get()`](/Mozilla/Add-ons/WebExtensions/API/storage/StorageArea/get). If the value isn't set, it uses the default `"blue"`. This retrieves the values from the `sync` storage area.
- When the user submits the form by clicking Save, it stores the value of the textbox using [`storage.sync.set()`](/Mozilla/Add-ons/WebExtensions/API/storage/StorageArea/set). This saves the value to the `sync` storage area.
...
> [!NOTE]
> The implementation of `storage.sync` in Firefox relies on the Add-on ID. If you use `storage.sync`, you must set an ID for your extension using the [`browser_specific_settings`](/Mozilla/Add-ons/WebExtensions/manifest.json/browser_specific_settings) key in `manifest.json`, as shown in the example manifest above. See [Firefox bug 1323228](https://bugzil.la/1323228) for related information.
...
- [`options_ui`](/Mozilla/Add-ons/WebExtensions/manifest.json/options_ui) key of `manifest.json` reference documentation
- [`storage`](/Mozilla/Add-ons/WebExtensions/API/storage) API reference documentation
- open the settings page directly from your extension using the [`runtime.openOptionsPage()`](/Mozilla/Add-ons/WebExtensions/API/runtime/openOptionsPage) API
