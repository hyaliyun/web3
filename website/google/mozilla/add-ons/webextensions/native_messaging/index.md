...
The extension must request the `"nativeMessaging"` [permission](/Mozilla/Add-ons/WebExtensions/manifest.json/permissions) or [optional permission](/Mozilla/Add-ons/WebExtensions/manifest.json/optional_permissions) in the `manifest.json` file. Also, the native application must grant permission for the extension by including the ID in the `"allowed_extensions"` field of the app manifest.
...
- Set the `"nativeMessaging"` [permission](/Mozilla/Add-ons/WebExtensions/manifest.json/permissions) or [optional permission](/Mozilla/Add-ons/WebExtensions/manifest.json/optional_permissions) in the [`manifest.json`](/Mozilla/Add-ons/WebExtensions/manifest.json) file.
- Specify your add-on ID explicitly. Use the [`browser_specific_settings`](/Mozilla/Add-ons/WebExtensions/manifest.json/browser_specific_settings) manifest key. (The app's manifest will identify the set of extensions that allow connecting to the IDs).
...
> [!NOTE]
> Chrome does not support the [browser_specific_settings](/Mozilla/Add-ons/WebExtensions/manifest.json/browser_specific_settings) key. You will need to use another manifest without this key to install an equivalent WebExtension on Chrome. See [Chrome incompatibilities below](#chrome_incompatibilities).
...
For details of native app manifest syntax and location, see [Native manifests](/Mozilla/Add-ons/WebExtensions/Native_manifests).
...
Native messaging cannot directly be used in content scripts. You must [do it indirectly via background scripts](/Mozilla/Add-ons/WebExtensions/Content_scripts#communicating_with_background_scripts).
...
- (new in Firefox 55) the ID (as given in the [browser_specific_settings](/Mozilla/Add-ons/WebExtensions/manifest.json/browser_specific_settings) `manifest.json` key) of the add-on that started it.
...
- macOS/Linux: check the native application's manifest file location as mentioned [here](/Mozilla/Add-ons/WebExtensions/Native_manifests#mac_os_x).
...
These differences are discussed in [Chrome incompatibilities > Native messaging](/Mozilla/Add-ons/WebExtensions/Chrome_incompatibilities#native_messaging).
