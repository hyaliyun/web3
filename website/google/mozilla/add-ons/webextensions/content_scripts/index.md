...
[Background scripts](/Mozilla/Add-ons/WebExtensions/Background_scripts) can access all the [WebExtension JavaScript APIs](/Mozilla/Add-ons/WebExtensions/API), but they can't directly access the content of web pages. So if your extension needs to do that, you need content scripts.

Just like the scripts loaded by normal web pages, content scripts can read and modify the content of their pages using the standard [Web APIs](/Web/API). However, they can only do this when [host permissions to the web page's origin have been granted](#permissions).

> [!NOTE] Some Web APIs are restricted to [secure contexts](/Web/Security/Secure_Contexts), which also applies to content scripts running in these contexts. Except for {{domxref("PointerEvent.getCoalescedEvents()")}}, which can be called from content scripts in insecure contexts in Firefox.
...
1. At install time, into pages that match URL patterns.
   - Using the [`content_scripts`](/Mozilla/Add-ons/WebExtensions/manifest.json/content_scripts) key in your `manifest.json`, you can ask the browser to load a content script whenever the browser loads a page whose URL [matches a given pattern](/Mozilla/Add-ons/WebExtensions/Match_patterns).
2. At runtime, into pages that match URL patterns.
   - Using {{WebExtAPIRef("scripting.registerContentScripts()")}} or (only in Manifest V2 in Firefox) {{WebExtAPIRef("contentScripts")}}, you can ask the browser to load a content script whenever the browser loads a page whose URL [matches a given pattern](/Mozilla/Add-ons/WebExtensions/Match_patterns). (This is similar to method 1, _except_ that you can add and remove content scripts at runtime.)
3. At runtime, into specific tabs.
   - Using {{WebExtAPIRef("scripting.executeScript()")}} or (in Manifest V2 only) {{WebExtAPIRef("tabs.executeScript()")}}, you can load a content script into a specific tab whenever you want. (For example, in response to the user clicking on a [browser action](/Mozilla/Add-ons/WebExtensions/user_interface/Toolbar_button).)
...
Using methods (1) and (2), you can only load scripts into pages whose URLs can be represented using a [match pattern](/Mozilla/Add-ons/WebExtensions/Match_patterns).
...
> **Note:** [Dynamic JS module imports](/Web/JavaScript/Guide/Modules#dynamic_module_loading) are now working in content scripts. For more details, see [Firefox bug 1536094](https://bugzil.la/1536094).
...
Registered content scripts are only executed if the extension is granted [host permissions](/Mozilla/Add-ons/WebExtensions/manifest.json/permissions#host_permissions) for the domain.

To inject scripts programmatically, the extension needs either the [`activeTab` permission](/Mozilla/Add-ons/WebExtensions/manifest.json/permissions#activetab_permission) or [host permissions](/Mozilla/Add-ons/WebExtensions/manifest.json/permissions#host_permissions). The `scripting` permission is required to use methods from the {{WebExtAPIRef("scripting")}} API.
...
Both [host permissions](/Mozilla/Add-ons/WebExtensions/manifest.json/permissions#host_permissions) and the [`activeTab` permission](/Mozilla/Add-ons/WebExtensions/manifest.json/permissions#activetab_permission) have exceptions for some domains. Content scripts are blocked from executing on these domains, for example, to protect the user from an extension escalating privileges through special pages.
...
As noted at ["Content script environment" at Chrome incompatibilities](/Mozilla/Add-ons/WebExtensions/Chrome_incompatibilities#content_script_environment), the behavior differs across browsers:

- In Firefox, this behavior is called [Xray vision](/Mozilla/Add-ons/WebExtensions/Sharing_objects_with_page_scripts#xray_vision_in_firefox).
...
> [!NOTE]
> Firefox provides [cloneInto()](/Mozilla/Add-ons/WebExtensions/Content_scripts/cloneInto) and [exportFunction()](/Mozilla/Add-ons/WebExtensions/Content_scripts/exportFunction) to enable content scripts to access JavaScript objects created by page scripts and expose their JavaScript objects to page scripts.
>
> See [Sharing objects with page scripts](/Mozilla/Add-ons/WebExtensions/Sharing_objects_with_page_scripts) for more details.

### WebExtension APIs

In addition to the standard DOM APIs, content scripts can use the following WebExtension APIs:

**From [`extension`](/Mozilla/Add-ons/WebExtensions/API/extension):**

- [`getURL()`](/Mozilla/Add-ons/WebExtensions/API/extension/getURL)
- [`inIncognitoContext`](/Mozilla/Add-ons/WebExtensions/API/extension/inIncognitoContext)

**From [`runtime`](/Mozilla/Add-ons/WebExtensions/API/runtime):**

- [`connect()`](/Mozilla/Add-ons/WebExtensions/API/runtime/connect)
- [`getManifest()`](/Mozilla/Add-ons/WebExtensions/API/runtime/getManifest)
- [`getURL()`](/Mozilla/Add-ons/WebExtensions/API/runtime/getURL)
- [`onConnect`](/Mozilla/Add-ons/WebExtensions/API/runtime/onConnect)
- [`onMessage`](/Mozilla/Add-ons/WebExtensions/API/runtime/onMessage)
- [`sendMessage()`](/Mozilla/Add-ons/WebExtensions/API/runtime/sendMessage)

**From [`i18n`](/Mozilla/Add-ons/WebExtensions/API/i18n):**

- [`getMessage()`](/Mozilla/Add-ons/WebExtensions/API/i18n/getMessage)
- [`getAcceptLanguages()`](/Mozilla/Add-ons/WebExtensions/API/i18n/getAcceptLanguages)
- [`getUILanguage()`](/Mozilla/Add-ons/WebExtensions/API/i18n/getUILanguage)
- [`detectLanguage()`](/Mozilla/Add-ons/WebExtensions/API/i18n/detectLanguage)

**From [`menus`](/Mozilla/Add-ons/WebExtensions/API/menus):**

- [`getTargetElement`](/Mozilla/Add-ons/WebExtensions/API/menus/getTargetElement)
...
Content scripts can make requests using the normal [`window.XMLHttpRequest`](/Web/API/XMLHttpRequest) and [`window.fetch()`](/Web/API/Fetch_API) APIs.

> [!NOTE]
> In Firefox in Manifest V2, content script requests (for example, using [`fetch()`](/Web/API/Fetch_API/Using_Fetch)) happen in the context of an extension, so you must provide an absolute URL to reference page content.
>
...
Content scripts get the same cross-domain privileges as the rest of the extension: so if the extension has requested cross-domain access for a domain using the [`permissions`](/Mozilla/Add-ons/WebExtensions/manifest.json/permissions) key in `manifest.json`, then its content scripts get access that domain as well.

> [!NOTE]
> When using Manifest V3, content scripts can perform cross-origin requests when the destination server opts in using [CORS](/Web/HTTP/CORS); however, host permissions don't work in content scripts, but they still do in regular extension pages.

This is accomplished by exposing more privileged XHR and fetch instances in the content script, which has the side effect of not setting the [`Origin`](/Web/HTTP/Headers/Origin) and [`Referer`](/Web/HTTP/Headers/Referer) headers like a request from the page itself would; this is often preferable to prevent the request from revealing its cross-origin nature.
  ...
  <tbody>
    <tr>
      ...
      <td>
        <code
          ><a
            href="/Mozilla/Add-ons/WebExtensions/API/runtime/sendMessage"
      ...
      <td>
        <code
          ><a
            href="/Mozilla/Add-ons/WebExtensions/API/tabs/sendMessage"
    ...
    <tr>
      ...
      <td>
        <code
          ><a
            href="/Mozilla/Add-ons/WebExtensions/API/runtime/onMessage"
...
The background script listens for these messages and displays a notification using the [`notifications`](/Mozilla/Add-ons/WebExtensions/API/notifications) API:
...
Both sides have a [`runtime.Port`](/Mozilla/Add-ons/WebExtensions/API/runtime/Port) object, which they can use to exchange messages.

To create the connection:

- One side listens for connections using [`runtime.onConnect`](/Mozilla/Add-ons/WebExtensions/API/runtime/onConnect)
- The other side calls:

  - [`tabs.connect()`](/Mozilla/Add-ons/WebExtensions/API/tabs/connect) (if connecting to a content script)
  - [`runtime.connect()`](/Mozilla/Add-ons/WebExtensions/API/runtime/connect) (if connecting to a background script)

This returns a [`runtime.Port`](/Mozilla/Add-ons/WebExtensions/API/runtime/Port) object.

- The [`runtime.onConnect`](/Mozilla/Add-ons/WebExtensions/API/runtime/onConnect) listener gets passed its own [`runtime.Port`](/Mozilla/Add-ons/WebExtensions/API/runtime/Port) object.
...
By default, content scripts don't get access to the objects created by page scripts. However, they can communicate with page scripts using the DOM [`window.postMessage`](/Web/API/Window/postMessage) and [`window.addEventListener`](/Web/API/EventTarget/addEventListener) APIs.
...
The same applies to [`setTimeout()`](/Web/API/setTimeout), [`setInterval()`](/Web/API/setInterval), and [`Function()`](/Web/JavaScript/Reference/Global_Objects/Function).
