...
> [!NOTE]
> If you want to share data using the Web Share API, see [Web Share API](/Web/API/Web_Share_API) and [`navigator.share()`](/Web/API/Navigator/share).
...
- `enctype` {{Optional_Inline}}
  - : The encoding of the share data when a [`POST`](/Web/HTTP/Methods/POST) request is used. Ignored with [`GET`](/Web/HTTP/Methods/GET) requests.
- `method` {{Optional_Inline}}
  - : The [HTTP request method](/Web/HTTP/Methods) to use. Either [`GET`](/Web/HTTP/Methods/GET) or [`POST`](/Web/HTTP/Methods/POST). Use `POST` if the shared data includes binary data like image(s), or if it changes the target app, for example, if it creates a data point like a bookmark.
- `params`
  - : An object to configure the share parameters. The object keys correspond to the [`data` object in `navigator.share()`](/Web/API/Navigator/share#parameters). The object values can be specified and will be used as query parameters:
...
The [URLSearchParams](/Web/API/URLSearchParams) interface can be useful to handle the shared data in your application and do something with it.
...
If the share request includes one or multiple files or causes a side effect in your application, the HTTP [`POST`](/Web/HTTP/Methods/POST) method should be used. For example, if your application receives images for further processing or wants to save a shared link as a bookmark in your database.
...
You can either handle `POST` share data using server-side code, or, to provide a better experience for offline users, a `fetch` event listener can be used to intercept the HTTP request which allows to access the data in a [service worker](/Web/API/Service_Worker_API).
...
The `POST` request is then ideally replied with an HTTP [303 See Other](/Web/HTTP/Status/303) redirect to avoid multiple `POST` requests from being submitted if a page refresh was initiated by the user, for example.
...
To handle shared file data, see the `POST` example above and the [`FileReader`](/Web/API/FileReader) API to read the files. In order to get the files from the service worker context over to client contexts, one solution is to temporarily write the files to the [`Cache`](/Web/API/Cache) or [IndexedDB](/Web/API/IndexedDB_API) and then notify its clients using [`Client.postMessage()`](/Web/API/Client/postMessage).

## Security & Privacy

Your PWA can only act as a web share target if it has been installed. See also [How to make PWAs installable](/Web/Progressive_web_apps/Tutorials/js13kGames/Installable_PWAs).
