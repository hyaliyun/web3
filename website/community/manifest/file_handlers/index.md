...
The `file_handlers` member specifies an array of objects representing the types of files an installed [progressive web app (PWA)](/Web/Progressive_web_apps) can handle.
...
- `action`
  ...
  - : A string containing the URL to navigate to when a file is handled.
    This URL must be within the navigation scope of the PWA, which is the set of URLs that the PWA can navigate to. The navigation scope of a PWA defaults to its [`start_url`](/Web/Manifest/start_url) member, but can also be defined by using the [`scope`](/Web/Manifest/scope) member.
...
Handling files is done in the application code that runs on the {{Glossary("main thread")}}, not in the application's [service worker](/Web/API/Service_Worker_API).
