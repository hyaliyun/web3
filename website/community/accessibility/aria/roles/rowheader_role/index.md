...
An element with `role="rowheader"` is a [cell](/Web/Accessibility/ARIA/Roles/cell_role) containing header information for a [row](/Web/Accessibility/ARIA/Roles/row_role) within a tabular structure of a [`grid`](/Web/Accessibility/ARIA/Roles/grid_role), [`table`](/Web/Accessibility/ARIA/Roles/table_role) or [`treegrid`](/Web/Accessibility/ARIA/Roles/treegrid_role).

## Description

`Rowheader` is the header [`cell`](/Web/Accessibility/ARIA/Roles/cell_role) for a row, establishing a relationship between it and the other cells in the same [`row`](/Web/Accessibility/ARIA/Roles/row_role).
...
> [!NOTE]
> Using the native [table elements](/Learn/HTML/Tables/Basics) whenever possible, is strongly encouraged.
...
Above is the semantic way of writing a table. The ARIA roles are only necessary if the native semantics of the table, and therefore the table row headers, are obliterated, such as by setting the [display property to flex or grid](/Web/CSS/display#accessibility).
...
- [The HTML `<table>` element](/Web/HTML/Element/table)
- [The HTML `<th>` element](/Web/HTML/Element/th)
