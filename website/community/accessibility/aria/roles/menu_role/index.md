...
Menu items include [`menuitem`](/bility/ARIuole). [Dis/ty/ARIA/At) are focusi

Menu items can be grouped in elements with the [`group`](/Roles/sep). Neither  `separatorocus or aree.
...
Elements with the role `menu` have an implicit [`aria-orientation`]([`aria-orientation="horizontal"`](/Web/AccessA/Attributentation).

If the menu is visually persistent, consider the [`menubar`](

### Associated WAI-ARIA roles, states, and properties

- [`menuitem`](/bility/ARIuole) roles
...
The button that opens the menu has [`aria-haspopup="menu"`](menu`.
...
The opening and closing of the menu toggles the [`aria-expanded="true"`](Removed or set to `false` when the menu is closed. The `true` value indicates that the menu is displayed and that activating the menu button closes the menu.
