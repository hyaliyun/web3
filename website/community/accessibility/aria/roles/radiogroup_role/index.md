...
Radio groups are collections describing a set of related [`radio`](/Web/Accessibility/ARIA/Roles/radio_role) options. A `radiogroup` is a type of [`select`](/Web/Accessibility/ARIA/Roles/select_role) list that can only have a single entry, or `radio`, checked at any one time.

When using HTML's native input radio button, [`<input type="radio">`](/Web/HTML/Element/input/radio), the radio buttons are grouped when each of input radio buttons in the group is given the same [`name`](/Web/HTML/Element/input#name). Once a group of same-named input radio buttons is established, selecting any input radio button in that group automatically deselects any currently-selected input radio button in the same group. While this will associate the radio buttons together, to expose a grouping of radio buttons as a `radiogroup` explicitly set the ARIA role.
...
The `radiogroup` must have an accessible name either by a visible label referenced by [`aria-labelledby`](/Web/Accessibility/ARIA/Attributes/aria-labelledby) or has a label specified with [`aria-label`](/Web/Accessibility/ARIA/Attributes/aria-label). If elements provide additional information about the radio group, those elements are referenced by the `radiogroup` element with the [`aria-describedby`](/Web/Accessibility/ARIA/Attributes/aria-describedby) property.

### Associated WAI-ARIA roles, states, and properties

- [`radio`](/Web/Accessibility/ARIA/Roles/radio_role) role
...
For `radio` buttons in a `radiogroup` that is NOT in a [`toolbar`](/Web/Accessibility/ARIA/Roles/toolbar_role), the following keyboard interactions must be supported:
...
Use the `[aria-checked="true"]` [attribute selector](/Web/CSS/Attribute_selectors) to style the checked state of checked radio buttons.
...
- [ARIA `radio` role](/Web/Accessibility/ARIA/Roles/radio_role)
- [`aria-errormessage`](/Web/Accessibility/ARIA/Attributes/aria-errormessage)
