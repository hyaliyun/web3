...
A `menubar` contains three types of menu items, including [`menuitem`](mradio`](/Web/AccessA/Roles/menrole) and [eckbox`](/Wility/ARIui_role). Ttems may optionally beone or more [`group`](ibility/ARIA/Roles/groroups or items may optseparated with [`separator`](/Webity/ARIA/Roles/separator_role Whinu item must be able to receiven , the `group` and `separator`re not focusable.
...
The menubar and all the menu items are focusable and have a [tabindex](en the menubar receives focus through tabbing, keyboard focus is placed on the first menuitem. Each item in the menu has `tabindex` set to `-1`, except the first item has which has its `tabindex` set to `0`.
...
If a `menubar` has a visible label, include [`aria-labelledby`]( to a value that refers to the labelling element. Otherwise, provide the menubar with an accessible name by including a descriptive [`aria-label`](/Web/AccessA/Attributel).

A `menuitem` element in the `menubar` can contain a child submenu of menu items. Submenus can be nested several deep. Generally, the outer `menubar` is horizontal and all the submenus are vertical. If this is not the case, if your menubar is vertical, include [`aria-orientation="vertical"`]( the `menubar` element. Otherwise, this attribute is not necessary, as the default value is horizontal.
...
See [`menuitem` keyboard interactions](teractions), [`menuitemradio` keyboard interactions](/Web/AccessA/Roles/menrole#keyboaions), and heckbox` keractions](/Web/Accessibility/ARIuitemcheckbox_role#keyactions) for more infokeyboard interactions is on a menuitem in a ich it always is).
