...
The `region` role is an [ARIA landmark](/Web/Accessibility/ARIA/Roles#3._landmark_roles) role.
The `region` role should be reserved for sections of content sufficiently important that users will likely want to navigate to the section easily and to have it listed in a summary of the page. A region role is a more generic term, and should only be used if the section needing to be identified is not accurately described by one of the other landmark roles, such as [`banner`](/Web/Accessibility/ARIA/Roles/banner_role), [`main`](/Web/Accessibility/ARIA/Roles/main_role), [`contentinfo`](/Web/Accessibility/ARIA/Roles/contentinfo_role), [`complementary`](/Web/Accessibility/ARIA/Roles/complementary_role), or [`navigation`](/Web/Accessibility/ARIA/Roles/navigation_role).

Every element with a `region` role should include a label that describes the purpose of the content in the region, preferably with an [`aria-labelledby`](/Web/Accessibility/ARIA/Attributes/aria-labelledby) referencing a visible header. If no visible appropriate header is present, [`aria-label`](/Web/Accessibility/ARIA/Attributes/aria-label) should be used.
...
- [`aria-labelledby`](/Web/Accessibility/ARIA/Attributes/aria-labelledby) or [`aria-label`](/Web/Accessibility/ARIA/Attributes/aria-label)
...
Use sparingly! [Landmark roles](/Web/Accessibility/ARIA/Roles#3._landmark_roles) are intended to be used sparingly, to identify larger overall sections of the document. Using too many landmark roles can create "noise" in screen readers, making it difficult to understand the overall layout of the page.

Only use the `region` role if no other relevant [content sectioning](/Web/HTML/Element#content_sectioning) element or [landmark role](/Web/Accessibility/ARIA/Roles#3._landmark_roles) applies. If multiple regions exist on a page, it may be worth reinvestigating the page's overall structure.
...
In this example, the region's label is being generated by the [`aria-labelledby` attribute](/Web/Accessibility/ARIA/Attributes/aria-labelledby).
...
- [Landmark roles: Using ARIA: Roles, States, and Properties](/Web/Accessibility/ARIA/ARIA_Techniques#landmark_roles)
