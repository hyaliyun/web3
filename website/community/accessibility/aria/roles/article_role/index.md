...
If an article is part of a feed, it can have the [`aria-posinset`](/Web/Accessibility/ARIA/Attributes/aria-posinset) and [`aria-setsize`](/Web/Accessibility/ARIA/Attributes/aria-setsize) attributes set to indicate which position within the feed this particular article represents.
...
This role corresponds to the {{HTMLElement('article')}} element in HTML, and that element should be used instead, if possible. This role does not require any specific roles to be present among its children. It is the only role allowed as a direct child of an element with the [`feed`](/Web/Accessibility/ARIA/Roles/feed_role) role.

## See also

- [`feed` role](/Web/Accessibility/ARIA/Roles/feed_role)
- [`section` role](/Web/Accessibility/ARIA/Roles/section_role)
