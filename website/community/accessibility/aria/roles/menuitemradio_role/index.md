...
The items in menu and menubars are menu items. There are three types of menu items: [`menuitem`](ibility/ARIA/Roles/menuitem_role), [`menuitemcheckbox`](/Web/AccessA/Roles/menuitemcheckbox_role), and `menuitemradio`. To limit the number of checked menu items to one within a group, use the `menuitemradio` role on all the elements in the group.

A `menuitemradio` is checkable menuitem in a set of elements with the same role, of which only can be checked at a time.

The three menu item elements can only be contained in, or owned by, an element with role [`menu`](ibility/ARIA/Roles/menu_role) or [`menubar`](/Web/AccessA/Roles/menubar_role), optionally nested within a grouping element with role of [`group`](/Web/Accessibility/ARIup_role). Being nested or otherwise owned (see [`aria-owns`](/Web/Accessibility/ARIA/Attribute)) in a `menu` or `menubar` identifies the menu items as being related widgets.
...
Menu items containing the role of `menuitemradio` must include the [`aria-checked`](ibility/ARIA/Attributes/aria-checked) attribute to expose the radio button's state to assistive technology, unless using [`<input type="radio">`](/Web/HTML/Et/checkbox), in which case the [`checked`](/Web/HTML/Element/inpuchecked) attribute should be used.
...
There are some types of user interface components that, when represented in a platform accessibility API, can only contain text. Accessibility APIs do not have a way of representing semantic elements contained in a `menuitemradio`. To deal with this limitation, browsers, automatically apply role [`presentation`](ibility/ARIA/Roles/presentation_role) to all descendant elements of any `menuitemradio` element as it is a role that does not support semantic children.
...
From the assistive technology user's perspective, the heading does not exist since the previous code snippets are equivalent to the following in the [accessibility tree](ccessibility_tree):
...
Don't use the [`background`](ckground) shorthand property, as that will override the [`background-clip`](/Web/CSS/baip) property we used to create the radio button effect.

### Prefer HTML

The first rule of ARIA is: if a native HTML element or attribute has the semantics and behavior you require, use it instead of re-purposing an element and adding an ARIA role, state or property to make it accessible. As such, it is recommended to use the native [HTML radio button](lement/input/radio) form control instead of recreating a radio button's functionality with JavaScript and ARIA.
...
- [`radio` role](ibility/ARIA/Roles/radio_role)
- [`<input type="radio">`](lement/input/radio)
