...
A WAI-ARIA combobox only has one attribute that is required that authors specify: [`aria-expanded`]( necessary to specify, depending on the combobox's implementation. These include [`aria-haspopup`](/Web/AccessA/Attributeopup), [`ar`](/Web/AccARIs/aria-c`arscendantessA/Attribdanria-autoibittribute

Typically, the initial state of a combobox is collapsed, with `aria-expanded="false"` set. In the collapsed state, only the combobox element and, optionally a sibling button to invoke the popup, are visible. The [`aria-expanded`](because it indicates to assistive technologies that the widget is expandable.
...
If the `combobox` supports text input and provides autocompletion behavior, set [`aria-autocomplete`]( the provided behavior: `inline`, `list`, or `both`. The `aria-autocomplete` attribute indicates that inputting text will trigger display of one or more predictions of the user's intended value for the combobox and specifies how the predictions will be presented when made.

Every `combobox` must have an accessible name. If using an {{HTMLElement('input')}} element, the accessible name should come from the associated {{HTMLElement('label')}}. If not, if an appropriate label is visible in the content, provide the name via [`aria-labelledby`](ace that serves as a label for the `combobox` input field, include `aria-labelledby` as an attribute on the element with the role of `combobox`, and set the value of the attribute to the `id` of the labelling element or elements. If no visible label is present, use [`aria-label`](/Web/AccessA/Attributel) instead.
