...
The `mark` element should not be given an accessible name; both [`aria-label`](/Web/Accessibility/ARIA/Attributes/aria-label) and [`aria-labelledby`](/Web/Accessibility/ARIA/Attributes/aria-labelledby) attributes are prohibited on `mark`.
...
The related comment is marked up using an HTML structure wrapped with a {{HTMLElement('div')}} containing [`role="comment"`](/Web/Accessibility/ARIA/Roles/comment_role).

To associate the comment with the text being commented, we need to wrap the commented text with an element containing the [`aria-details`](/Web/Accessibility/ARIA/Attributes/aria-details) attribute, the value of which should be the ID of the comment.
