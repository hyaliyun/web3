...
There are other HTML and WAI-ARIA properties that have similar purposes. The HTML {{HTMLElement('label')}} element and the [`aria-label`](/Web/Accessibility/ARIA/Attributes/aria-label) and [`aria-labelledby`](/Web/Accessibility/ARIA/Attributes/aria-labelledby) properties are used to provide short labels for an object. The HTML `title` attribute and the [`aria-description`](/Web/Accessibility/ARIA/Attributes/aria-description) and [`aria-describedby`](/Web/Accessibility/ARIA/Attributes/aria-describedby) properties provide for longer plain text descriptions of an object. However, when additional information, complex descriptions, or navigable content related to the object is necessary and available, the `aria-details` attribute should be used.
...
The `aria-details` attribute takes the [`id`](/Web/HTML/Global_attributes#id), or space separated list of `id`s of the elements to get more detailed information as its values. When `aria-details` is included on an element, assistive technologies inform users of the availability of extended information, enabling the user to navigate to the referenced content.

Elements referenced by `aria-details` are intended to contain more information than would normally be provided via [`aria-describedby`](/Web/Accessibility/ARIA/Attributes/aria-describedby).
...
When it comes to definition and term roles, the `aria-details` would be included on the [`term`](/Web/Accessibility/ARIA/Roles/term_role) element with the `id` of the element with a [`definition`](/Web/Accessibility/ARIA/Roles/definition_role) role.
...
- HTML [id](/Web/HTML/Global_attributes/id) attribute
...
- [The image `alt` attribute](/Web/API/HTMLImageElement/alt)
- HTML [title](/Web/HTML/Global_attributes/title) attribute
