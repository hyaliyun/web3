...
If there is a [`required`](/Web/HTML/Attributes/required) attribute on a form control that isn't filled out, the form will not submit, and an error message appears reading "Please fill out this field" or something similar. The messaging for native validation varies depending on the browser, and cannot be styled.
...
Note that the alert has the ARIA role attribute set to [`alert`](/Web/Accessibility/ARIA/Roles/alert_role).
...
- [Form Validation](/Learn/Forms/Form_validation) tutorial
