...
The global `aria-braillelabel` attribute is similar to the global [`aria-label`](/Web/Accessibility/ARIA/Attributes/aria-label) in that it defines a string value that labels the current element. While `aria-label` is read by the screen reader, the contents of the `aria-braillelabel` attribute are converted into Braille; providing the user with a recognizable name of the object in braille.
...
- [`aria-brailleroledescription`](/Web/Accessibility/ARIA/Attributes/aria-brailleroledescription)
