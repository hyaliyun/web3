...
Providing a mechanism, such as a [skip link](/Web/HTML/Element/a#skip_links), to bypass blocks of content that are repeated on multiple web pages.
...
When a user navigates using a keyboard, the UI should make it obvious which element currently has focus. Don't alter or remove the browser's default [`:focus`](/Web/CSS/:focus) styling, unless you're making focus even more obvious.
...
All users benefit from information about where they are in a site's hierarchy, especially for sites or applications with large amounts of content or many subsections. [Breadcrumbs](/Glossary/Breadcrumb), [site maps](/Glossary/Site_map), and identifying the current page in the navigation as "current" are all techniques that help communicate the current location.
...
The language of every page must be declared by using the [`lang`](/Web/HTML/Global_attributes/lang) attribute on the {{HTMLElement('html')}} element. Include the `lang` attribute again on text that is in a language other than the main language of the document.
...
Provide an expanded form of the abbreviation the first time it is used, followed by the abbreviation placed within an {{HTMLElement('abbr')}} element. If an abbreviation has no expanded form, or is an abbreviation for a word that is not in the document's main language (such as Latin), explain its meaning. Also, consider using [ruby text](/Web/HTML/Element/ruby) for initialism (pronunciation of acronyms).
...
Creating a change in context should be intentional. Because of this, when a UI feature receives [focus](/Web/API/HTMLElement/focus), it should not trigger further user-facing actions. Rather, users should need to "activate" a feature to trigger the change.
...
If a form control is required, indicate it both visually and [via code](/Web/Accessibility/ARIA/Attributes/aria-required). If a form control changes context, describe what will happen in a way that the user will understand it before they cause the context change to happen.
...
- [What is accessibility](/Learn/Accessibility/What_is_accessibility)
- [Accessibility for seizure disorders](/Web/Accessibility/Seizure_disorders)
- [Understanding WCAG Guidelines](/Web/Accessibility/Understanding_WCAG)
- [Accessibility overview](/Learn/Accessibility)
