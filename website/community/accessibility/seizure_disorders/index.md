...
Understanding color is important for accessibility. See [understanding colors and luminance](/Web/Accessibility/Understanding_Colors_and_Luminance) as it relates to web accessibility and accessibility in general.
...
[CSS transitions](/Web/CSS/CSS_transitions) can be used to set the duration to zero for the initial stage of animation.
...
- **SVGs (Vector)**: The MDN document, ["SVG: Scalable Vector Graphics"](/Web/SVG), notes that _"SVG is a text-based open Web standard. It is explicitly designed to work with other web standards such as [CSS](/Web/CSS), [DOM](/Web/API/Document_Object_Model), and [SMIL](/Web/SVG/SVG_animation_with_SMIL)."_ SVGs can be used as an image like in this example: `<img src="example.svg" alt="This is an image using a svg as a source">`. This means that SVG appearance and animation can be controlled through CSS keyframes and animations. For interaction with JavaScript, see the MDN documents on [SVG Interfaces](/Web/API/Document_Object_Model#svg_dom) and [Applying SVG effects to HTML content](/Web/SVG/Applying_SVG_effects_to_HTML_content).
...
[document.getAnimations()](/Web/API/Document/getAnimations) is an experimental technology, and includes [CSS Animations](/Web/CSS/CSS_animations), [CSS Transitions](/Web/CSS/CSS_transitions), and [Web Animations](/Web/API/Web_Animations_API). The MDN page on [Document.getAnimations()](/Web/API/Document/getAnimations) provides the following code sample of how to slow down all animations on a page to half speed:
...
JavaScript is also an option for reducing contrast dynamically. Here's a code example from the section titled, ["Example: Setting the background color of a paragraph"](/Web/API/Document_Object_Model/Traversing_an_HTML_table_with_JavaScript_and_DOM_Interfaces#setting_the_background_color_of_a_paragraph) from the MDN document, [Traversing an HTML table with JavaScript and DOM Interfaces](/Web/API/Document_Object_Model/Traversing_an_HTML_table_with_JavaScript_and_DOM_Interfaces). Notice that the color in the example is described in the **RGB** color space.

**HTML Content [(link to source page)](/Web/API/Document_Object_Model/Traversing_an_HTML_table_with_JavaScript_and_DOM_Interfaces#html_2)**
...
**JavaScript Content [(link to source page)](/Web/API/Document_Object_Model/Traversing_an_HTML_table_with_JavaScript_and_DOM_Interfaces#javascript_2)**
...
To see a great example of how to use the code `prefers-reduced-motion`, visit the MDN document, [`prefers-reduced-motion`](/Web/CSS/@media/prefers-reduced-motion), or see the example below from the section on ["New in Chrome 74"](https://developer.chrome.com/blog/new-in-chrome-74/).
...
There is a powerful tool available to developers via Window.matchMedia(). A great resource is MDN's document on [`Window.matchMedia()`](/Web/API/Window/matchMedia).
...
- [`forced-colors`](/Web/CSS/@media/forced-colors)
...
- [Accessibility: What users can do to browse more safely](/Web/Accessibility/Accessibility:_What_users_can_to_to_browse_safely)
- [Accessibility: Understanding color and luminance](/Web/Accessibility/Understanding_Colors_and_Luminance)
- [Applying SVG effects to HTML Content](/Web/SVG/Applying_SVG_effects_to_HTML_content)
- [Basic Animations](/Web/API/Canvas_API/Tutorial/Basic_animations) (Canvas Tutorial)
- [Canvas API](/Web/API/Canvas_API)
- [CanvasRenderingContext2D.drawImage()](/Web/API/CanvasRenderingContext2D/drawImage)
- [\<color>](/Web/CSS/color_value)
- [Document Object Model](/Web/API/Document_Object_Model)
- [MediaQueryList](/Web/API/MediaQueryList)
- [Using dynamic styling information](/Web/API/CSS_Object_Model/Using_dynamic_styling_information)
- [WebGL: 2D and 3D graphics for the web](/Web/API/WebGL_API)
- [WebVR API](/Web/API/WebVR_API)
