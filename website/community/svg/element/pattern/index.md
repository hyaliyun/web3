...
The `<pattern>` is referenced by the {{SVGAttr("fill")}} and/or {{SVGAttr("stroke")}} attributes on other [graphics elements](/Web/SVG/Tutorial/Basic_Shapes) to fill or stroke those elements with the referenced pattern.
...
- {{SVGAttr("patternTransform")}}
  - : This attribute contains the definition of an optional additional transformation from the pattern coordinate system onto the target coordinate system.
    _Value type_: **[\<transform-list>](/Web/SVG/Content_type#transform-list)**; _Default value_: _none_; _Animatable_: **yes**
