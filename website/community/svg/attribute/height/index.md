...
<table class="properties">
  <tbody>
    <tr>
      ...
      <td>
        <strong
          ><a href="/Web/SVG/Content_type#length">&#x3C;length></a></strong
        >
        |
        <strong
...
> **Note:** `height` has no effect on `use` elements, unless the element referenced has a [viewBox](/Web/SVG/Attribute/viewBox) - i.e. they only have an effect when `use` refers to a `svg` or `symbol` element.
