...
In this example, we define three shapes using {{SVGElement('path')}} elements, each with a different [`stroke`](/Web/SVG/Attribute/stroke) and `fill` color set on them. We also define a {{SVGElement('circle')}} element as a marker via the {{SVGElement('marker')}} element. Each shape has the marker applied via the `marker` CSS property.
...
<table class="properties">
  <tbody>
    <tr>
      ...
      <td>
        <strong
          ><a href="/Web/SVG/Content_type#paint">&#x3C;paint></a></strong
        >
      </td>
    </tr>
    <tr>
