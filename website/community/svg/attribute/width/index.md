...
> **Note:** `width` has no effect on `use` elements, unless the element referenced has a [viewBox](/Web/SVG/Attribute/viewBox) - i.e. they only have an effect when `use` refers to a `svg` or `symbol` element.
