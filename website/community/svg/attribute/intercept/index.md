...
The `linear` function is defined as `slope * color + intercept`, where `color` is the color value, the `intercept` provides a base value for the result, and the [`slope`](/Web/SVG/Attribute/slope) is a scaling factor.
