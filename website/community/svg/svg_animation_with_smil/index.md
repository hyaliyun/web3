...
- animate the numeric attributes of an element ([x](/Web/SVG/Attribute/x), [y](/Web/SVG/Attribute/y), etc.)
- animate [transform](/Web/SVG/Attribute/transform) attributes ([translation](/Web/SVG/Attribute/transform#translate), [rotate](/Web/SVG/Attribute/transform#rotate), etc.)
- animate [color](/Web/SVG/Attribute/color) attributes
- follow a motion [path](/Web/SVG/Element/path)

The following sections show how to use SMIL in [SVG](/Web/SVG) for these four use cases.

## Animating attributes of an element

The following example animates the [`cx` attribute](/Web/SVG/Attribute/cx) of a circle. To do so, we add an {{ SVGElement("animate") }} element inside the {{ SVGElement("circle") }} element. The important attributes for {{ SVGElement("animate") }} are:
...
The {{ SVGElement("animateTransform") }} element let you animate [transform](/Web/SVG/Attribute/transform) attributes. This element is necessary because we are not animating a single attribute like [x](/Web/SVG/Attribute/x) which is a number. Rotation attributes look like this: `rotation(theta, x, y)`, where `theta` is the angle in degrees, and `x` and `y` are absolute positions. In the example below, we animate the center of the rotation and the angle.
