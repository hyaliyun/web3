...
- The nearest [section](/Web/HTML/Content_categories#sectioning_content) ancestor of the `<dfn>` element,
...
The [`title`](/Web/HTML/Global_attributes#title) attribute has special meaning, as noted below.
...
1. If the `<dfn>` element has a [`title`](/Web/HTML/Global_attributes#title) attribute, the value of the `title` attribute is considered to be the term being defined. The element must still have text within it, but that text may be an abbreviation (perhaps using {{HTMLElement("abbr")}}) or another form of the term.
...
Here we see the definition — now with an [`id`](/Web/HTML/Global_attributes#id) attribute, `"definition-dfn"`, which can be used as the target of a link. Later on, a link is created using `<a>` with the [`href`](/Web/HTML/Element/a#href) attribute set to `"#definition-dfn"` to set up the link back to the definition.
...
<table class="properties">
  <tbody>
    ...
    <tr>
      ...
      <td><a href="/Web/Accessibility/ARIA/Roles/term_role"><code>term</code></a></td>
    </tr>
    <tr>
