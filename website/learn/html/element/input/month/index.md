...
In browsers that don't support `month` inputs, the control degrades gracefully to a simple [`<input type="text">`](/Web/HTML/Element/input/text), although there may be automatic validation of the entered text to ensure it's formatted as expected.
...
The format of the month string used by this input type is described in [Month strings](/Web/HTML/Date_and_time_formats#month_strings).

### Setting a default value

You can set a default value for the input control by including a month and year inside the [`value`](/Web/HTML/Element/input#value) attribute, like so:
...
Any values in the list that are not compatible with the [`type`](/Web/HTML/Element/input#type) are not included in the suggested options.
...
If the [`value`](/Web/HTML/Element/input#value) entered into the element exceeds this, the element fails [constraint validation](/Web/HTML/Constraint_validation).
...
If the [`value`](/Web/HTML/Element/input#value) of the element is less than this, the element fails [constraint validation](/Web/HTML/Constraint_validation).
...
Only values which are equal to the basis for stepping ([`min`](#min) if specified, [`value`](/Web/HTML/Element/input#value) otherwise, and an appropriate default value if neither of those is provided) are valid.
...
The simplest use of `<input type="month">` involves a basic  and {{htmlelement("label")}} element combination, as seen below:
...
You can use the [`min`](/Web/HTML/Element/input#min) and [`max`](/Web/HTML/Element/input#max) attributes to restrict the range of dates that the user can choose.
...
`<input type="month">` doesn't support form sizing attributes such as [`size`](/Web/HTML/Element/input#size).
You'll have to resort to [CSS](/Web/CSS) for sizing needs.
...
To help avoid this, you can use [`min`](/Web/HTML/Element/input#min) and [`max`](/Web/HTML/Element/input#max) to restrict the available dates (see [Setting maximum and minimum dates](#setting_maximum_and_minimum_dates)), and in addition use the [`required`](/Web/HTML/Element/input#required) attribute to make filling in the date mandatory.
...
One way around this is to put a [`pattern`](/Web/HTML/Element/input#pattern) attribute on your `month` input.
...
In order to detect whether the browser supports `<input type="month">`, we create a new  element, try setting its `type` to `month`, then immediately check what its type is set to.
...
- [`<input type="datetime-local">`](/Web/HTML/Element/input/datetime-local), [`<input type="date">`](/Web/HTML/Element/input/date), [`<input type="time">`](/Web/HTML/Element/input/time), and [`<input type="week">`](/Web/HTML/Element/input/week)
