...
An `<input type="button">` elements' [`value`](/Web/HTML/Element/input#value) attribute contains a string that is used as the button's label. The `value` provides the {{glossary("accessible description")}} for the button.
...
`<input type="button">` elements have no default behavior (their cousins, `<input type="submit">` and [`<input type="reset">`](/Web/HTML/Element/input/reset) are used to submit and reset forms, respectively). To make buttons do anything, you have to write JavaScript code to do the work.
...
In this example, <kbd>s</kbd> is specified as the access key (you'll need to press <kbd>s</kbd> plus the particular modifier keys for your browser/OS combination; see [accesskey](/Web/HTML/Global_attributes/accesskey) for a useful list of those).
...
To disable a button, specify the [`disabled`](/Web/HTML/Attributes/disabled) global attribute on it, like so:
...
> [!NOTE]
> Unlike other browsers, Firefox persists the `disabled` state of an `<input>` element even after the page is reloaded. As a workaround, set the `<input>` element's [`autocomplete`](/Web/HTML/Element/input#autocomplete) attribute to `off`. (See [Firefox bug 654072](https://bugzil.la/654072) for more details.)
