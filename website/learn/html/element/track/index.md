...
The tracks are formatted in [WebVTT format](/Web/API/WebVTT_API) (`.vtt` files) — Web Video Text Tracks.
...
- `src`
  - : Address of the track (`.vtt` file). Must be a valid URL. This attribute must be specified and its URL value must have the same origin as the document — unless the {{HTMLElement("audio")}} or {{HTMLElement("video")}} parent element of the `track` element has a [`crossorigin`](/Web/HTML/Attributes/crossorigin) attribute.
- `srclang`
...
- [WebVTT text track format](/Web/API/WebVTT_API)
