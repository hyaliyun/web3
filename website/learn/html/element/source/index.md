...
This element supports all [global attributes](/Web/HTML/Global_attributes). In addition, the following attributes can be used with it:

- `type`

  - : Specifies the [MIME media type of the image](/Web/Media/Formats/Image_types) or [other media type](/Web/Media/Formats/Containers), optionally including a [`codecs` parameter](/Web/Media/Formats/codecs_parameter).
...
- `media`
  ...
  - : Specifies the [media query](/Web/CSS/CSS_media_queries) for the resource's intended media.
...
For more examples, the [Video and audio content](/Learn/HTML/Multimedia_and_embedding/Video_and_audio_content) article in the Learn area is a great resource.
...
A [media query](/Web/CSS/CSS_media_queries/Using_media_queries) allows the browser to select an image to display with the `height` and `width` attributes based on the [viewport](/Glossary/Viewport) size.
...
- [Media type and format guide](/Web/Media/Formats)
- [Web performance](/Learn/Performance)
