...
- Displaying exponents, such as "x<sup>3</sup>". It may be worth considering the use of [MathML](/Web/MathML) for these, especially in more complex cases. See [Exponents](#exponents) under [Examples](#examples) below.
...
<table class="properties">
  <tbody>
    ...
    <tr>
      ...
      <td>
        <code
          ><a href="/Web/Accessibility/ARIA/Roles/structural_roles#structural_roles_with_html_equivalents">superscript</a
          ></code
        >
      </td>
    </tr>
...
- The {{HTMLElement("sub")}} HTML element that produces subscripts. Note that you cannot use `sub` and `sup` at the same time: you need to use [MathML](/Web/MathML) to produce both a superscript and a subscript next to the chemical symbol of an element, representing its atomic number and its nuclear number.
- The [`<msub>`](/Web/MathML/Element/msub), [`<msup>`](/Web/MathML/Element/msup), and [`<msubsup>`](/Web/MathML/Element/msubsup) MathML elements.
