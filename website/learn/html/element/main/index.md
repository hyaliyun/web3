...
The **`<main>`** [HTML](/Web/HTML) element represents the dominant content of the {{HTMLElement("body")}} of a document. The main content area consists of content that is directly related to or expands upon the central topic of a document, or the central functionality of an application.

{{EmbedInteractiveExample("pages/tabbed/main.html","tabbed-shorter")}}

A document mustn't have more than one `<main>` element that doesn't have the [`hidden`](/Web/HTML/Global_attributes#hidden) attribute specified.
...
The `<main>` element behaves like a [`main` landmark](/Web/Accessibility/ARIA/Roles/main_role) role. [Landmarks](/Web/Accessibility/ARIA/ARIA_Techniques#landmark_roles) can be used by assistive technology to quickly identify and navigate to large sections of the document. Prefer using the `<main>` element over declaring `role="main"`, unless there are [legacy browser support concerns](#browser_compatibility).
...
Adding an [`id`](/Web/HTML/Global_attributes#id) attribute to the `<main>` element lets it be a target of a skip navigation link.
...
Browser reader mode functionality looks for the presence of the `<main>` element, as well as [heading](/Web/HTML/Element/Heading_Elements) and [content sectioning elements](/Web/HTML/Element#content_sectioning) when converting content into a specialized reader view.
