...
An import map is a JSON object that allows developers to control how the browser resolves module specifiers when importing [JavaScript modules](/Web/JavaScript/Guide/Modules).
It provides a mapping between the text used as the module specifier in an [`import` statement](/Web/JavaScript/Reference/Statements/import) or [`import()` operator](/Web/JavaScript/Reference/Operators/import), and the corresponding value that will replace the text when resolving the specifier.
...
Note that the import map applies only to module specifiers in the [`import` statement](/Web/JavaScript/Reference/Statements/import) or [`import()` operator](/Web/JavaScript/Reference/Operators/import) for modules loaded into documents; it does not apply to the path specified in the `src` attribute of a `<script>` element or to modules loaded into workers or worklets.

For more information, see the [Importing modules using import maps](/Web/JavaScript/Guide/Modules#importing_modules_using_import_maps) section in the JavaScript modules guide.
...
An [`error` event](/Web/API/HTMLElement/error_event) is fired at script elements with `type="importmap"` that are not processed.
...
When importing a [JavaScript module](/Web/JavaScript/Guide/Modules), both the [`import` statement](/Web/JavaScript/Reference/Statements/import) and [`import()` operator](/Web/JavaScript/Reference/Operators/import) have a "module specifier" that indicates the module to be imported.
...
You can use the `integrity` key to provide mapping for module [integrity metadata](/Web/Security/Subresource_Integrity#using_subresource_integrity).
...
The map values represent integrity metadata, identical to that used in [`integrity`](/Web/HTML/Element/script#integrity) attribute values.
...
- `integrity` {{optional_inline}}
  ...
  - : Defines a valid JSON object where the _keys_ are strings containing valid absolute or relative URLs (starting with `/`, `./`, or `../`),
    and the corresponding _values_ are valid [integrity metadata](/Web/Security/Subresource_Integrity#using_subresource_integrity).
...
- [JavaScript modules > Importing modules using import maps](/Web/JavaScript/Guide/Modules#importing_modules_using_import_maps)
- [The `type` attribute of HTML `<script>` elements](/Web/HTML/Element/script#type)
- [`import` statement](/Web/JavaScript/Reference/Statements/import)
- [`import()` operator](/Web/JavaScript/Reference/Operators/import)
