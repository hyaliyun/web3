...
The **`popover`** [global attribute](/Web/HTML/Global_attributes) is used to designate an element as a popover element.

Popover elements are hidden via `display: none` until opened via an invoking/control element (i.e. a `<button>` or `<input type="button">` with a [`popovertarget`](/Web/HTML/Element/button#popovertarget) attribute) or a {{domxref("HTMLElement.showPopover()")}} call.
...
A popover attribute can have values [`"auto"`](/Web/API/Popover_API/Using#auto_state_and_light_dismiss) (default) or [`"manual"`](/Web/API/Popover_API/Using#using_manual_popover_state).
...
- [`popovertarget`](/Web/HTML/Element/button#popovertarget) HTML attribute
- [`popovertargetaction`](/Web/HTML/Element/button#popovertargetaction) HTML attribute
- [`::backdrop`](/Web/CSS/::backdrop) CSS pseudo-element
- [`:popover-open`](/Web/CSS/:popover-open) CSS pseudo-class
