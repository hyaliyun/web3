...
> [!NOTE]
> While `inert` is a global attribute and can be applied to any element, it is generally used for sections of content. To make individual controls "inert", consider using the [`disabled`](/Web/HTML/Attributes/disabled) attribute, along with CSS [`:disabled`](/Web/CSS/:disabled) styles, instead.
