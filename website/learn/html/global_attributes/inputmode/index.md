...
The **`inputmode`** [global attribute](/Web/HTML/Global_attributes) is an [enumerated](/Glossary/Enumerated) attribute that hints at the type of data that might be entered by the user while editing the element or its contents.
This allows a browser to display an appropriate virtual keyboard.

It is used primarily on  elements, but is usable on any element in [`contenteditable`](/Web/HTML/Global_attributes#contenteditable) mode.

It's important to understand that the `inputmode` attribute doesn't cause any validity requirements to be enforced on input. To require that input conforms to a particular data type, choose an appropriate [`<input>`](/Web/HTML/Element/input#input_types) element type. For specific guidance on choosing  types, see the [Values](#values) section.
...
- [`enterkeyhint`](/Web/HTML/Global_attributes/enterkeyhint) global attribute
