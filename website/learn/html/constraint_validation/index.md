...
The creation of web forms has always been a complex task. While marking up the form itself is easy, checking whether each field has a valid and coherent value is more difficult, and informing the user about the problem may become a headache. [HTML5](/Glossary/HTML5) introduced new mechanisms for forms: it added new semantic types for the {{ HTMLElement("input") }} element and _constraint validation_ to ease the work of checking the form content on the client side. Basic, usual constraints can be checked, without the need for JavaScript, by setting new attributes; more complex constraints can be tested using the Constraint Validation API.

For a basic introduction to these concepts, with examples, see the [Form validation tutorial](/Learn/Forms/Form_validation).
...
- By choosing the most semantically appropriate value for the [`type`](/Web/HTML/Element/input#type) attribute of the {{ HTMLElement("input") }} element, e.g., choosing the `email` type automatically creates a constraint that checks whether the value is a valid email address.
...
The intrinsic constraints for the [`type`](/Web/HTML/Element/input#type) attribute are:

| Input type                                                         | Constraint description                                                                                                                                                      | Associated violation                                                                    |
| ------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------- |
| [`<input type="URL">`](/Web/HTML/Element/input/url)     | The value must be an absolute [URL](/Learn/Common_questions/Web_mechanics/What_is_a_URL), as defined in the [URL Living Standard](https://url.spec.whatwg.org/). | **[TypeMismatch](/Web/API/ValidityState/typeMismatch)** constraint violation |
| [`<input type="email">`](/Web/HTML/Element/input/email) | The value must be a syntactically valid email address, which generally has the format `username@hostname.tld` but can also be local such as `username@hostname`.            | **[TypeMismatch](/Web/API/ValidityState/typeMismatch)** constraint violation |

For both of these input types, if the [`multiple`](/Web/HTML/Element/input#multiple) attribute is set, several values can be set, as a comma-separated list. If any of these do not satisfy the condition described here, the **Type mismatch** constraint violation is triggered.
...
<table class="standard-table">
  ...
  <tbody>
    <tr>
      <td>
        <code
          ><a href="/Web/HTML/Attributes/pattern">pattern</a></code
        >
      </td>
      <td>
      ...
      <td>
        A
        <a href="/Web/JavaScript/Guide/Regular_expressions"
      ...
      <td>
        <a href="/Web/API/ValidityState/patternMismatch"
    ...
    <tr>
      ...
      <td rowspan="3">
        <strong
          ><code
            ><a href="/Web/API/ValidityState/rangeUnderflow"
              >rangeUnderflow</a
            ></code
          ></strong
        >
    ...
    <tr>
      ...
      <td rowspan="3">
        <strong
          ><code
            ><a href="/Web/API/ValidityState/rangeOverflow"
              >rangeOverflow</a
            ></code
          ></strong
        >
    ...
    <tr>
      <td>
        <code
          ><a href="/Web/HTML/Attributes/required">required</a></code
        >
      </td>
      <td>
        <code>text</code>, <code>search</code>, <code>url</code>,
      ...
      <td>
        <strong
          ><code
            ><a href="/Web/API/ValidityState/valueMissing"
              >valueMissing</a
            ></code
          ></strong
        >
    ...
    <tr>
      ...
      <td rowspan="5">
        <strong
          ><code
            ><a href="/Web/API/ValidityState/stepMismatch"
              >stepMismatch</a
            ></code
          ></strong
        >
    ...
    <tr>
      <td>
        <code
          ><a href="/Web/HTML/Attributes/minlength"
            >minlength</a
          ></code
        >
      </td>
      ...
      <td>
        <strong
          ><code
            ><a href="/Web/API/ValidityState/tooShort"
              >tooShort</a
            ></code
          ></strong
        >
    ...
    <tr>
      <td>
        <code
          ><a href="/Web/HTML/Attributes/maxlength"
            >maxlength</a
          ></code
        >
      </td>
      ...
      <td>
        <strong
          ><code
            ><a href="/Web/API/ValidityState/tooLong"
              >tooLong</a
            ></code
          ></strong
        >
...
> [!NOTE]
>
> - If the [`novalidate`](/Web/HTML/Element/form#novalidate) attribute is set on the {{ HTMLElement("form") }} element, interactive validation of the constraints doesn't happen.
> - Calling the `submit()` method on the [`HTMLFormElement`](/Web/API/HTMLFormElement) interface doesn't trigger a constraint validation. In other words, this method sends the form data to the server even if it doesn't satisfy the constraints. Call the `click()` method on a submit button instead.
...
The {{cssxref(':required')}} and {{cssxref(':optional')}} [pseudo-classes](/Web/CSS/Pseudo-classes) allow writing selectors that match form elements that have the [`required`](/Web/HTML/Element/input#required) attribute, or that don't have it.
...
The {{cssxref(':valid')}} and {{cssxref(':invalid')}} [pseudo-classes](/Web/CSS/Pseudo-classes) are used to represent \<input> elements whose content validates and fails to validate respectively according to the input's type setting. These classes allow the user to style valid or invalid form elements to make it easier to identify elements that are either formatted correctly or incorrectly.
...
- The [`ValidityState`](/Web/API/ValidityState) interface describes the object returned by the `validity` property of the element types listed above. It represents various ways that an entered value can be invalid. Together, they help explain why an element's value fails to validate, if it's not valid.
