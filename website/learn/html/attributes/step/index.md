...
If `any` is not explicitly set, valid values for the `number`, date/time input types, and `range` input types are equal to the basis for stepping - the [`min`](/Web/HTML/Attributes/min) value and increments of the step value, up to the [`max`](/Web/HTML/Attributes/max) value, if specified. The following example results in any even integer, 10 or greater, being valid:
...
See [Client-side validation](/Web/HTML/Constraint_validation) and {{domxref("ValidityState.stepMismatch", "stepMismatch")}} for more information.
