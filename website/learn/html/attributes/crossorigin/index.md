...
The **`crossorigin`** attribute, valid on the {{HTMLElement("audio")}}, {{HTMLElement("img")}}, {{HTMLElement("link")}}, {{HTMLElement("script")}}, and {{HTMLElement("video")}} elements, provides support for [CORS](/Web/HTTP/CORS), defining how the element handles cross-origin requests, thereby enabling the configuration of the CORS requests for the element's fetched data. Depending on the element, the attribute can be a CORS settings attribute.

The `crossorigin` content attribute on media elements is a CORS settings attribute.

These attributes are [enumerated](/Glossary/Enumerated), and have the following possible values:
...
<table class="no-markdown">
  <tbody>
    ...
    <tr>
      ...
      <td>
        When resource is placed in {{HTMLElement("canvas")}}, element is marked as <a href="/Web/HTML/CORS_enabled_image#security_and_tainted_canvases"><em>tainted</em></a>.
      </td>
    </tr>
    <tr>
...
> [!NOTE]
> The `crossorigin` attribute is not supported for [`rel="icon"`](/Web/HTML/Attributes/rel#icon) in Chromium-based browsers. See the [open Chromium issue](https://crbug.com/1121645).
...
The `use-credentials` value must be used when fetching a [manifest](/Web/Manifest) that requires credentials, even if the file is from the same origin.
...
{{QuickLinksWithSubpages("/Web/HTML/")}}
