...
The **`<mfrac>`** [MathML](/Web/MathML) element is used to display fractions. It can also be used
...
> [!NOTE]
> For the `linethickness` attribute, some browsers may also accept the deprecated values `medium`, `thin` and `thick` (whose exact interpretation is left to implementers) or [legacy MathML lengths](/Web/MathML/Values#legacy_mathml_lengths).
