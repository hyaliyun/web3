...
The **`<mi>`** [MathML](/Web/MathML) element indicates that the content should be rendered as an **identifier**, such as a function name, variable or symbolic constant.

By default `<mi>` elements that contain multiple characters are a rendered as normal text, while single character characters are rendered as italic: the same formatting behaviour as the [CSS `text-transform`](/Web/CSS/text-transform) property with a value of `math-auto`.
...
This element also accepts the [global MathML attributes](/Web/MathML/Global_attributes).
