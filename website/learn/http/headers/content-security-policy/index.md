...
For more information, see the introductory article on [Content Security Policy (CSP)](/Web/HTTP/CSP).
...
- {{CSP("child-src")}}
  ...
  - : Defines the valid sources for [web workers](/Web/API/Web_Workers_API) and nested browsing contexts loaded using elements such as
...
Document directives govern the properties of a document or [worker](/Web/API/Web_Workers_API) environment to which a policy
applies.
...
- {{CSP("sandbox")}}
  - : Enables a sandbox for the requested resource similar to the
    {{HTMLElement("iframe")}} [`sandbox`](/Web/HTML/Element/iframe#sandbox) attribute.
...
For detailed reference see [CSP Source Values](/Web/HTTP/Headers/Content-Security-Policy/Sources#sources) and the documentation for individual directives.
...
[Workers](/Web/API/Worker) are in general _not_ governed
...
See [Content Security Policy (CSP) implementation](/Web/Security/Practical_implementation_guides/CSP) for more examples.
...
- [Learn about: Content Security Policy](/Web/HTTP/CSP)
- [Content Security in WebExtensions](/Mozilla/Add-ons/WebExtensions/Content_Security_Policy)
