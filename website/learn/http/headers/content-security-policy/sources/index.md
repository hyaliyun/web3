...
- `<host-source>`
  ...
  - : Internet host by name or IP address. The [URL scheme](/Learn/Common_questions/Web_mechanics/What_is_a_URL), port number, and path are optional.
...
- `<scheme-source>`
  ...
  - : A scheme such as `http:` or `https:`.
    ...
    - `data:` Allows [`data:` URLs](/Web/URI/Schemes/data) to be used as a content source.
      _This is insecure; an attacker can also inject arbitrary `data:` URLs. Use this sparingly and definitely not for scripts._
    - `mediastream:` Allows [`mediastream:` URIs](/Web/API/Media_Capture_and_Streams_API) to be used as a content source.
    - `blob:` Allows [`blob:` URIs](/Web/API/Blob) to be used as a content source.
    - `filesystem:` Allows [`filesystem:` URIs](/Web/API/FileSystem) to be used as a content source.
...
- `'unsafe-hashes'`
  - : Allows enabling specific inline [event handlers](/Web/Events/Event_handlers).
    If you only need to allow inline event handlers and not inline {{HTMLElement("script")}} elements or [`javascript:` URLs](/Web/URI/Schemes/javascript), this is a safer method than using the `unsafe-inline` expression.
- `'unsafe-inline'`
  - : Allows the use of inline resources, such as inline {{HTMLElement("script")}} elements, [`javascript:` URLs](/Web/URI/Schemes/javascript), inline event handlers, and inline {{HTMLElement("style")}} elements.
    The single quotes are required.
- `'none'`
...
- `'nonce-<base64-value>'`
  ...
  - : An allowlist for specific inline scripts using a cryptographic nonce (number used once).
    ...
    See [unsafe inline script](/Web/HTTP/Headers/Content-Security-Policy/script-src#unsafe_inline_script) for an example.
...
- `'<hash-algorithm>-<base64-value>'`
  - : A sha256, sha384 or sha512 hash of scripts or styles.
    ...
    See the [script-src](/Web/HTTP/Headers/Content-Security-Policy/script-src#unsafe_inline_script) and [style-src](/Web/HTTP/Headers/Content-Security-Policy/style-src#unsafe_inline_styles) pages for more information and examples.
- `'strict-dynamic'`
  - : The `strict-dynamic` source expression specifies that the trust explicitly given to a script present in the markup, by accompanying it with a nonce or a hash, shall be propagated to all the scripts loaded by that root script.
    ...
    See [script-src](/Web/HTTP/Headers/Content-Security-Policy/script-src#strict-dynamic) for an example.
- `'report-sample'`
  - : Requires a sample of the violating code to be included in the violation report.
- `'inline-speculation-rules'`
  - : Allows the inclusion of [speculation rules](/Web/API/Speculation_Rules_API) in scripts (see also [`<script type="speculationrules">`](/Web/HTML/Element/script/type/speculationrules)).
