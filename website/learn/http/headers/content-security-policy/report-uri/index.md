...
- `script-sample`
  ...
  - : The first 40 characters of the inline script, event handler, or style that caused the violation.
    ...
    This is only applicable to [`script-src*`](/Web/HTTP/Headers/Content-Security-Policy#script-src) and [`style-src*`](/Web/HTTP/Headers/Content-Security-Policy#style-src) violations, when the corresponding `Content-Security-Policy` directive contains the [`'report-sample'`](/Web/HTTP/Headers/Content-Security-Policy#report-sample) keyword.
