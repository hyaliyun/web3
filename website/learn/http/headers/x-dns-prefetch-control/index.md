...
individual documents, using the [`http-equiv`](/Web/HTML/Element/meta#http-equiv) attribute on
...
that hostname by using the [`rel`](/Web/HTML/Element/link#rel) attribute on the
{{HTMLElement("link")}} element with a [link type](/Web/HTML/Attributes/rel) of `dns-prefetch`:
