...
If a cross-origin document with COOP is opened in a new window, the opening document will not have a reference to it, and the [`window.opener`](/Web/API/Window/opener) property of the new window will be `null`. This allows you to have more control over references to a window than [`rel=noopener`](/Web/HTML/Attributes/rel/noopener), which only affects outgoing navigations.
