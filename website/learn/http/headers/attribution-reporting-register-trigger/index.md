...
The **`Attribution-Reporting-Register-Trigger`** header registers a page feature as an [attribution trigger](/Web/API/Attribution_Reporting_API/Registering_triggers). This is included as part of a response to a request that included an {{httpheader("Attribution-Reporting-Eligible")}} header.

See the [Attribution Reporting API](/Web/API/Attribution_Reporting_API) for more details.

> [!NOTE]
> If the calling site does not have the Attribution Reporting API included in a successful [privacy sandbox enrollment process](/Web/Privacy/Privacy_sandbox/Enrollment), the `Attribution-Reporting-Register-Trigger` header is ignored and attribution triggers are not registered.
...
- `<json-string>`
  ...
  - : A JSON string providing data that can be included in generated reports, such as the ID of the trigger, and priority and deduplication values. Available fields are as follows:
    ...
    - `"debug_key"` {{optional_inline}}
      - : A number representing a debug key. Set this if you want to generate a [debug report](/Web/API/Attribution_Reporting_API/Generating_reports#debug_reports) alongside the associated attribution report.
    ...
    - `"filters"` {{optional_inline}}
      - : An object containing custom data that can be used to filter which triggers generate reports. See [Filters](/Web/API/Attribution_Reporting_API/Generating_reports#filters) for more details.
    - `"event_trigger_data"`
