...
The **`Cache-Control`** HTTP header field holds _directives_ (instructions) — in both requests and responses — that control [caching](/Web/HTTP/Caching) in browsers and shared caches (e.g. Proxies, CDNs).
...
- Revalidate response
  - : Ask the origin server whether or not the stored response is still [fresh](/Web/HTTP/Caching#fresh_and_stale_based_on_age). Usually, the revalidation is done through a conditional request.
- Fresh response
  - : Indicates that the response is [fresh](/Web/HTTP/Caching#fresh_and_stale_based_on_age). This usually means the response can be reused for subsequent requests, depending on request directives.
- Stale response
  - : Indicates that the response is a [stale response](/Web/HTTP/Caching#fresh_and_stale_based_on_age). This usually means the response can't be reused as-is. Cache storage isn't required to remove stale responses immediately because revalidation could change the response from being stale to being [fresh](/Web/HTTP/Caching#fresh_and_stale_based_on_age) again.
- Age
  - : The time since a response was generated. It is a criterion for whether a response is [fresh or stale](/Web/HTTP/Caching#fresh_and_stale_based_on_age).
...
The `max-age=N` response directive indicates that the response remains [fresh](/Web/HTTP/Caching#fresh_and_stale_based_on_age) until _N_ seconds after the response is generated.

```http
Cache-Control: max-age=604800
```

Indicates that caches can store this response and reuse it for subsequent requests while it's [fresh](/Web/HTTP/Caching#fresh_and_stale_based_on_age).
...
So if the other cache(s) — on the network route taken by the response — store the response for 100 seconds (indicated using the `Age` response header field), the browser cache would deduct 100 seconds from its [freshness lifetime](/Web/HTTP/Caching#fresh_and_stale_based_on_age).
...
The `s-maxage` response directive indicates how long the response remains [fresh](/Web/HTTP/Caching#fresh_and_stale_based_on_age) in a shared cache.
...
The `must-revalidate` response directive indicates that the response can be stored in caches and can be reused while [fresh](/Web/HTTP/Caching#fresh_and_stale_based_on_age). If the response becomes [stale](/Web/HTTP/Caching#fresh_and_stale_based_on_age), it must be validated with the origin server before reuse.
...
The `immutable` response directive indicates that the response will not be updated while it's [fresh](/Web/HTTP/Caching#fresh_and_stale_based_on_age).
...
`immutable` tells a cache that the response is immutable while it's [fresh](/Web/HTTP/Caching#fresh_and_stale_based_on_age) and avoids those kinds of unnecessary conditional requests to the server.
...
In the example above, the response is [fresh](/Web/HTTP/Caching#fresh_and_stale_based_on_age) for 7 days (604800s).
After 7 days it becomes [stale](/Web/HTTP/Caching#fresh_and_stale_based_on_age), but the cache is allowed to reuse it for any requests that are made in the following day (86400s), provided that they revalidate the response in the background.

Revalidation will make the cache be [fresh](/Web/HTTP/Caching#fresh_and_stale_based_on_age) again, so it appears to clients that it was always [fresh](/Web/HTTP/Caching#fresh_and_stale_based_on_age) during that period — effectively hiding the latency penalty of revalidation from them.

If no request happened during that period, the cache became [stale](/Web/HTTP/Caching#fresh_and_stale_based_on_age) and the next request will revalidate normally.

#### `stale-if-error`

The `stale-if-error` response directive indicates that the cache can reuse a [stale response](/Web/HTTP/Caching#fresh_and_stale_based_on_age) when an upstream server generates an error, or when the error is generated locally. Here, an error is considered any response with a status code of 500, 502, 503, or 504.

```http
Cache-Control: max-age=604800, stale-if-error=86400
```

In the example above, the response is [fresh](/Web/HTTP/Caching#fresh_and_stale_based_on_age) for 7 days (604800s). Afterwards, it becomes [stale](/Web/HTTP/Caching#fresh_and_stale_based_on_age), but can be used for an extra 1 day (86400s) when an error is encountered.
...
`no-cache` allows clients to request the most up-to-date response even if the cache has a [fresh](/Web/HTTP/Caching#fresh_and_stale_based_on_age) response.
...
The `max-stale=N` request directive indicates that the client allows a stored response that is [stale](/Web/HTTP/Caching#fresh_and_stale_based_on_age) within _N_ seconds.
...
The `min-fresh=N` request directive indicates that the client allows a stored response that is [fresh](/Web/HTTP/Caching#fresh_and_stale_based_on_age) for at least _N_ seconds.
...
Clients can use this header when the user requires the response to not only be [fresh](/Web/HTTP/Caching#fresh_and_stale_based_on_age), but also requires that it won't be updated for a period of time.
...
The client indicates that an already-cached response should be returned. If a cache has a stored response, even a stale one, it will be returned. If no cached response is available, a [504 Gateway Timeout](/Web/HTTP/Status/504) response will be returned.
...
Adding `no-cache` to the response causes revalidation to the server, so you can serve a [fresh](/Web/HTTP/Caching#fresh_and_stale_based_on_age) response every time — or if the client already has a new one, just respond `304 Not Modified`.

```http
Cache-Control: no-cache
```

Most HTTP/1.0 caches don't support `no-cache` directives, so historically `max-age=0` was used as a workaround. But only `max-age=0` could cause a [stale response](/Web/HTTP/Caching#fresh_and_stale_based_on_age) to be reused when caches disconnected from the origin server. `must-revalidate` addresses that. That's why the example below is equivalent to `no-cache`.
...
Imagine that clients/caches store a [fresh](/Web/HTTP/Caching#fresh_and_stale_based_on_age) response for a path, with no request flight to the server. There is nothing a server could do to that path.
