...
The **`Sec-CH-Prefers-Reduced-Motion`** [user agent client hint](/Web/HTTP/Client_hints#user_preference_media_features_client_hints) request header indicates the user agent's preference for animations to be displayed with reduced motion.
...
The server responds, telling the client via {{httpheader("Accept-CH")}} that it accepts `Sec-CH-Prefers-Reduced-Motion`. In this example {{httpheader("Critical-CH")}} is also used, indicating that `Sec-CH-Prefers-Reduced-Motion` is considered a [critical client hint](/Web/HTTP/Client_hints#critical_client_hints).
...
- [`prefers-reduced-motion` CSS Media Query](/Web/CSS/@media/prefers-reduced-motion)
