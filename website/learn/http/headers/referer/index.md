...
The request's _referrer policy_ defines the data that can be included. See {{HTTPHeader("Referrer-Policy")}} for more [information](/Web/HTTP/Headers/Referrer-Policy#directives) and [examples](/Web/HTTP/Headers/Referrer-Policy#examples).
...
> See [Referer header: privacy and security concerns](/Web/Security/Referer_header:_privacy_and_security_concerns) for more information and mitigation hints.
...
- \<url>
  - : An absolute or partial address of the web page that makes the request.
    ...
    Origin, path, and query string may be included, depending on the [referrer policy](/Web/HTTP/Headers/Referrer-Policy#directives).

## Examples

```http
Referer: https://developer.mozilla.org/Web/JavaScript
...
- [Fetch](/Web/API/Fetch_API): {{domxref("Request.referrerPolicy")}}
