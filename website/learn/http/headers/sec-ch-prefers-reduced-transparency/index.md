...
The **`Sec-CH-Prefers-Reduced-Transparency`** [user agent client hint](/Web/HTTP/Client_hints#user_preference_media_features_client_hints) request header indicates the user agent's preference for reduced transparency.
...
The server responds, telling the client via {{httpheader("Accept-CH")}} that it accepts `Sec-CH-Prefers-Reduced-Transparency`. In this example {{httpheader("Critical-CH")}} is also used, indicating that `Sec-CH-Prefers-Reduced-Transparency` is considered a [critical client hint](/Web/HTTP/Client_hints#critical_client_hints).
