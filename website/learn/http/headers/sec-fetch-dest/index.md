...
- `fencedframe` {{experimental_inline}}
  - : The destination is a [fenced frame](/Web/API/Fenced_frame_API).
- `font`
...
- `manifest`
  - : The destination is a manifest. This might originate from an HTML [\<link rel=manifest>](/Web/HTML/Attributes/rel/manifest).
- `object`
...
- `webidentity`
  - : The destination is an endpoint associated with verifying user identify. For example, it is used in the [FedCM API](/Web/API/FedCM_API) to verify the authenticity of identity provider (IdP) endpoints, guarding against {{glossary("CSRF")}} attacks.
- `worker`
