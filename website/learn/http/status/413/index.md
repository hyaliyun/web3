...
The HTTP **`413 Content Too Large`** [client error response](/Web/HTTP/Status#client_error_responses) status code indicates that the request entity was larger than limits defined by server.
...
The following example shows what the client may send when an [`<input type="file">`](/Web/HTML/Element/input/file) element includes an image on form submission with `method="post"`:
