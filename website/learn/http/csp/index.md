...
Browsers that don't support it still work with servers that implement it, and vice versa. Browsers that don't support CSP ignore it, functioning as usual; they will only apply the protections of the standard [same-origin policy](/Web/Security/Same-origin_policy) without the further restrictions that the CSP would add.
...
A complete data transmission security strategy includes not only enforcing HTTPS for data transfer, but also marking all [cookies with the `secure` attribute](/Web/HTTP/Cookies) and providing automatic redirects from HTTP pages to their HTTPS counterparts.
...
For a complete list of policy directives, see the reference page for the [Content-Security-Policy header](/Web/HTTP/Headers/Content-Security-Policy).
...
The recommended method for reporting CSP violations is to use the [Reporting API](/Web/API/Reporting_API), declaring endpoints in {{HTTPHeader("Reporting-Endpoints")}} and specifying one of them as the CSP reporting target using the `Content-Security-Policy` header's {{CSP("report-to")}} directive.
...
- [CSP in Web Workers](/Web/HTTP/Headers/Content-Security-Policy#csp_in_workers)
