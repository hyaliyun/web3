...
To fix this problem, make sure you use HTTPS URLs when issuing requests involving CORS, such as {{domxref("Window/fetch", "fetch()")}}, {{domxref("XMLHttpRequest")}}, Web Fonts (`@font-face`), and [WebGL textures](/Web/API/WebGL_API/Tutorial/Using_textures_in_WebGL), and XSL stylesheets.

### Loading a local file

Local files from the same directory and subdirectories were historically treated as being from the [same origin](/Web/Security/Same-origin_policy).
...
Developers who need to perform local testing should now [set up a local server](/Learn/Common_questions/Tools_and_setup/set_up_a_local_testing_server).
