...
A server can advertise support for `PATCH` by adding it to the list in the {{HTTPHeader("Allow")}} or {{HTTPHeader("Access-Control-Allow-Methods")}} (for [CORS](/Web/HTTP/CORS)) response headers.
...
Success can be indicated by any of the [successful response status codes](/Web/HTTP/Status#successful_responses).
...
An {{HTTPHeader("ETag")}} is provided so the caller can perform a [conditional request](/Web/HTTP/Conditional_requests) in future:
