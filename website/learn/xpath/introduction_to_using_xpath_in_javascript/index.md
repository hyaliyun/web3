...
This document describes the interface for using [XPath](/Web/XPath) in JavaScript. Mozilla implements a fair amount of the [DOM 3 XPath](https://www.w3.org/TR/2004/NOTE-DOM-Level-3-XPath-20040226/), which means that XPath expressions can be run against both HTML and XML documents.

The main interface to using XPath is the [evaluate](/Web/API/Document/evaluate) function of the [document](/Web/API/Document) object.

## document.evaluate()

This method evaluates [XPath](/Web/XPath) expressions against an [XML](/Glossary/XML) based document (including HTML documents), and returns a [`XPathResult`](/Web/API/XPathResult) object, which can be a single node or a set of nodes. The existing documentation for this method is located at [document.evaluate](/Web/API/Document/evaluate), but it is rather sparse for our needs at the moment; a more comprehensive examination will be given below.
...
The [`evaluate()`](/Web/API/Document/evaluate) method takes a total of five parameters:

- `xpathExpression`: A string containing the XPath expression to be evaluated.
- `contextNode`: A node in the document against which the `xpathExpression` should be evaluated, including any and all of its child nodes. The [document](/Web/API/Document) node is the most commonly used.
...
Returns `xpathResult`, which is an `XPathResult` object of the type [specified](#specifying_the_return_type) in the `resultType` parameter. The `XPathResult` Interface is defined [here](/Web/API/XPathResult).

### Implementing a Default Namespace Resolver

We use the [`document`](/Web/API/Document) object as a namespace resolver.
...
The following uses the XPath expression [`count(//p)`](/Web/XPath/Functions/count) to obtain the number of `<p>` elements in an HTML document:
...
Since we wish to search over the entire document for the headings, we have used the [document](/Web/API/Document) object itself as the `contextNode`.
...
will select all [MathML](/Web/MathML) expressions that are the children of (X)HTML table data cell elements.
...
While one can adapt the approach in the above section to test for namespaced elements regardless of the prefix chosen (using [`local-name()`](/Web/XPath/Functions/local-name) in combination with [`namespace-uri()`](/Web/XPath/Functions/namespace-uri) instead of [`name()`](/Web/XPath/Functions/name)), a more challenging situation occurs, however, if one wishes to grab an element with a particular namespaced attribute in a predicate (given the absence of implementation-independent variables in XPath 1.0).
...
This could inadvertently grab some elements if one of its attributes existed that had a local name of `href`, but it was a different attribute which had the targeted (XLink) namespace (instead of [`@href`](/Web/XPath/Axes#attribute)).
