...
The **`view-timeline-name`** [CSS](/Web/CSS) property is used to define the name of a _named view progress timeline_, which is progressed through based on the change in visibility of an element (known as the _subject_) inside a scrollable element (_scroller_). `view-timeline` is set on the subject.
...
The {{cssxref("view-timeline-axis")}} and `view-timeline-name` properties can also be set using the [`view-timeline`](/Web/CSS/scroll-timeline) shorthand property.
...
- `<dashed-ident>`
  ...
  - : An arbitrary custom identifier defining a name for a view progress timeline, which can then be referenced in an [`animation-timeline`](/Web/CSS/animation-timeline) property.
...
- [`view-timeline`](/Web/CSS/view-timeline), [`view-timeline-axis`](/Web/CSS/view-timeline-axis)
