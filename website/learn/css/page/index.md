...
The **`page`** [CSS](/Web/CSS) property is used to specify the named page, a specific type of page defined by the {{cssxref("@page")}} [at-rule](/Web/CSS/At-rule).

If there are multiple selectors that are using a named page consecutively then a forced page break using [`break-after`](/Web/CSS/break-after) may be needed.
...
- {{cssxref("custom-ident")}}
  - : Case-sensitive name defined in a [`@page`](/Web/CSS/@page) at-rule.
...
The first part of the CSS sets up the **named** pages, these include the size and orientation and also some content to go in the [`@top-center` margin](/Web/CSS/@page#margin_at-rules) of the printed pages.
...
The next part of the CSS uses [attribute selectors](/Web/CSS/Attribute_selectors) to apply the print dimensions, orientation, and margins defined in the named `@page` rules defined in the previous CSS section to elements using the `page` property.
