...
The **`border-image-slice`** [CSS](/Web/CSS) property divides the image specified by {{cssxref("border-image-source")}} into regions. These regions form the components of an element's [border image](/Web/CSS/border-image).
...
- Zones 5-8 are edge regions. These are [repeated, scaled, or otherwise modified](/Web/CSS/border-image-repeat) in the final border image to match the dimensions of the element.
...
The diamonds are 30px across, therefore setting 30 pixels as the value for both [`border-width`](/Web/CSS/border-width) and `border-image-slice` will get you complete and fairly crisp diamonds in your border:
...
- [Illustrated description of the 1-to-4-value syntax](/Web/CSS/Shorthand_properties#tricky_edge_cases)
