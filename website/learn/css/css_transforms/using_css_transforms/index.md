...
> [!WARNING]
> Only transformable elements can be `transform`ed; that is, all elements whose layout is governed by the CSS [box model](/Web/CSS/CSS_box_model) except for: [non-replaced inline boxes](/Web/CSS/Visual_formatting_model#inline-level_and_block-level_boxes), [table-column boxes](/Web/HTML/Element/col), and [table-column-group boxes](/Web/HTML/Element/colgroup).
...
- The [CSS `transform` property](/Web/CSS/transform) and the [CSS `<transform-function>` data types](/Web/CSS/transform-function)
- The individual transforms properties: {{cssxref('translate')}}, {{cssxref('rotate')}}, and {{cssxref('scale')}} (There is no `skew` property)
- [Using device orientation with 3D Transforms](/Web/API/Device_orientation_events/Using_device_orientation_with_3D_transforms)
