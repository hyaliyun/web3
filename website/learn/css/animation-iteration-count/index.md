...
The **`animation-iteration-count`** [CSS](/Web/CSS) property sets the number of times an animation sequence should be played before stopping.
...
> [!NOTE]
> When creating [CSS scroll-driven animations](/Web/CSS/CSS_scroll-driven_animations), specifying an `animation-iteration-count` causes the animation to repeat that number of times over the course of the timeline's progression. If an `animation-iteration-count` is not provided, the animation will only occur once. `infinite` is a valid value for scroll-driven animations, but it results in an animation that doesn't work.
