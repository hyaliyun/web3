...
> [!NOTE]
> For information on the basic fundamentals of CSS anchor positioning, see [Using CSS anchor positioning](/Web/CSS/CSS_anchor_positioning/Using).
...
The predefined fallback option values of the `position-try-fallbacks` property (defined as [`<try-tactic>`](/Web/CSS/position-try-fallbacks#try-tactic)s in the spec) will "flip" the position of the anchor-positioned element across one or both axes if the element would otherwise overflow.
...
- [Inset properties](/Glossary/Inset_properties)
- Margin properties (e.g. {{cssxref("margin-left")}}, {{cssxref("margin-block-start")}})
- [self-alignment](/Web/CSS/CSS_anchor_positioning/Using#centering_on_the_anchor_using_anchor-center) properties
...
Finally, we include some JavaScript that sets a [`change`](/Web/API/HTMLElement/change_event) event handler on the radio buttons. When a radio button is selected, its value is applied to the infobox's `position-try-order` property.
