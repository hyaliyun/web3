...
The [CSS](/Web/CSS) **`align-content`** property sets the distribution of space between and around content items along a [flexbox](/Web/CSS/CSS_flexible_box_layout)'s [cross axis](/Glossary/Cross_Axis), or a [grid](/Web/CSS/CSS_grid_layout) or [block-level](/Glossary/Block-level_content) element's block axis.
...
> [!NOTE]
> The `<content-distribution>` values (`space-between`, `space-around`, `space-evenly`, and `stretch`) have no effect in [block layout](/Web/CSS/CSS_box_alignment/Box_alignment_in_block_abspos_tables#align-content_and_justify-content) as all the content in that block is treated as a single [alignment-subject](/Glossary/Alignment_Subject)
...
In [block layout](/Web/CSS/CSS_box_alignment/Box_alignment_in_block_abspos_tables#align-content_and_justify-content), child elements are treated as a single element, meaning `space-between`, `space-around`, and `space-evenly` behave differently.
...
- [Block-level_content](/Glossary/Block-level_content)
- {{CSSXRef("display")}}
