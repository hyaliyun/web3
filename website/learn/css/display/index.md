...
The **`display`** [CSS](/Web/CSS) property sets whether an element is treated as a [block or inline box](/Web/CSS/CSS_flow_layout) and the layout used for its children, such as [flow layout](/Web/CSS/CSS_flow_layout), [grid](/Web/CSS/CSS_grid_layout) or [flex](/Web/CSS/CSS_flexible_box_layout).

Formally, the **`display`** property sets an element's inner and outer _display types_. The outer type sets an element's participation in [flow layout](/Web/CSS/CSS_flow_layout); the inner type sets the layout of children. Some values of `display` are fully defined in their own individual specifications; for example the detail of what happens when `display: flex` is declared is defined in the CSS Flexible Box Model specification.
...
> ```
>
> See [Using the multi-keyword syntax with CSS display](/Web/CSS/display/multi-keyword_syntax_of_display) for more information.
...
- {{CSSxRef("&lt;display-inside&gt;")}}
  ...
  - : These keywords specify the element's inner display type, which defines the type of formatting context that its contents are laid out in (assuming it is a non-replaced element):
    ...
    - `flow-root`
      - : The element generates a block box that establishes a new [block formatting context](/Web/CSS/CSS_display/Block_formatting_context), defining where the formatting root lies.
    - `table`
      - : These elements behave like HTML {{HTMLElement("table")}} elements. It defines a block-level box.
    - `flex`
      - : The element behaves like a block-level element and lays out its content according to the [flexbox model](/Web/CSS/CSS_flexible_box_layout).
    - `grid`
      - : The element behaves like a block-level element and lays out its content according to the [grid model](/Web/CSS/CSS_grid_layout/Basic_concepts_of_grid_layout).
    - `ruby`
...
The [CSS display module](/Web/CSS/CSS_display) describes a multi-keyword syntax for values you can use with the `display` property to explicitly define **outer** and **inner** display.
...
For more information on these changes, see the [Using the multi-keyword syntax with CSS display](/Web/CSS/display/multi-keyword_syntax_of_display) guide.
...
- [Relationship to other layout methods](/Web/CSS/CSS_grid_layout/Relationship_of_grid_layout_with_other_layout_methods)
...
[Supporting browsers](#browser_compatibility) animate `display` with a [discrete animation type](/Web/CSS/CSS_animated_properties#discrete). This generally means that the property will flip between two values 50% through animating between the two.
...
This behavior is useful for creating entry/exit animations where you want to for example remove a container from the DOM with `display: none`, but have it fade out with [`opacity`](/Web/CSS/opacity) rather than disappearing immediately.

When animating `display` with [CSS animations](/Web/CSS/CSS_animations), you need to provide the starting `display` value in an explicit keyframe (for example using `0%` or `from`). See [Using CSS animations](/Web/CSS/CSS_animations/Using_CSS_animations) for an example.

When animating `display` with [CSS transitions](/Web/CSS/CSS_transitions), two additional features are needed:

- [`@starting-style`](/Web/CSS/@starting-style) provides starting values for properties you want to transition from when the animated element is first shown. This is needed to avoid unexpected behavior. By default, CSS transitions are not triggered on an element's first style update or when the `display` type changes from `none` to another type.
- [`transition-behavior: allow-discrete`](/Web/CSS/transition-behavior) needs to be set on the {{cssxref("transition-property")}} declaration (or the {{cssxref("transition")}} shorthand) to enable `display` transitions.

For examples of transitioning the `display` property, see the [`@starting-style`](/Web/CSS/@starting-style#examples) and [`transition-behavior`](/Web/CSS/transition-behavior#examples) pages.

## Accessibility

### display: none

Using a `display` value of `none` on an element will remove it from the [accessibility tree](/Learn/Accessibility/What_is_accessibility#accessibility_apis). This will cause the element and all its descendant elements to no longer be announced by screen reading technology.
...
- [MDN Understanding WCAG, Guideline 1.3 explanations](/Web/Accessibility/Understanding_WCAG/Perceivable#guideline_1.3_%e2%80%94_create_content_that_can_be_presented_in_different_ways)
