...
The [CSS display module](/Web/CSS/CSS_display) defines a multi-keyword syntax for the CSS [`display`](/Web/CSS/display) property. This guide explains the multi-keyword syntax.
...
One of the first things we learn about CSS is that some elements are block-level and some are inline-level. These are their [outer](/Web/CSS/display-outside) display types. For example, an `<h1>` or a `<p>` are block-level by default, and a `<span>` is inline-level. Using the {{cssxref("display")}} property we can switch between block and inline. For example to make a heading inline we would use the following CSS:
...
The `display` property also lets us use [CSS grid layout](/Web/CSS/CSS_grid_layout) and [Flexbox](/Web/CSS/CSS_flexible_box_layout) when `display: grid` or `display: flex` is set. The important concept to understand is that changing an element's `display` value can change the formatting context of its direct children. When you use `display: flex` or `display: grid`, the element's children become flex or grid items and respond to the properties in grid and flexbox specifications.
...
You might expect the top margin to effectively push the child element down within the parent element, but what happens instead is something called [_margin collapse_](/Web/CSS/CSS_box_model/Mastering_margin_collapsing).
...
This is easier to see if you inspect the box model of the child element [in your browser's developer tools](/Learn/CSS/Building_blocks/The_box_model#use_browser_devtools_to_view_the_box_model).
