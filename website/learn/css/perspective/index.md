...
The **`perspective`** [CSS](/Web/CSS) property determines the distance between the z=0 plane and the user in order to give a 3D-positioned element some perspective.
...
Using this property with a value other than `none` creates a new [stacking context](/Web/CSS/CSS_positioned_layout/Understanding_z-index/Stacking_context). Also, in that case, the object will act as a containing block for `position: fixed` elements that it contains.
...
An example showing how a cube varies if the `perspective` is set at different positions is given in [Using CSS transforms > Setting perspective](/Web/CSS/CSS_transforms/Using_CSS_transforms#setting_perspective).
