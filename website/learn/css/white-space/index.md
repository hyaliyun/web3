...
The **`white-space`** [CSS](/Web/CSS) property sets how {{Glossary("whitespace", "white space")}} inside an element is handled.
...
A tab defaults to 8 spaces and can be configured using the [`tab-size`](/Web/CSS/tab-size) property. In the case of `normal`, `nowrap`, and `pre-line` values, every tab is converted to a space (U+0020) character.
...
The {{cssxref("white-space-collapse")}} property page explains the [browser algorithm for collapsing white space](/Web/CSS/white-space-collapse#collapsing_of_white_space).
