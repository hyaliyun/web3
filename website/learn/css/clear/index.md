...
The **`clear`** [CSS](/Web/CSS) property sets whether an element must be moved below (cleared) [floating](/Web/CSS/float) elements that precede it. The `clear` property applies to floating and non-floating elements.

{{EmbedInteractiveExample("pages/css/clear.html")}}

When applied to non-floating blocks, it moves the [border edge](/Web/CSS/CSS_box_model/Introduction_to_the_CSS_box_model#border_area) of the element down until it is below the [margin edge](/Web/CSS/CSS_box_model/Introduction_to_the_CSS_box_model#margin_area) of all relevant floats. The non-floated block's top margin collapses.
...
The floats that are relevant to be cleared are the earlier floats within the same [block formatting context](/Web/CSS/CSS_display/Block_formatting_context).

> [!NOTE]
> If an element contains only floated elements, its height collapses to nothing. If you want it to always be able to resize, so that it contains floating elements inside it, set the value of the element's [`display`](/Web/CSS/display) property to [`flow-root`](/Web/CSS/display#flow-root).
>
> ```css
> #container {
>   display: flow-root;
...
- [CSS basic box model](/Web/CSS/CSS_box_model/Introduction_to_the_CSS_box_model)
