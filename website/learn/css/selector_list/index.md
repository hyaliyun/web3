...
A relative selector list is a comma-separated selector list parsed as [relative selectors](/Web/CSS/CSS_selectors/Selector_structure#relative_selector), which begin with an explicit or implied combinator.
...
In the above example, the italic style will be applied to any `h2` heading that is immediately followed by `<p>` or `<ul class="red">`. Note that pseudo-elements and the `:has()` selector are not valid within the [`:has()`](/Web/CSS/:has) relative selector list.
...
- The [`:is()`](/Web/CSS/:is) and [`:where()`](/Web/CSS/:where) pseudo-classes accept forgiving selector lists.
- The [`:not()`](/Web/CSS/:not) pseudo-class accepts a regular selector list
- The [`:has()`](/Web/CSS/:has) pseudo-class accepts a relative selector list.
