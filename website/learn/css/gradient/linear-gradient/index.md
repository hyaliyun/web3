...
The **`linear-gradient()`** [CSS](/Web/CSS) [function](/Web/CSS/CSS_Functions) creates an image consisting of a progressive transition between two or more colors along a straight line. Its result is an object of the {{CSSxRef("&lt;gradient&gt;")}} data type, which is a special kind of {{CSSxRef("&lt;image&gt;")}}.
...
- `<linear-color-stop>`
  - : A color-stop's  value, followed by one or two optional stop positions, (each being either a {{CSSxRef("&lt;percentage&gt;")}} or a {{CSSxRef("&lt;length&gt;")}} along the gradient's axis).
- `<color-hint>`
...
> [!NOTE]
> Rendering of [color stops in CSS gradients](#composition_of_a_linear_gradient) follows the same rules as color stops in [SVG gradients](/Web/SVG/Tutorial/Gradients).

## Description

As with any gradient, a linear gradient has [no intrinsic dimensions](/Web/CSS/image#description); i.e., it has no natural or preferred size, nor a preferred ratio. Its concrete size will match the size of the element it applies to.
...
Because `<gradient>`s belong to the `<image>` data type, they can only be used where [`<image>`](/Web/CSS/image)s can be used. For this reason, `linear-gradient()` won't work on {{CSSxRef("background-color")}} and other properties that use the  data type.
...
The box on the top uses [shorter interpolation](/Web/CSS/hue-interpolation-method#shorter), meaning color goes straight from red to blue using the shorter arc on [color wheel](/Glossary/Color_wheel). The box on the bottom uses [longer interpolation](/Web/CSS/hue-interpolation-method#longer), meaning the color goes from red to blue using the longer arc, traversing through greens, yellows, and oranges.
...
- {{CSSxRef("&lt;image&gt;")}}
- [CSS images module](/Web/CSS/CSS_images)
