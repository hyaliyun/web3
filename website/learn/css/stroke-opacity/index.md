...
The **`stroke-opacity`** [CSS](/Web/CSS) property defines the opacity of an [SVG](/Web/SVG) shape's stroke. The effect is identical to that of {{CSSxref('opacity')}}, except it is applied only to the stroke, not to the entire element. If present, it overrides the element's {{SVGAttr("stroke-opacity")}} attribute.
