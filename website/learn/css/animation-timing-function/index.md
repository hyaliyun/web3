...
The **`animation-timing-function`** [CSS](/Web/CSS) property sets how an animation progresses through the duration of each cycle.
...
> **Note:** `animation-timing-function` has the same effect when creating [CSS scroll-driven animations](/Web/CSS/CSS_scroll-driven_animations) as it does for regular time-based animations.

## Description

Easing functions may be specified on individual keyframes in a [@keyframes](/Web/CSS/@keyframes) rule. If no **`animation-timing-function`** is specified on a keyframe, the corresponding value of **`animation-timing-function`** from the element to which the animation is applied is used for that keyframe.
