...
The **`position-anchor`** [CSS](/Web/CSS) property specifies the anchor name of the **anchor element** (i.e. an element that has an **anchor name** set on it via the {{cssxref("anchor-name")}} property) a positioned element is associated with.
...
- `auto`
  ...
  - : Associates a positioned element with its implicit anchor element, if it has one — for example as set by the non-standard HTML [`anchor`](/Web/HTML/Global_attributes/anchor) attribute.
...
If the associated anchor is hidden, for example with {{cssxref("display", "display: none")}} or {{cssxref("visibility", "visibility: hidden")}}, or if it is part of the [skipped contents](/Web/CSS/CSS_containment/Using_CSS_containment#skips_its_contents) of another element due to it having {{cssxref("content-visibility", "content-visibility: hidden")}} set on it, the anchor positioned element will not be displayed.

The `position-anchor` property is supported on all elements that are positioned, including [pseudo-elements](/Web/CSS/Pseudo-elements) like {{cssxref("::before")}} and {{cssxref("::after")}}. Pseudo elements are implicitly anchored to the same element as the pseudo-element's originating element, unless otherwise specified.

For more information on anchor features and usage, see the [CSS anchor positioning](/Web/CSS/CSS_anchor_positioning) module landing page and the [Using CSS anchor positioning](/Web/CSS/CSS_anchor_positioning/Using) guide.
...
See the `anchor-name` documentation for [basic usage](/Web/CSS/anchor-name#basic_usage) and additional [`position-anchor` examples](/Web/CSS/anchor-name#examples).
...
We include an [`<input type="range">`](/Web/HTML/Element/input/range) element and an [`<output>`](/Web/HTML/Element/output) element to display the value of the range. The value displayed in the `<output>` element is updated via JavaScript as the slider value changes.
...
We give the thumb, represented by the [`::-webkit-slider-thumb`](/Web/CSS/::-webkit-slider-thumb) pseudo-element, an anchor name of `--thumb`. We then set that name as the value of the `<output>` element's `position-anchor` property, and give it a {{cssxref("position")}} value of `fixed`. These steps associated the `<output>` with the thumb.
...
- {{cssxref("anchor-name")}}
- HTML [`anchor`](/Web/HTML/Global_attributes/anchor) attribute
