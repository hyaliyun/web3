...
- `<attr>`
  - : An {{cssxref("ident")}}, that is, the unquoted name of the attribute. This can be any valid language-specific attribute (SVG, HTML, XML, etc), a [`data-*` attribute](/Web/HTML/Global_attributes/data-*), or an author-created attribute.
- `<value>`
...
If the attribute value is case-sensitive, like [`class`](/Web/HTML/Global_attributes/class), [`id`](/Web/HTML/Global_attributes/id), and [`data-*`](/Web/HTML/Global_attributes/data-*) attributes, the attribute selector value match is case-sensitive. Attributes defined outside of the HTML specification, like [`role`](/Web/Accessibility/ARIA/Roles) and [`aria-*`](/Web/Accessibility/ARIA/Attributes) attributes, are also case-sensitive. Case-sensitive attribute selectors can be made case-insensitive with the inclusion of the case-insensitive modifier (`i`).
...
The HTML specification requires the [`type`](/Web/HTML/Element/input#type) attribute to be matched case-insensitively because it is primarily used in the  element.
