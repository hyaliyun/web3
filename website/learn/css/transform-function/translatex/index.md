...
The **`translateX()`** [CSS](/Web/CSS) [function](/Web/CSS/CSS_Functions) repositions an element horizontally on the 2D plane. Its
...
- `<length-percentage>`
  - : Is a {{cssxref("&lt;length&gt;")}} or {{cssxref("&lt;percentage&gt;")}} representing the abscissa (horizontal, x-component) of the
    translating vector [tx, 0]. In [Cartesian coordinate system](/Web/CSS/transform-function#cartesian_coordinates) it represents shift along x-axis. A percentage value refers to the width of the reference box defined by the
