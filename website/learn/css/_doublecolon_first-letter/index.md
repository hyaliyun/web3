...
The **`::first-letter`** [CSS](/Web/CSS) [pseudo-element](/Web/CSS/Pseudo-elements) applies styles to the first letter of the first line of a [block container](/Web/CSS/Visual_formatting_model#block_containers), but only when not preceded by other content (such as images or inline tables).
...
> [!NOTE]
> CSS introduced the `::first-letter` notation (with two colons) to distinguish [pseudo-classes](/Web/CSS/Pseudo-classes) from [pseudo-elements](/Web/CSS/Pseudo-elements). For backward compatibility, browsers also accept `:first-letter`, introduced earlier.
>
