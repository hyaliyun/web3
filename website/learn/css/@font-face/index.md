...
The **`@font-face`** [CSS](/Web/CSS) [at-rule](/Web/CSS/At-rule) specifies a custom font with which to display text; the font can be loaded from either a remote server or a locally-installed font on the user's own computer.
...
> [!NOTE]
> Fallback strategies for loading fonts on older browsers are described in the [`src` descriptor page](/Web/CSS/@font-face/src#specifying_fallbacks_for_older_browsers).

The `@font-face` at-rule may be used not only at the top level of a CSS, but also inside any [CSS conditional-group at-rule](/Web/CSS/CSS_conditional_rules#at-rules).
...
- Web fonts are subject to the same domain restriction (font files must be on the same domain as the page using them), unless [HTTP access controls](/Web/HTTP/CORS) are used to relax this restriction.
...
- [About WOFF](/Web/CSS/CSS_fonts/WOFF)
