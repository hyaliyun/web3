...
- The [`<link>`](/Web/HTML/Element/link) element is never matched by `:visited`.
- DOM methods that match elements via CSS selectors — such as [`querySelector()`](/Web/API/Document/querySelector) and [`querySelectorAll()`](/Web/API/Document/querySelectorAll) — will always return an "empty" result even if there are visited links in a document. For the aforementioned methods, this will be `null` or an empty [`NodeList`](/Web/API/NodeList), respectively.

> [!NOTE]
> For more information on these limitations and the reasons behind them, see [Privacy and the :visited selector](/Web/CSS/Privacy_and_the_:visited_selector).
