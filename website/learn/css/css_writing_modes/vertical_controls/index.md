...
- [`<input type="range">`](/Web/HTML/Element/input/range) sliders, {{htmlelement("progress")}} bars, and {{htmlelement("meter")}} elements.
- {{htmlelement("select")}} elements.
- {{htmlelement("button")}} elements and button input types such as [`<input type="button">`](/Web/HTML/Element/input/button), [`<input type="reset">`](/Web/HTML/Element/input/reset), and [`<input type="submit">`](/Web/HTML/Element/input/submit).
- {{htmlelement("textarea")}} elements and text-based input types such as [`<input type="text">`](/Web/HTML/Element/input/text), [`<input type="datetime-local">`](/Web/HTML/Element/input/datetime-local), and [`<input type="url">`](/Web/HTML/Element/input/url).
...
You could use a [transform](/Web/CSS/transform) to rotate the controls by 90 degrees, but that would place the controls in their own layer and cause unintended layout side effects such as other content being overlapped. Using `writing-mode` offers a more reliable solution.
...
A typical set of visual [`<input type="range">`](/Web/HTML/Element/input/range) slider, {{htmlelement("progress")}}, and {{htmlelement("meter")}} controls is created like this:
...
- The non-standard [`appearance: slider-vertical`](/Web/CSS/appearance) property can be used in older versions of Safari and Chrome.
...
The HTML for this example includes an [`<input type="range">`](/Web/HTML/Element/input/range) slider only, with `orient="vertical"` added to make it display vertically in older Firefox versions:
...
This section shows how to handle vertical {{htmlelement("button")}} elements. Note that while we have only used a `<button>` element in the examples below, the behavior is the same for other elements that create buttons, such as [`<input>`](/Web/HTML/Element/input) types of [`button`](/Web/HTML/Element/input/button), [`reset`](/Web/HTML/Element/input/reset), and [`submit`](/Web/HTML/Element/input/submit).
...
Last but not least, we'll look at handling vertical {{htmlelement("textarea")}}s and textual `<input>` types. Note that, while the only `<input>` type we are including is an `<input type="text">` element in the examples below, the behavior is the same for other textual [`<input>`](/Web/HTML/Element/input) types: [`password`](/Web/HTML/Element/input/button), [`number`](/Web/HTML/Element/input/reset), [`url`](/Web/HTML/Element/input/submit), etc.
...
- The [`<input>`](/Web/HTML/Element/input) element.
- Other relevant elements: {{htmlelement("button")}}, {{htmlelement("meter")}}, {{htmlelement("progress")}}, and {{htmlelement("select")}}.
- [Handling different text directions](/Learn/CSS/Building_blocks/Handling_different_text_directions)
