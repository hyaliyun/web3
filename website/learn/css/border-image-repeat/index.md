...
The **`border-image-repeat`** [CSS](/Web/CSS) property defines how the [edge regions](/Web/CSS/border-image-slice#edge-regions) and [middle region](/Web/CSS/border-image-slice#middle-region) of a source image are adjusted to fit the dimensions of an element's [border image](/Web/CSS/border-image). The middle region can be displayed by using the keyword "fill" in the border-image-slice property.
