...
The **`border`** [shorthand](/Web/CSS/Shorthand_properties) [CSS](/Web/CSS) property sets an element's border. It sets the values of {{Cssxref("border-width")}}, {{Cssxref("border-style")}}, and {{Cssxref("border-color")}}.
...
- [`border-width`](/Web/CSS/border-width)
- [`border-style`](/Web/CSS/border-style)
- [`border-color`](/Web/CSS/border-color)
...
As with all shorthand properties, any omitted sub-values will be set to their [initial value](/Web/CSS/initial_value). Importantly, `border` cannot be used to specify a custom value for {{cssxref("border-image")}}, but instead sets it to its initial value, i.e., `none`.
...
Borders and [outlines](/Web/CSS/outline) are very similar. However, outlines differ from borders in the following ways:
