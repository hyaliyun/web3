...
The **`calc()`** [CSS](/Web/CSS) [function](/Web/CSS/CSS_Functions) lets you perform calculations when specifying CSS property values. It can be used with {{cssxref("&lt;length&gt;")}}, {{cssxref("&lt;frequency&gt;")}}, {{cssxref("&lt;angle&gt;")}}, {{cssxref("&lt;time&gt;")}}, {{cssxref("&lt;percentage&gt;")}}, {{cssxref("&lt;number&gt;")}}, {{cssxref("&lt;integer&gt;")}}, and {{cssxref("color_value", "&lt;color-function&gt;")}} values.
...
- Serializing the arguments inside `calc()` follows the IEEE-754 standard for floating point math, which means there's a few cases to be aware of regarding the `infinity` and `NaN` constants.
  For more details on how constants are serialized, see the [`calc-keyword`](/Web/CSS/calc-keyword) page.
...
The `calc()` function can be used to manipulate color channels directly within the context of [relative colors](/Web/CSS/CSS_colors/Relative_colors). This allows for dynamic adjustments of color channels in color models such as [`rgb()`](/Web/CSS/color_value/rgb), [`hsl()`](/Web/CSS/color_value/hsl), and [`lch()`](/Web/CSS/color_value/lch).

The relative color syntax defines a number of color-channel keywords, each of which represents the value of the color channel as a {{cssxref("&lt;number&gt;")}} (see [Channel values resolve to `<number>` values](/Web/CSS/CSS_colors/Relative_colors#channel_values_resolve_to_number_values) for more information). The `calc()` function can use these color-channel keywords to perform dynamic adjustments on the color channels, for example, `calc(r + 10)`.
...
When `calc()` is used for controlling text size, be sure that one of the values includes a [relative length unit](/Web/CSS/length#relative_length_units), for example:
...
You can use `calc()` with [CSS variables](/Web/CSS/CSS_cascading_variables). Consider the following code:
...
The `calc()` function can be used to adjust individual color channels in [relative colors](/Web/CSS/CSS_colors/Relative_colors) without the need for storing color channel values as variables.

In the example below, the first paragraph uses a [`<named-color>`](/Web/CSS/named-color).
In the paragraphs that follow, `calc()` is used with the [`rgb()`](/Web/CSS/color_value/rgb) and [`hsl()`](/Web/CSS/color_value/hsl) functions to adjust the values of each color channel relative to the original named color.
...
For another example of using the `calc()` function to derive relative colors, see the [Using math functions](/Web/CSS/CSS_colors/Relative_colors#using_math_functions) section on the _Using relative colors_ page.
...
- [CSS functions](/Web/CSS/CSS_Functions)
