...
The **`:fullscreen`** [CSS](/Web/CSS) [pseudo-class](/Web/CSS/Pseudo-classes) matches every element that is currently in fullscreen mode. If multiple elements have been put into fullscreen mode, this selects them all.
...
The `:fullscreen` pseudo-class is used to override the [`background-color`](/Web/CSS/background-color) of the `<div>` when it is in fullscreen mode.
...
- [Guide to the Fullscreen API](/Web/API/Fullscreen_API/Guide)
- {{cssxref("::backdrop")}}
- DOM API: {{ domxref("Element.requestFullscreen()") }}, {{ domxref("Document.exitFullscreen()") }}, {{ domxref("Document.fullscreenElement") }}
- [`allowfullscreen`](/Web/HTML/Element/iframe#allowfullscreen) attribute
