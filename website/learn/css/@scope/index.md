...
The **`@scope`** [CSS](/Web/CSS) [at-rule](/Web/CSS/At-rule) enables you to select elements in specific DOM subtrees, targeting elements precisely without writing overly-specific selectors that are hard to override, and without coupling your selectors too tightly to the DOM structure.
...
In fact, `:scope` is implicitly prepended to all scoped style rules. If you want, you can explicitly prepend `:scope` or prepend the [nesting](/Web/CSS/CSS_nesting) selector (`&`) to get the same effect if you find these representations easier to understand.
...
`@scope` adds a new criterion to the [CSS cascade](/Web/CSS/CSS_cascade): **scoping proximity**. This states that when two scopes have conflicting styles, the style that has the smallest number of hops up the DOM tree hierarchy to the scope root is applied. Let's look at an example to see what this means.
...
> [!NOTE]
> Scoping proximity overrules source order but is itself overridden by other, higher-priority criteria such as [importance](/Web/CSS/important), [layers](/Learn/CSS/Building_blocks/Cascade_layers), and [specificity](/Web/CSS/Specificity).
