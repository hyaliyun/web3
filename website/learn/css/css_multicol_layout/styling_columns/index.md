...
The gap between columns is controlled using the {{CSSXref("column-gap")}} or {{CSSXref("gap")}} property. The `column-gap` property is defined in the [multi-column layout](/Web/CSS/CSS_multicol_layout) module. The `gap` property is defined in the [box alignment](/Web/CSS/CSS_box_alignment) module. This is a unified property to define gaps between boxes in all layouts that support gaps, including [CSS grid layout](/Web/CSS/CSS_grid_layout/Box_alignment_in_grid_layout) and [CSS flexible box layout](/Web/CSS/CSS_flexible_box_layout/Mastering_wrapping_of_flex_items).
...
This article details all the current ways in which column boxes can be styled. In the next guide, we will take a look at making elements inside a container [span across all columns](/Web/CSS/CSS_multicol_layout/Spanning_balancing_columns).
