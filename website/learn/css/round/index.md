...
The **`round()`** [CSS](/Web/CSS) [function](/Web/CSS/CSS_Functions) returns a rounded number based on a selected rounding strategy.

Authors should use a [custom CSS property](/Web/CSS/--*) (e.g., `--my-property`) for the rounding value, interval, or both; using the `round()` function is redundant if these have known values.
...
- `<rounding-strategy>`
  ...
  - : The rounding strategy.
    ...
    - `up`
      - : Round `valueToRound` up to the nearest integer multiple of `roundingInterval` (if the value is negative, it will become "more positive"). This is equivalent to the JavaScript [`Math.ceil()`](/Web/JavaScript/Reference/Global_Objects/Math/ceil) method.
    - `down`
      - : Round `valueToRound` down to the nearest integer multiple of `roundingInterval` (if the value is negative, it will become "more negative"). This is equivalent to the JavaScript [`Math.floor()`](/Web/JavaScript/Reference/Global_Objects/Math/floor) method.
    - `nearest` (default)
      - : Round `valueToRound` to the nearest integer multiple of `roundingInterval`, which may be either above or below the value.
        ...
        Equivalent to JavaScript [`Math.round()`](/Web/JavaScript/Reference/Global_Objects/Math/round).
    - `to-zero`
      - : Round `valueToRound` to the nearest integer multiple of `roundingInterval` closer to/towards zero (a positive number will decrease, while a negative value will become "less negative"). This is equivalent to the JavaScript [`Math.trunc()`](/Web/JavaScript/Reference/Global_Objects/Math/trunc) method.
...
Note that we apply a [custom CSS property](/Web/CSS/--*) named `--rounding-interval`, that we will use for the rounding interval.
