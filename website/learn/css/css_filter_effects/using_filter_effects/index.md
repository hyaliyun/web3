...
The CSS filter effects module provides 10 [`<filter-function>`](/Web/CSS/filter#functions) functions, as well as the ability to define an almost endless array of effects using SVG filters applied via a `url()` reference.

The following table lists the 10 filter functions, along with their value types, the minimum valid value if applicable, the largest value that creates an effect, and the initial value used for [interpolation](/Glossary/Interpolation).
...
The image is set to be sepia by specifying the value of the `filter` property as the [`sepia()`](/Web/CSS/filter-function/sepia) filter function. The filter is removed on [`:hover`](/Web/CSS/:hover) and [`:focus`](/Web/CSS/:focus) by setting `filter: none`.
...
In the {{HTMLElement("img")}} element, [`tabindex`](/Web/HTML/Global_attributes/tabindex) is set to `0` to enable focus without altering the tabbing order for keyboard users because `<img>` is not an interactive element.
...
In this example, a glow effect is added using a [`drop-shadow()`](/Web/CSS/filter-function/drop-shadow) filter with a `3px` blur and `0` offset.
...
This example applies two filters — [`hue-rotate()`](/Web/CSS/filter-function/hue-rotate) and [`blur()`](/Web/CSS/filter-function/blur) — via the `backdrop-filter` property. The backdrop, the area behind the {{HTMLElement("p")}} element, has a color shift and a blur applied.
...
As filters are applied in sequential order, you can use filter functions more than once. In this example, the [`drop-shadow()`](/Web/CSS/filter-function/drop-shadow) filter has been used four times, each time with a different `<shadow>` value.
...
The [`<filter>`](/Web/SVG/Element/filter)'s {{SVGAttr("stdDeviation")}} attribute accepts up to two values enabling creating more complex blur values. To create an equivalent blur, we include one value for `stdDeviation`:
...
The SVG `url()` filter value can be included as the value of the SVG [`<image>`](/Web/SVG/Element/image) element's [`filter`](/Web/SVG/Attribute/filter) attribute or as part of the value of the CSS `filter` and `backdrop-filter` properties.
...
- [CSS filter effects](/Web/CSS/CSS_filter_effects)
- SVG {{SVGElement("filter")}} element, SVG {{SVGAttr("filter")}} attribute in [SVG](/Web/SVG)
