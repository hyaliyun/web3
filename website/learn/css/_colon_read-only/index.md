...
The **`:read-only`** [CSS](/Web/CSS) [pseudo-class](/Web/CSS/Pseudo-classes) selects elements (such as certain  types and {{htmlelement("textarea")}}) that are not editable by the user. Elements on which the HTML attribute [`readonly`](/Web/HTML/Attributes/readonly) doesn't have an effect (such as [`<input type="radio">`](/Web/HTML/Element/input/radio), [`<input type="checkbox">`](/Web/HTML/Element/input/checkbox), and all other non-form elements) are also selected by the `:read-only` pseudo-class. In fact, `:read-only` matches anything that {{cssxref(":read-write")}} doesn't match, making it equivalent to `:not(:read-write)`.
...
This selector doesn't just select /{{htmlElement("textarea")}} elements — it will select _any_ element that cannot be edited by the user.
...
- {{cssxref(":read-write")}}
- HTML [`contenteditable`](/Web/HTML/Global_attributes#contenteditable) attribute
