...
A [CSS](/Web/CSS) **_pseudo-class_** is a keyword added to a selector that specifies a special state of the selected element(s). For example, the pseudo-class {{CSSxRef(":hover")}} can be used to select a button when a user's pointer hovers over the button and this selected button can then be styled.
...
> [!NOTE]
> In contrast to pseudo-classes, [pseudo-elements](/Web/CSS/Pseudo-elements) can be used to style a _specific part_ of an element.
...
These pseudo-classes apply when viewing something which has timing, such as a [WebVTT](/Web/API/WebVTT_API) caption track.
...
These pseudo-classes accept a [selector list](/Web/CSS/Selector_list) or [forgiving selector list](/Web/CSS/Selector_list#forgiving_selector_list) as a parameter.
