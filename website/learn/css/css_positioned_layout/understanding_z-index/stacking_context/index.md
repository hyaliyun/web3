...
- Element with a {{cssxref("container-type")}} value `size` or `inline-size` set, intended for [container queries](/Web/CSS/CSS_containment/Container_queries).
- Element that is a child of a [flex](/Web/CSS/CSS_flexible_box_layout/Basic_concepts_of_flexbox) container, with {{cssxref("z-index")}} value other than `auto`.
...
- Element placed into the [top layer](/Glossary/Top_layer) and its corresponding {{cssxref("::backdrop")}}. Examples include [fullscreen](/Web/API/Fullscreen_API) and [popover](/Web/API/Popover_API) elements.
- Element that has had stacking context-creating properties (such as `opacity`) animated using {{cssxref("@keyframes")}}, with [`animation-fill-mode`](/Web/CSS/animation-fill-mode) set to [`forwards`](/Web/CSS/animation-fill-mode#forwards).
