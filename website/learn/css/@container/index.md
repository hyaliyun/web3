...
The **`@container`** [CSS](/Web/CSS) [at-rule](/Web/CSS/At-rule) is a conditional group rule that applies styles to a [containment context](/Web/CSS/CSS_containment/Container_queries#naming_containment_contexts).
...
- `orientation`
  ...
  - : The [orientation](/Web/CSS/@media/orientation) of the container, either `landscape` or `portrait`.
...
Container queries can also evaluate the computed style of the container element. A _container style query_ is a `@container` query that uses one or more `style()` functional notations. The boolean syntax and logic combining style features into a style query is the same as for [CSS feature queries](/Web/CSS/CSS_conditional_rules/Using_feature_queries).
...
The parameter of each `style()` is a single `<style-feature>`. A **`<style-feature>`** is a valid CSS [declaration](/Web/CSS/Syntax#css_declarations), a CSS property, or a [`<custom-property-name>`](/Web/CSS/var#values).
