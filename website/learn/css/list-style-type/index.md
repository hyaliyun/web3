...
The **`list-style-type`** [CSS](/Web/CSS) property sets the marker (such as a disc, character, or custom counter style) of a list item element.

{{EmbedInteractiveExample("pages/css/list-style-type.html")}}

The marker will be [`currentcolor`](/Web/CSS/color_value#currentcolor_keyword), the same as the computed [color](/Web/CSS/color_value) of the element it applies to.
...
Safari will not recognize an ordered or unordered list as a list in the accessibility tree if it has a `list-style-type` value of `none`. This can be resolved by adding `role="list"` to the list's opening tag. To learn more about this and potential workarounds, see [`list-style`](/Web/CSS/list-style#accessibility).
