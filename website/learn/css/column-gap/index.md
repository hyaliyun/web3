...
The **`column-gap`** [CSS](/Web/CSS) property sets the size of the gap ({{glossary("Gutters","gutter")}}) between an element's columns.

Initially a part of [Multi-column Layout](/Web/CSS/CSS_multicol_layout), the definition of `column-gap` has been broadened to include multiple layout methods. Now specified in [CSS box alignment](/Web/CSS/CSS_box_alignment), it may be used in multi-column, flexible box, and grid layouts.
