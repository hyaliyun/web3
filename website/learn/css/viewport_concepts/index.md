...
- The document element's {{DOMxRef("Element.clientWidth")}} is the inner width of a document in [CSS pixels](/Web/HTML/Viewport_meta_tag#screen_density), including padding (but not borders, margins, or vertical scrollbars, if present). **This is the viewport width**.
...
SVG also has an internal coordinate system defined via the [viewBox](/Web/SVG/Attribute/viewBox) attribute, which is not related to this viewport discussion.
...
The [Visual Viewport API](/Web/API/Visual_Viewport_API) provides a mechanism for querying and modifying the properties of the visual viewport.
...
- [Using the viewport meta tag to control layout on mobile browsers](/Web/HTML/Viewport_meta_tag)
