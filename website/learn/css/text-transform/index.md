...
The **`text-transform`** [CSS](/Web/CSS) property specifies how to capitalize an element's text. It can be used to make text appear in all-uppercase or all-lowercase, or with each word capitalized. It also can help improve legibility for ruby.
...
The language is defined by the [`lang`](/Web/HTML/Global_attributes/lang) HTML attribute or the [`xml:lang`](/Web/SVG/Attribute/xml:lang) XML attribute.
...
However, you are encouraged to use [MathML](/Web/MathML) for mathematical formulas, as it provides a more robust and accessible way to represent mathematical content. Here's the same formula using MathML:
