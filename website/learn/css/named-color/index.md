...
The **`<named-color>`** [CSS](/Web/CSS) [data type](/Web/CSS/CSS_Types) is the name of a color, such as `red`, `blue`, `black`, or `lightseagreen`. Syntactically, a `<named-color>` is an [`<ident>`](/Web/CSS/ident).

A `<named-color>` value can be used anywhere a [`<color>`](/Web/CSS/color_value) can be used.
...
Named colors consists of standard colors, the [`transparent`](#transparent) and [`currentcolor`](/Web/CSS/color_value#currentcolor_keyword) keywords.
...
- [`<color>`](/Web/CSS/color_value): the data type of whose definition `<named-color>` is a constituent part.
