...
The **`@media`** [CSS](/Web/CSS) [at-rule](/Web/CSS/At-rule) can be used to apply part of a style sheet based on the result of one or more [media queries](/Web/CSS/CSS_media_queries/Using_media_queries). With it, you specify a media query and a block of CSS to apply to the document if and only if the media query matches the device on which the content is being used.
...
For a discussion of media query syntax, please see [Using media queries](/Web/CSS/CSS_media_queries/Using_media_queries#syntax).
...
- `print`
  - : Intended for paged material and documents viewed on a screen in print preview mode. (Please see [paged media](/Web/CSS/CSS_paged_media) for information about formatting issues that are specific to these formats.)
- `screen`
...
- {{cssxref("@media/display-mode", "display-mode")}}
  - : The mode in which an application is being displayed: for example, [fullscreen](/Web/CSS/@media/display-mode#fullscreen) or [picture-in-picture](/Web/CSS/@media/display-mode#picture-in-picture) mode.
...
Some media queries have corresponding [user agent client hints](/Web/HTTP/Client_hints).
...
To best accommodate people who adjust a site's text size, use [`em`](/Web/CSS/CSS_Values_and_Units#numeric_data_types)s when you need a {{cssxref("&lt;length&gt;")}} for your [media queries](/Web/CSS/CSS_media_queries/Using_media_queries).

Both [`em`](/Web/CSS/CSS_Values_and_Units#numeric_data_types) and [`px`](/Web/CSS/CSS_Values_and_Units#numeric_data_types) are valid units, but [`em`](/Web/CSS/CSS_Values_and_Units#numeric_data_types) works better if the user changes the browser text size.

Also consider media queries or [HTTP user agent client hints](/Web/HTTP/Client_hints#user-agent_client_hints) to improve the user's experience.
For example, the media query [`prefers-reduced-motion`](/Web/CSS/@media/prefers-reduced-motion) or the equivalent HTTP header {{HTTPHeader("Sec-CH-Prefers-Reduced-Motion")}}) can be used to minimize the amount of animation or motion used based on user preferences.

## Security

Because media queries provide insights into the capabilities—and by extension, the features and design—of the device the user is working with, there is the potential that they could be abused to construct a ["fingerprint"](/Glossary/Fingerprinting) which identifies the device, or at least categorizes it to some degree of detail that may be undesirable to users.
...
For more examples, please see [Using media queries](/Web/CSS/CSS_media_queries/Using_media_queries).
