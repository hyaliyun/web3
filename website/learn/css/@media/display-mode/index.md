...
The **`display-mode`** [CSS](/Web/CSS) [media feature](/Web/CSS/@media#media_features) can be used to test whether a web app is being displayed in a normal browser tab or in some alternative way, such as a standalone app or fullscreen mode.

For example:

- A [progressive web app](/Web/Progressive_web_apps) [can set its display mode](/Web/Progressive_web_apps/How_to/Create_a_standalone_app) by setting the [`display`](/Web/Manifest/display) member in its manifest. In this case, the value of `display-mode` identifies the value that was set (but note that this may not be the same as the value requested in the manifest, since a browser may not support the requested mode).

- Any web app can use the [Fullscreen API](/Web/API/Fullscreen_API) or the [Document Picture-in-Picture API](/Web/API/Document_Picture-in-Picture_API) to set the display mode, and in this case the value of `display-mode` identifies the mode that was set.
...
- `fullscreen`
  - : All of the available display area is used and no user agent {{glossary("chrome")}} is shown. This can be used to apply CSS only when the app has been put in fullscreen mode by the [Fullscreen API](/Web/API/Fullscreen_API) or by using the `fullscreen` value of the [`display`](/Web/Manifest/display) member of the Wep App Manifest.
- `minimal-ui`
...
- `window-controls-overlay`
  - : In this mode, the application looks and feels like a standalone desktop application, and the [Window Controls Overlay](/Web/API/Window_Controls_Overlay_API) feature is enabled.
...
See [Using the Document Picture-in-Picture API](/Web/API/Document_Picture-in-Picture_API/Using) for more information and a full example.
