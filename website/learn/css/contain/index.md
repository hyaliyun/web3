...
The **`contain`** [CSS](/Web/CSS) property indicates that an element and its contents are, as much as possible, independent from the rest of the document tree.
...
>
> 1. A new [containing block](/Web/CSS/Containing_block) (for the descendants whose {{cssxref("position")}} property is `absolute` or `fixed`).
> 2. A new [stacking context](/Web/CSS/CSS_positioned_layout/Understanding_z-index/Stacking_context).
> 3. A new [block formatting context](/Web/CSS/CSS_display/Block_formatting_context).
...
Style containment scopes [counters](/Web/CSS/CSS_counter_styles/Using_CSS_counters) and [quotes](/Web/CSS/quotes) to the contained element.
...
CSS quotes are similarly affected in that the [`content`](/Web/CSS/content) values relating to quotes are scoped to the element:
