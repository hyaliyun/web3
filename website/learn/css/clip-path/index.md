...
The **`clip-path`** [CSS](/Web/CSS) property creates a clipping region that sets what part of an element should be shown. Parts that are inside the region are shown, while those outside are hidden.
...
- `<clip-source>`
  - : A {{cssxref("url_value", "&lt;url&gt;")}} referencing an [SVG](/Web/SVG) {{SVGElement("clipPath")}} element.
- {{cssxref("&lt;basic-shape&gt;")}}
...
- `<geometry-box>`
  ...
  - : If specified in combination with a `<basic-shape>`, this value defines the reference box for the basic shape. If specified by itself, it causes the edges of the specified box, including any corner shaping (such as a {{cssxref("border-radius")}}), to be the clipping path. The geometry box can be one of the following values:
    ...
    - `margin-box`
      - : Uses the [margin box](/Web/CSS/CSS_shapes/From_box_values#margin-box) as the reference box.
    - `border-box`
      - : Uses the [border box](/Web/CSS/CSS_shapes/From_box_values#border-box) as the reference box.
    - `padding-box`
      - : Uses the [padding box](/Web/CSS/CSS_shapes/From_box_values#padding-box) as the reference box.
    - `content-box`
      - : Uses the [content box](/Web/CSS/CSS_shapes/From_box_values#content-box) as the reference box.
    - `fill-box`
...
> [!NOTE]
> A computed value other than **`none`** results in the creation of a new [stacking context](/Web/CSS/CSS_positioned_layout/Understanding_z-index/Stacking_context) the same way that CSS {{cssxref("opacity")}} does for values other than `1`.
