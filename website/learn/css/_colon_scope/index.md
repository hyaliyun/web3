...
The **`:scope`** [CSS](/Web/CSS) [pseudo-class](/Web/CSS/Pseudo-classes) represents elements that are a reference point, or scope, for selectors to match against.
...
The scope of `context` is the element with the [`id`](/Web/HTML/Global_attributes#id) of `context`. The selected elements are the `<div>` elements that are direct children of that context — `element-1` and `element-2` — but not their descendants.
...
- The {{cssxref("@scope")}} [at-rule](/Web/CSS/At-rule)
- The {{cssxref(":root")}} [pseudo-class](/Web/CSS/Pseudo-classes)
