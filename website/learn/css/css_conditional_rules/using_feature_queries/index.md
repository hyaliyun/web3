...
Feature queries are created using the CSS at-rule [`@supports`](/Web/CSS/@supports) (or the `supports()` function within [`@import`](/Web/CSS/@import) at-rules).

## Syntax

CSS feature queries are part of the [CSS conditional rules](/Web/CSS/CSS_conditional_rules) module, which also defines the media query [`@media`](/Web/CSS/@media) at-rule. Feature queries behave similarly to [media queries](/Web/CSS/CSS_media_queries/Using_media_queries). The difference is that with a media query, you are testing something about the environment in which the web page is running, whereas with feature queries you are testing browser support for CSS features.
...
The value part of the property-value pair matters more if you are testing for new values of a particular property. All browsers support `color: red`: this dates back to CSS1. However, there are often additional values added to properties in CSS, like [relative colors](/Web/CSS/CSS_colors/Relative_colors), which may not be supported. Feature queries enable testing property and value pairs, meaning we can detect support for values.
...
Feature queries are not limited to property-value pairs. You can include [`font-tech()`](/Web/CSS/@supports#font-tech), [`font-format()`](/Web/CSS/@supports#font-format), and [`selector()`](/Web/CSS/@supports#function_syntax) functions in your feature queries to selectively apply CSS based on whether the user-agent supports a specified font technology, font format, or selector syntax, respectively.
