...
The **`x`** [CSS](/Web/CSS) property defines the x-axis coordinate of the top left corner of the SVG {{SVGElement("rect")}} shape, {{SVGElement("image")}} image, {{SVGElement("foreignObject")}} viewport or nested {{SVGElement("svg")}} viewport relative to the nearest `<svg>` ancestor's user [coordinate system](/Web/CSS/CSSOM_view/Coordinate_systems). If present, it overrides the element's {{SVGAttr("x")}} attribute.
