...
**Specificity** is the algorithm used by browsers to determine the [CSS declaration](/Learn/CSS/First_steps/What_is_CSS#css_syntax) that is the most relevant to an element, which in turn, determines the property value to apply to the element. The specificity algorithm calculates the weight of a [CSS selector](/Web/CSS/Reference#selectors) to determine which rule from competing CSS declarations gets applied to an element.

> [!NOTE]
> Browsers consider specificity **after** determining [cascade origin and importance](/Web/CSS/Cascade). In other words, for competing property declarations, specificity is relevant and compared only between selectors from the one [cascade origin and layer](/Web/CSS/@layer) that has precedence for the property. [Scoping proximity](/Web/CSS/@scope#how_scope_conflicts_are_resolved) and order of appearance become relevant when the selector specificities of the competing declarations in the cascade layer with precedence are equal.
...
- ID column
  - : Includes only [ID selectors](/Web/CSS/ID_selectors), such as `#example`. For each ID in a matching selector, add 1-0-0 to the weight value.
- CLASS column
  - : Includes [class selectors](/Web/CSS/Class_selectors), such as `.myClass`, attribute selectors like `[type="radio"]` and `[lang|="fr"]`, and pseudo-classes, such as `:hover`, `:nth-of-type(3n)`, and `:required`. For each class, attribute selector, or pseudo-class in a matching selector, add 0-1-0 to the weight value.
- TYPE column
  - : Includes [type selectors](/Web/CSS/Type_selectors), such as `p`, `h1`, and `td`, and pseudo-elements like `::before`, `::placeholder`, and all other selectors with double-colon notation. For each type or pseudo-element in a matching selector, add 0-0-1 to the weight value.
- No value
...
Combinators, such as {{CSSxRef("Next-sibling_combinator", "+")}}, {{CSSxRef("Child_combinator", "&gt;")}}, {{CSSxRef("Subsequent-sibling_combinator", "~")}}, [" "](/Web/CSS/Descendant_combinator), and {{CSSxRef("Column_combinator", "||")}}, may make a selector more specific in what is selected but they don't add any value to the specificity weight.
...
When creating complex selector lists with [CSS nesting](/Web/CSS/CSS_nesting) this behaves in exactly the same way as the `:is()` pseudo-class.
...
CSS declarations marked as important override any other declarations within the same cascade layer and origin. Although technically, [`!important`](/Web/CSS/important) has nothing to do with specificity, it interacts directly with specificity and the cascade. It reverses the [cascade](/Web/CSS/Cascade) order of stylesheets.
...
If you're unable to remove `!important` flags from an authors style sheet, the only solution to overriding the important styles is by using `!important`. Creating a [cascade layer](/Web/CSS/@layer) of important declaration overrides is an excellent solution. Two ways of doing this include:
...
1. Specificity only applies when the same element is targeted by multiple declarations in the same cascade layer or origin. Specificity only matters for declarations of the same importance and same origin and [cascade layer](/Web/CSS/@layer). If matching selectors are in different origins, the [cascade](/Web/CSS/Cascade) determines which declaration takes precedence.

2. When two selectors in the same cascade layer and origin have the same specificity, scoping proximity is then calculated; the ruleset with the lowest scoping proximity wins. See [How `@scope` conflicts are resolved](/Web/CSS/@scope#how_scope_conflicts_are_resolved) for more details and an example.
...
- ["Specificity" in "Cascade and inheritance"](/Learn/CSS/Building_blocks/Cascade_and_inheritance#specificity_2)
