...
The **`place-self`** [CSS](/Web/CSS) [shorthand property](/Web/CSS/Shorthand_properties) allows you to align an individual item in both the block and inline directions at once (i.e. the {{cssxref("align-self")}} and {{cssxref("justify-self")}} properties) in a relevant layout system such as [Grid](/Web/CSS/CSS_grid_layout) or [Flexbox](/Web/CSS/CSS_flexible_box_layout). If the second value is not present, the first value is also used for it.
...
- [`align-self`](/Web/CSS/align-self)
- [`justify-self`](/Web/CSS/justify-self)
...
- `anchor-center`
  - : In the case of [anchor-positioned](/Web/CSS/CSS_anchor_positioning) elements, aligns the item to the center of the associated anchor element in the block and inline direction. See [Centering on the anchor using `anchor-center`](/Web/CSS/CSS_anchor_positioning/Using#centering_on_the_anchor_using_anchor-center).
...
In the following example we have a simple 2 x 2 grid layout. Initially the grid container has [`justify-items`](/Web/CSS/justify-items) and [`align-items`](/Web/CSS/align-items) values of `stretch` — the defaults — which causes the grid items to stretch across the entire width of their cells.
