...
The **`<line-style>`** {{glossary("enumerated")}} value type represents keyword values that define the style of a line, or the lack of a line. The `<line-style>` keyword values are used in the following longhand and shorthand [border](/Web/CSS/CSS_backgrounds_and_borders) and [column](/Web/CSS/CSS_multicol_layout) properties:
...
This example uses multiple {{HTMLElement( "div" )}} elements, each with a different `border-color` set as an inline [`style`](/Web/HTML/Global_attributes/style).
...
The four sides of each `<div>` have a different `<line-style>` value, and each list item has a different {{cssxref("color_value", "&lt;color>")}} value. We use [generated content](/Web/CSS/content) to display the CSS declared inline.
...
- [CSS basic user interface](/Web/CSS/CSS_basic_user_interface) module
