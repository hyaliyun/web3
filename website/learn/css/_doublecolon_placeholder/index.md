...
The **`::placeholder`** [CSS](/Web/CSS) [pseudo-element](/Web/CSS/Pseudo-elements) represents the [placeholder text](/Web/HTML/Element/input#placeholder) in an  or {{HTMLElement("textarea")}} element.
...
An alternate approach to providing placeholder information is to include it outside of the input in close visual proximity, then use [`aria-describedby`](/Web/Accessibility/ARIA/Attributes/aria-describedby) to programmatically associate the  with its hint.
...
Placeholders are not a replacement for the {{htmlelement("label")}} element. Without a label that has been programmatically associated with an input using a combination of the [`for`](/Web/HTML/Element/label#for) and [`id`](/Web/HTML/Global_attributes#id) attributes, assistive technology such as screen readers cannot parse  elements.
...
- Related HTML elements: , {{HTMLElement("textarea")}}
