...
We're going to create this layout using the _named template areas_ that we learned about in the guide _[Grid template areas](/Web/CSS/CSS_grid_layout/Grid_template_areas)_.
...
With our mobile layout in place, we can now proceed to add a [media query](/Web/CSS/CSS_media_queries) to adapt this layout for bigger screens with enough real estate to display two columns.
...
Many layouts are essentially sets of "cards" – product listings, image galleries, and so on. A grid can make it very easy to create these listings in a way that is responsive without needing to add [media queries](/Web/CSS/CSS_media_queries) to make it so. In this next example I'm combining CSS grid and flexbox layouts to make a simple product listing layout.
...
We can cause a grid to backfill those gaps by setting {{cssxref("grid-auto-flow", "grid-auto-flow: dense")}} on the grid container. Take care when doing this however as it does take items away from their logical source order. You should only do this if your items do not have a set order – and be aware of the [issues](/Web/CSS/CSS_grid_layout/Grid_layout_and_accessibility#visual_not_logical_re-ordering) of the tab order following the source and not your reordered display.
...
- [CSS Layout: Grids](/Learn/CSS/CSS_layout/Grids)
