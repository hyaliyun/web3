...
The **`zoom`** [CSS](/Web/CSS) property can be used to control the magnification level of an element.
...
In this first block, of CSS, we are setting the starting value for the `--zoom-level` using [custom properties](/Web/CSS/--*) and then using that as the value for `zoom` on the content block.
