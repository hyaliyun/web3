...
The **`scroll-timeline-name`** [CSS](/Web/CSS) property is used to define the name of a _named scroll progress timeline_, which is progressed through by scrolling a scrollable element (_scroller_) between top and bottom (or left and right). `scroll-timeline-name` is set on the scroller that will provide the timeline.

The name is then referenced in an [`animation-timeline`](/Web/CSS/animation-timeline) declaration to indicate the container's element that is used to drive the progress of the animation through the scrolling action.
...
The {{cssxref("scroll-timeline-axis")}} and `scroll-timeline-name` properties can also be set using the [`scroll-timeline`](/Web/CSS/scroll-timeline) shorthand property.
...
- `<dashed-ident>`
  ...
  - : An arbitrary custom identifier defining a name for a scroll progress timeline, which can then be referenced in an [`animation-timeline`](/Web/CSS/animation-timeline) property.

    > **Note:** [`<dashed-ident>`](/Web/CSS/dashed-ident) values must start with `--`, which helps avoid name clashes with standard CSS keywords.
...
The CSS for the container sets it as the source of a scroll timeline named `--squareTimeline` using the `scroll-timeline-name` property. No [scrollbar axis](/Web/CSS/scroll-timeline-axis) is defined here because the vertical axis will be used by default.
...
- [`scroll-timeline`](/Web/CSS/scroll-timeline), [`scroll-timeline-axis`](/Web/CSS/scroll-timeline-axis)
