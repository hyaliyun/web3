...
The **`xywh()`** [CSS](/Web/CSS) function creates a rectangle using the specified distances from the left (`x`) and top (`y`) edges of the containing block and the specified width (`w`) and height (`h`) of the rectangle. It is a basic shape function of the {{cssxref("&lt;basic-shape&gt;")}} [data type](/Web/CSS/CSS_Types). You can use the `xywh()` function in CSS properties such as {{cssxref("offset-path")}} to create the rectangular path along which an element moves and in {{cssxref("clip-path")}} to define the shape of the clipping region.
...
- `round <'border-radius'>`
  - : Specifies the radius of the rounded corners of the rectangle using the same syntax as the CSS [`border-radius`](/Web/CSS/border-radius) shorthand property. This parameter is optional.
