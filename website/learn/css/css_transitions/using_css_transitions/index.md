...
CSS transitions let you decide which properties to animate (by [_listing them explicitly_](/Web/CSS/transition-property)), when the animation will start (by setting a [_delay_](/Web/CSS/transition-delay)), how long the transition will last (by setting a [_duration_](/Web/CSS/transition-duration)), and how the transition will run (by defining an [_easing function_](/Web/CSS/transition-timing-function), e.g., linearly or quick at the beginning, slow at the end).

## Which CSS properties can be transitioned?

The Web author can define which property has to be animated and in which way. This allows the creation of complex transitions. However, some properties are [not animatable](/Web/CSS/CSS_animated_properties) as it doesn't make sense to animate them.
...
- {{cssxref("transition-timing-function")}}
  - : Specifies a function to define how intermediate values for properties are computed. _Easing functions_ determine how intermediate values of the transition are calculated. Most [easing functions](/Web/CSS/easing-function) can be specified by providing the graph of the corresponding function, as defined by four points defining a cubic bezier. You can also choose easing from [Easing functions cheat sheet](https://easings.net/).
...
This example demonstrates how [`display`](/Web/CSS/display) and [`content-visibility`](/Web/CSS/content-visibility) can be transitioned. This behavior is useful for creating entry/exit animations where you want to for example remove a container from the DOM with `display: none`, but have it fade out with [`opacity`](/Web/CSS/opacity) rather than disappearing immediately.

Supporting browsers transition `display` and `content-visibility` with a variation on the [discrete animation type](/Web/CSS/CSS_animated_properties#discrete). This generally means that properties will flip between two values 50% through animating between the two.
