...
The **`<hex-color>`** [CSS](/Web/CSS) [data type](/Web/CSS/CSS_Types) is a notation for describing the _hexadecimal color syntax_ of an [sRGB](/Glossary/RGB) color using its primary color components (red, green, blue) written as hexadecimal numbers, as well as its transparency.

A `<hex-color>` value can be used everywhere where a [`<color>`](/Web/CSS/color_value) can be used.
...
- [`rgb()`](/Web/CSS/color_value/rgb) color function
