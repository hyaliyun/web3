...
The **`:read-write`** [CSS](/Web/CSS) [pseudo-class](/Web/CSS/Pseudo-classes) represents an element (such as `input` or `textarea`) that is editable by the user.
...
This selector doesn't just select /{{htmlElement("textarea")}} elements — it will select _any_ element that can be edited by the user, such as a {{htmlelement("p")}} element with [`contenteditable`](/Web/HTML/Global_attributes#contenteditable) set on it.
