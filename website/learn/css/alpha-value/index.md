...
The **`<alpha-value>`** [CSS](/Web/CSS) [data type](/Web/CSS/CSS_Types) represents a value that can be either a {{cssxref("&lt;number&gt;")}} or a {{cssxref("&lt;percentage&gt;")}}, specifying the **{{Glossary("alpha", "alpha channel")}}** or **transparency** of a color.

## Syntax

The value of an `<alpha-value>` is given as either a [`<number>`](/Web/CSS/number) or a {{cssxref("percentage")}}.
...
When animated, values of the `<alpha-value>` CSS data type are {{Glossary("interpolation", "interpolated")}} as real, floating-point numbers. The speed of the interpolation is determined by the [easing function](/Web/CSS/easing-function) associated with the animation.

## Examples

### Setting text color opacity

The [`rgb()`](/Web/CSS/color_value/rgb) function accepts a fourth optional value to specify an alpha value.
