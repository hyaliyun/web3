...
> [!NOTE]
> For compatibility reasons, [Web API](/Web/API/Window/getComputedStyle)-serialized color values are expressed as `rgb()` colors if the alpha channel value is exactly 1, and `rgba()` colors otherwise. In both cases, legacy syntax is used, with commas as separators (for example `rgb(255, 0, 0)`).
...
These variants are defined using relative colors — the `--base-color` [custom property](/Web/CSS/--*) is passed into an `rgb()` function, and the output color has its red and blue channels modified to achieve the desired effect via `calc()` functions, while the green channel is left unchanged.
...
- The  data type for a list of all color notations
- [sRGB color picker and conversion tool](/Web/CSS/CSS_colors/Color_picker_tool)
