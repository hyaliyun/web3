...
The **`<color>`** [CSS](/Web/CSS) [data type](/Web/CSS/CSS_Types) represents a color.
...
- By using [relative color](/Web/CSS/CSS_colors/Relative_colors) syntax to output a new color based on an existing color. Any of the above color functions can take an **origin color** preceded by the `from` keyword and followed by definitions of the channel values for the new **output color**.
...
Color interpolation happens with [gradients](/Web/CSS/gradient), [transitions](/Web/CSS/CSS_transitions/Using_CSS_transitions), and [animations](/Web/CSS/CSS_animations/Using_CSS_animations).

When interpolating `<color>` values, they are first converted to a given color space, and then each component of the [computed values](/Web/CSS/computed_value) are interpolated linearly, with interpolation's speed being determined by the [easing function](/Web/CSS/easing-function) in transitions and animations. The interpolation color space defaults to Oklab, but can be overridden through {{CSSXref("&lt;color-interpolation-method&gt;")}} in some color-related functional notations.
...
Some people have difficulty distinguishing colors. The [WCAG 2.2](/Web/Accessibility/Understanding_WCAG/Perceivable/Use_of_color) recommendation strongly advises against using color as the only means of conveying a specific message, action, or result. See [color and color contrast](/Web/Accessibility/Understanding_WCAG/Perceivable/Color_contrast) for more information.
