...
The **`@import`** [CSS](/Web/CSS) [at-rule](/Web/CSS/At-rule) is used to import style rules from other valid stylesheets.
An `@import` rule _must_ be defined at the top of the stylesheet, before any other at-rule (except [@charset](/Web/CSS/@charset) and [@layer](/Web/CSS/@layer)) and style declarations, or it will be ignored.
...
- _list-of-media-queries_
  - : Is a comma-separated list of [media queries](/Web/CSS/CSS_media_queries/Using_media_queries), which specify the media-dependent conditions for applying the CSS rules defined in the linked URL. If the browser does not support any of these queries, it does not load the linked resource.
- _layer-name_
  - : Is the name of a [cascade layer](/Web/CSS/@layer) into which the contents of the linked resource are imported.
...
Imported rules must come before all other types of rules, except {{CSSxRef("@charset")}} rules and layer creating [`@layer`](/Web/CSS/@layer) statements.
...
The `@import` rule is not a [nested statement](/Web/CSS/Syntax#nested_statements). Therefore, it cannot be used inside [conditional group at-rules](/Web/CSS/CSS_conditional_rules#at-rules).

So that {{glossary("user agent", "user agents")}} can avoid retrieving resources for unsupported media types, authors may specify media-dependent import conditions. These conditional imports specify comma-separated [media queries](/Web/CSS/CSS_media_queries/Using_media_queries) after the URL. In the absence of any media query, the import is not conditional on the media used. Specifying `all` for the `list-of-media-queries` has the same effect.
...
For example, the code below shows an `@import` that is conditional on both [child combinators](/Web/CSS/Child_combinator) (`selector()`) and the `font-tech()` function:
