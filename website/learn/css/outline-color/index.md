...
The **`outline-color`** [CSS](/Web/CSS) property sets the color of an element's outline.
...
The specification also lists an additional value, `auto`, which is not currently supported in any browsers. When implemented, `auto` will compute to [`currentcolor`](/Web/CSS/color_value#currentcolor_keyword) unless [`outline-style`](/Web/CSS/outline-style) is set to `auto` then it will compute to the [accent color](/Web/CSS/accent-color).
