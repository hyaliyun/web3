...
The **`animation-duration`** [CSS](/Web/CSS) property sets the length of time that an animation takes to complete one cycle.
...
- `auto`
  ...
  - : For time-based animations, `auto` is equivalent to a value of `0s` (see below). For [CSS scroll-driven animations](/Web/CSS/CSS_scroll-driven_animations), `auto` fills the entire timeline with the animation.
...
- {{cssxref("&lt;time&gt;")}}
  ...
  - : The time that an animation takes to complete one cycle. This may be specified in either seconds (`s`) or milliseconds (`ms`). The value must be positive or zero and the unit is required.
    ...
    If no value is provided, the default value of `0s` is used, in which case the animation still executes (the [`animationStart`](/Web/API/Element/animationstart_event) and [`animationEnd`](/Web/API/Element/animationend_event) events are fired). Whether or not the animation will be visible when the duration is `0s` will depend on the value of [`animation-fill-mode`](/Web/CSS/animation-fill-mode), as explained below:

    - If `animation-fill-mode` is set to `backwards` or `both`, the first frame of the animation as defined by `animation-direction` will be displayed during [`animation-delay`](/Web/CSS/animation-delay) countdown.
