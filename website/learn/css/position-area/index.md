...
The **`position-area`** [CSS](/Web/CSS) property enables an anchor-positioned element to be positioned relative to the edges of its associated anchor element by placing the positioned element on one or more tiles of an implicit 3x3 grid, where the anchoring element is the center cell.
...
- The three rows are represented by the [physical values](/Web/CSS/position-area_value#physical_grid_keywords) `top`, `center`, and `bottom`. They also have [logical equivalents](/Web/CSS/position-area_value#logical_grid_keywords) such as `block-start`, `center`, and `block-end`, and [coordinate equivalents](/Web/CSS/position-area_value#coordinate_grid_keywords) — `y-start`, `center`, and `y-end`.
...
The dimensions of the center tile are defined by the [containing block](/Web/CSS/Containing_block) of the anchor element, while the dimensions of the grid's outer edge are defined by the positioned element's containing block.

The [`<position-area>`](/Web/CSS/position-area_value) value is composed of one or two keywords, which define the region of the grid the positioned element should be placed inside. To be exact, the containing block of the positioned element is set to the grid area.
...
For detailed information on anchor features, usage, and the `position-area` property, see the [CSS anchor positioning](/Web/CSS/CSS_anchor_positioning) module landing page and the [Using CSS anchor positioning](/Web/CSS/CSS_anchor_positioning/Using) guide, specifically the section on [setting a `position-area`](/Web/CSS/CSS_anchor_positioning/Using#setting_a_position-area).

### Adjusted default behavior

When a [`<position-area>`](/Web/CSS/position-area_value) value is set on a positioned element, some of its properties will have their default behavior adjusted to provide a good default alignment.

#### Self-alignment property `normal` value

The `normal` value of the self-alignment properties, including {{cssxref("align-items")}}, {{cssxref("align-self")}}, {{cssxref("justify-items")}}, and {{cssxref("justify-self")}}, behaves as either `start`, `end`, or [`anchor-center`](/Web/CSS/CSS_anchor_positioning/Using#centering_on_the_anchor_using_anchor-center). Which value a self-alignment property defaults to depends on the positioning of the element:
...
When an anchor-positioned element is positioned using the `position-area` property, any {{glossary("inset properties")}} set, such as {{cssxref("top")}} or {{cssxref("inset-inline-end")}}, specify offsets from the position-area. Some other property values, like [`max-block-size: 100%`](/Web/CSS/max-block-size), will also be relative to the position-area. Any inset properties set or defaulting to `auto` will behave as if their value was set to `0`.

### An aside on positioned element width

If the positioned element does not have a specific size set on it, its size will default to its [intrinsic size](/Glossary/Intrinsic_Size), but it will also be affected by the size of the position-area grid.
...
The HTML includes a {{htmlelement("div")}} and a {{htmlelement("p")}}. The `<p>` will be positioned relative to the `<div>` with CSS. We also include a style block that will be made visible. All elements are set to be directly editable via the [`contenteditable`](/Web/HTML/Global_attributes/contenteditable) attribute.
...
We've also included two forms that contain the {{htmlelement("select")}} and [`<input type="text">`](/Web/HTML/Element/input/text) elements for setting different `position-area` values, and the [`<input type="checkbox">`](/Web/HTML/Element/input/checkbox) element for toggling the vertical {{cssxref("writing-mode")}} on and off. The code for these, along with the JavaScript, has been hidden for the sake of brevity.
...
- The [`anchor()`](/Web/CSS/anchor) function
- The [`<position-area>`](/Web/CSS/position-area_value) value
