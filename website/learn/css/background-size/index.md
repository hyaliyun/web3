...
The **`background-size`** [CSS](/Web/CSS) property sets the size of the element's background image.
...
> [!NOTE]
> In Gecko, background images created using the [`element()`](/Web/CSS/element) function are currently treated as images with the dimensions of the element, or of the background positioning area if the element is SVG, with the corresponding intrinsic proportion. This is non-standard behavior.
...
- **If the `background-size` is `auto` or `auto auto`:**
  ...
  > [!NOTE]
  > SVG images have a [`preserveAspectRatio`](/Web/SVG/Attribute/preserveAspectRatio) attribute that defaults to the equivalent of `contain`; an explicit `background-size` causes `preserveAspectRatio` to be ignored.
...
See [Resizing background images](/Web/CSS/CSS_backgrounds_and_borders/Resizing_background_images) for more examples.
...
- [Scaling of SVG backgrounds](/Web/CSS/Scaling_of_SVG_backgrounds)
- {{cssxref("object-fit")}}
