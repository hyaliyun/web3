...
`http://` is the [_scheme_](/Web/URI/Schemes) of the URL, indicating which protocol the browser must use. Usually it is the HTTP protocol or its secured version, HTTPS. The Web requires one of these two, but browsers also know how to handle other protocols such as `mailto:` (to open a mail client) or `ftp:` to handle a file transfer, so don't be surprised if you see such protocols. The [schemes](/Web/URI/Schemes) reference provides a list of the most common schemes, and documentation for some of them.
...
The URI [_authority_](/Web/URI/Authority) is comprised of user info (optional and usually unspecified), the host name, and the port.
...
`#SomewhereInTheDocument` is the [_fragment_](/Web/URI/Fragment) of the URL, which is an anchor to another part of the resource itself. An anchor represents a sort of "bookmark" inside the resource, giving the browser the directions to show the content located at that "bookmarked" spot. On an HTML document, for example, the browser will scroll to the point where the anchor is defined; on a video or audio document, the browser will try to go to the time the anchor represents. It is worth noting that the part after the #, also known as the fragment identifier, is never sent to the server with the request.

There's a special [text fragment](/Web/URI/Fragment/Text_fragments) feature that allows you to link to a specific part of a web page identified by its text content.
...
- [What is a URL?](/Learn/Common_questions/Web_mechanics/What_is_a_URL)
