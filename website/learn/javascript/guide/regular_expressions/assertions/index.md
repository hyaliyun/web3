...
Use `^` for matching at the beginning of input. In this example, we can get the fruits that start with 'A' by a `/^A/` regex. For selecting appropriate fruits we can use the [`filter`](/Web/JavaScript/Reference/Global_Objects/Array/filter) method with an [arrow](/Web/JavaScript/Reference/Functions/Arrow_functions) function.
...
In the second example `^` is used both for matching at the beginning of input and for creating negated or complemented character class when used within [character classes](/Web/JavaScript/Guide/Regular_expressions/Character_classes).
...
See more examples in the [input boundary assertion](/Web/JavaScript/Reference/Regular_expressions/Input_boundary_assertion) reference.
...
See more examples in the [word boundary assertion](/Web/JavaScript/Reference/Regular_expressions/Word_boundary_assertion) reference.
...
See more examples in the [lookahead assertion](/Web/JavaScript/Reference/Regular_expressions/Lookahead_assertion) reference.
...
The `?!` combination has different meanings in assertions like `/x(?!y)/` and [character classes](/Web/JavaScript/Guide/Regular_expressions/Character_classes) like `[^?!]`.
...
See more examples in the [lookbehind assertion](/Web/JavaScript/Reference/Regular_expressions/Lookbehind_assertion) reference.
