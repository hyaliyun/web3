...
See more examples in the [capturing group](/Web/JavaScript/Reference/Regular_expressions/Capturing_group) reference.
...
See more examples in the [named capturing group](/Web/JavaScript/Reference/Regular_expressions/Named_capturing_group) reference.

### Using groups and back references

In this example, we first match a single or double quote character with `['"]`, remember it, match an arbitrary number of characters with `.*?` (`*?` is a [non-greedy quantifier](/Web/JavaScript/Guide/Regular_expressions/Quantifiers)), until we match the remembered quote character again with `\1`. The `\1` is a backreference to the first capturing group, which matches the same type of quote. The result will therefore be two strings: `"'"` and `'"'`.
...
See more examples in the [backreference](/Web/JavaScript/Reference/Regular_expressions/Backreference) reference.
