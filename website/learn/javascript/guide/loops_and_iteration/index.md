...
chapter of the [JavaScript Guide](/Web/JavaScript/Guide)
...
3. The `statement` executes. To execute multiple statements, use a [block statement](/Web/JavaScript/Reference/Statements/block) (`{ }`) to group those statements.
...
the number of selected options in a scrolling list (a [`<select>`](/Web/HTML/Element/select)
...
over [iterable objects](/Web/JavaScript/Reference/Iteration_protocols) (including
...
The `for...of` and `for...in` statements can also be used with [destructuring](/Web/JavaScript/Reference/Operators/Destructuring_assignment). For example, you can simultaneously loop over the keys and values of an object using {{jsxref("Object.entries()")}}.
