...
- A class method called `constructor` cannot be a [getter](/Web/JavaScript/Reference/Functions/get), [setter](/Web/JavaScript/Reference/Functions/set), [async](/Web/JavaScript/Reference/Statements/async_function), or [generator](/Web/JavaScript/Reference/Statements/function*).
...
> [!NOTE]
> The difference between an explicit constructor like the one above and the default constructor is that the latter doesn't actually invoke [the array iterator](/Web/JavaScript/Reference/Global_Objects/Array/Symbol.iterator) through [argument spreading](/Web/JavaScript/Reference/Operators/Spread_syntax).
...
However, if you provide your own constructor, and your class derives from some parent class, then you must explicitly call the parent class constructor using [`super()`](/Web/JavaScript/Reference/Operators/super).
For example:
...
Using [`new`](/Web/JavaScript/Reference/Operators/new) on a class goes through the following steps:
...
3. The current class's [fields](/Web/JavaScript/Reference/Classes/Public_class_fields) are initialized.
4. The `constructor` body after the `super()` call (or the entire body, if it's a base class) is evaluated.

Within the `constructor` body, you can access the object being created through [`this`](/Web/JavaScript/Reference/Operators/this) and access the class that is called with [`new`](/Web/JavaScript/Reference/Operators/new) through [`new.target`](/Web/JavaScript/Reference/Operators/new.target). Note that methods (including [getters](/Web/JavaScript/Reference/Functions/get) and [setters](/Web/JavaScript/Reference/Functions/set)) and the [prototype chain](/Web/JavaScript/Inheritance_and_the_prototype_chain) are already initialized on `this` before the `constructor` is executed, so you can even access methods of the subclass from the constructor of the superclass. However, if those methods use `this`, the `this` will not have been fully initialized yet. This means reading public fields of the derived class will result in `undefined`, while reading private fields will result in a `TypeError`.
...
If the parent class constructor returns an object, that object will be used as the `this` value on which [class fields](/Web/JavaScript/Reference/Classes/Public_class_fields) of the derived class will be defined. This trick is called ["return overriding"](/Web/JavaScript/Reference/Classes/Private_properties#returning_overriding_object), which allows a derived class's fields (including [private](/Web/JavaScript/Reference/Classes/Private_properties) ones) to be defined on unrelated objects.

The `constructor` follows normal [method](/Web/JavaScript/Reference/Functions/Method_definitions) syntax, so [parameter default values](/Web/JavaScript/Reference/Functions/Default_parameters), [rest parameters](/Web/JavaScript/Reference/Functions/rest_parameters), etc. can all be used.
...
The constructor must be a literal name. [Computed properties](/Web/JavaScript/Reference/Operators/Object_initializer#computed_property_names) cannot become constructors.
