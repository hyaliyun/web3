...
> **Note:** `Number()` can be called with or without [`new`](/Web/JavaScript/Reference/Operators/new), but with different effects. See [Return value](#return_value).
...
When `Number()` is called as a function (without [`new`](/Web/JavaScript/Reference/Operators/new)), it returns `value` [coerced to a number primitive](/Web/JavaScript/Reference/Global_Objects/Number#number_coercion). Specially, [BigInts](/Web/JavaScript/Reference/Global_Objects/BigInt) values are converted to numbers instead of throwing. If `value` is absent, it becomes `0`.
...
Note that this may result in loss of precision, if the BigInt is too large to be [safely represented](/Web/JavaScript/Reference/Global_Objects/Number/isSafeInteger).
