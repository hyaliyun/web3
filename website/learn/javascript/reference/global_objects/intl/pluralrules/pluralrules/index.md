...
> **Note:** `Intl.PluralRules()` can only be constructed with [`new`](/Web/JavaScript/Reference/Operators/new). Attempting to call it without `new` throws a {{jsxref("TypeError")}}.
...
- `options` {{optional_inline}}
  ...
  - : An object containing the following properties, in the order they are retrieved (all of them are optional):
    ...
    `Intl.PluralRules` also supports the `Intl.NumberFormat()` [digit options](/Web/JavaScript/Reference/Global_Objects/Intl/NumberFormat/NumberFormat#digit_options) (see `Intl.NumberFormat()` for details):
