...
An array of strings representing all hour cycle types commonly used for the `Locale`, sorted in descending preference. If the `Locale` already has an [`hourCycle`](/Web/JavaScript/Reference/Global_Objects/Intl/Locale/hourCycle), then the returned array contains that single value.
...
If the `Locale` object doesn't have a `hourCycle` already, `getHourCycles()` lists all commonly-used hour cycle identifiers for the given `Locale`. For examples of explicitly setting a `hourCycle`, see [`hourCycle` examples](/Web/JavaScript/Reference/Global_Objects/Intl/Locale/hourCycle#examples).
...
- {{jsxref("Intl.Locale")}}
- [`Intl.Locale.prototype.hourCycle`](/Web/JavaScript/Reference/Global_Objects/Intl/Locale/hourCycle)
