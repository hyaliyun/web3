...
- `regexp`
  ...
  - : A regular expression object, or any object that has a [`Symbol.search`](/Web/JavaScript/Reference/Global_Objects/Symbol/search) method.
...
The implementation of `String.prototype.search()` itself is very simple — it simply calls the `Symbol.search` method of the argument with the string as the first parameter. The actual implementation comes from [`RegExp.prototype[Symbol.search]()`](/Web/JavaScript/Reference/Global_Objects/RegExp/Symbol.search).

The `g` flag of `regexp` has no effect on the `search()` result, and the search always happens as if the regex's `lastIndex` is 0. For more information on the behavior of `search()`, see [`RegExp.prototype[Symbol.search]()`](/Web/JavaScript/Reference/Global_Objects/RegExp/Symbol.search).
