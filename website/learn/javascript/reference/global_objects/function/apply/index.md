...
- `thisArg`
  - : The value of `this` provided for the call to `func`. If the function is not in [strict mode](/Web/JavaScript/Reference/Strict_mode), [`null`](/Web/JavaScript/Reference/Operators/null) and [`undefined`](/Web/JavaScript/Reference/Global_Objects/undefined) will be replaced with the global object, and primitive values will be converted to objects.
- `argsArray` {{optional_inline}}
  - : An array-like object, specifying the arguments with which `func` should be called, or [`null`](/Web/JavaScript/Reference/Operators/null) or [`undefined`](/Web/JavaScript/Reference/Global_Objects/undefined) if no arguments should be provided to the function.
...
With the [rest parameters](/Web/JavaScript/Reference/Functions/rest_parameters) and parameter [spread syntax](/Web/JavaScript/Reference/Operators/Spread_syntax), this can be rewritten as:
...
In general, `fn.apply(null, args)` is equivalent to `fn(...args)` with the parameter spread syntax, except `args` is expected to be an array-like object in the former case with `apply()`, and an [iterable](/Web/JavaScript/Reference/Iteration_protocols#the_iterable_protocol) object in the latter case with spread syntax.
