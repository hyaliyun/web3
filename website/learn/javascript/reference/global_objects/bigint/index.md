...
**`BigInt`** values represent numeric values which are [too large](/Web/JavaScript/Reference/Global_Objects/Number/MAX_SAFE_INTEGER) to be represented by the `number` {{Glossary("Primitive", "primitive")}}.
...
BigInt values are similar to Number values in some ways, but also differ in a few key matters: A BigInt value cannot be used with methods in the built-in [`Math`](/Web/JavaScript/Reference/Global_Objects/Math) object and cannot be mixed with a Number value in operations; they must be coerced to the same type. Be careful coercing values back and forth, however, as the precision of a BigInt value may be lost when it is coerced to a Number value.
...
- [Bitwise operators](/Web/JavaScript/Reference/Operators#bitwise_shift_operators): `>>`, `<<`, `&`, `|`, `^`, `~`
- [Unary negation (`-`)](/Web/JavaScript/Reference/Operators/Unary_negation)
- [Increment/decrement](/Web/JavaScript/Reference/Operators#increment_and_decrement): `++`, `--`

The boolean-returning operators allow mixing numbers and BigInts as operands:

- [Relational operators](/Web/JavaScript/Reference/Operators#relational_operators) and [equality operators](/Web/JavaScript/Reference/Operators#equality_operators): `>`, `<`, `>=`, `<=`, `==`, `!=`, `===`, `!==`
- [Logical operators](/Web/JavaScript/Reference/Operators#binary_logical_operators) only rely on the [truthiness](/Glossary/Truthy) of operands

A couple of operators do not support BigInt at all:

- [Unary plus (`+`)](/Web/JavaScript/Reference/Operators/Unary_plus) cannot be supported due to conflicting usage in asm.js, so it has been left out [in order to not break asm.js](https://github.com/tc39/proposal-bigint/blob/master/ADVANCED.md#dont-break-asmjs).
- [Unsigned right shift (`>>>`)](/Web/JavaScript/Reference/Operators/Unsigned_right_shift) is the only bitwise operator that's unsupported, as every BigInt value is signed.
...
- it is converted to a [`Boolean`](/Web/JavaScript/Reference/Global_Objects/Boolean): via the [`Boolean`](/Web/JavaScript/Reference/Global_Objects/Boolean) function;
- when used with [logical operators](/Web/JavaScript/Reference/Operators) `||`, `&&`, and `!`; or
- within a conditional test like an [`if`](/Web/JavaScript/Reference/Statements/if...else) statement.

Namely, only `0n` is [falsy](/Glossary/Falsy); everything else is [truthy](/Glossary/Truthy).
...
If you do not wish to patch `BigInt.prototype`, you can use the [`replacer`](/Web/JavaScript/Reference/Global_Objects/JSON/stringify#the_replacer_parameter) parameter of `JSON.stringify` to serialize BigInt values:
...
You can then use the [`reviver`](/Web/JavaScript/Reference/Global_Objects/JSON/parse#using_the_reviver_parameter) parameter of `JSON.parse` to handle them:
...
- Strings are converted by parsing them as if they contain an integer literal. Any parsing failure results in a {{jsxref("SyntaxError")}}. The syntax is a subset of [string numeric literals](/Web/JavaScript/Reference/Global_Objects/Number#number_coercion), where decimal points or exponent indicators are not allowed.
...
- `BigInt.prototype[Symbol.toStringTag]`
  - : The initial value of the [`[Symbol.toStringTag]`](/Web/JavaScript/Reference/Global_Objects/Symbol/toStringTag) property is the string `"BigInt"`. This property is used in {{jsxref("Object.prototype.toString()")}}. However, because `BigInt` also has its own [`toString()`](/Web/JavaScript/Reference/Global_Objects/BigInt/toString) method, this property is not used unless you call [`Object.prototype.toString.call()`](/Web/JavaScript/Reference/Global_Objects/Function/call) with a BigInt as `thisArg`.

## Instance methods

- {{jsxref("BigInt.prototype.toLocaleString()")}}
  - : Returns a string with a language-sensitive representation of this BigInt value. Overrides the [`Object.prototype.toLocaleString()`](/Web/JavaScript/Reference/Global_Objects/Object/toLocaleString) method.
- {{jsxref("BigInt.prototype.toString()")}}
  - : Returns a string representing this BigInt value in the specified radix (base). Overrides the [`Object.prototype.toString()`](/Web/JavaScript/Reference/Global_Objects/Object/toString) method.
- {{jsxref("BigInt.prototype.valueOf()")}}
  - : Returns this BigInt value. Overrides the [`Object.prototype.valueOf()`](/Web/JavaScript/Reference/Global_Objects/Object/valueOf) method.
...
- [`Number.MAX_SAFE_INTEGER`](/Web/JavaScript/Reference/Global_Objects/Number/MAX_SAFE_INTEGER)
