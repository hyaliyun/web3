...
`Reflect.get()` provides the reflective semantic of a [property access](/Web/JavaScript/Reference/Operators/Property_accessors). That is, `Reflect.get(target, propertyKey, receiver)` is semantically equivalent to:
...
`Reflect.get()` invokes the `[[Get]]` [object internal method](/Web/JavaScript/Reference/Global_Objects/Proxy#object_internal_methods) of `target`.
...
- [`handler.get()`](/Web/JavaScript/Reference/Global_Objects/Proxy/Proxy/get)
