...
You can also define functions using the [`function` expression](/Web/JavaScript/Reference/Operators/function).
...
> [!WARNING]
> In [non-strict mode](/Web/JavaScript/Reference/Strict_mode), function declarations inside blocks behave strangely. Only declare functions in blocks if you are in strict mode.

Functions can be conditionally declared — that is, a function statement can be nested within an [`if`](/Web/JavaScript/Reference/Statements/if...else) statement. However, in non-strict mode, the results are inconsistent across implementations.
...
In [strict mode](/Web/JavaScript/Reference/Strict_mode), [block](/Web/JavaScript/Reference/Statements/block)-level function declarations are scoped to that block and are hoisted to the top of the block.
...
Function declarations in JavaScript are [hoisted](/Glossary/Hoisting) to the top of the enclosing function or global scope. You can use the function before you declared it:
...
Note that [function expressions](/Web/JavaScript/Reference/Operators/function) are not hoisted:
