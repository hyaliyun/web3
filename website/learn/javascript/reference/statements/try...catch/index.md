...
Unlike other constructs such as [`if`](/Web/JavaScript/Reference/Statements/if...else) or [`for`](/Web/JavaScript/Reference/Statements/for), the `try`, `catch`, and `finally` blocks must be _blocks_, instead of single statements.
...
You can also use the `try` statement to handle JavaScript exceptions. See the [JavaScript Guide](/Web/JavaScript/Guide/Control_flow_and_error_handling#exception_handling_statements) for more information on JavaScript exceptions.
...
It doesn't need to be a single identifier. You can use a [destructuring pattern](/Web/JavaScript/Reference/Operators/Destructuring_assignment) to assign multiple identifiers at once.
