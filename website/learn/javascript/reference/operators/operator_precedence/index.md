...
Because `+` has higher precedence than [`yield`](/Web/JavaScript/Reference/Operators/yield), this would become `(a + yield) 1` — but because `yield` is a [reserved word](/Web/JavaScript/Reference/Lexical_grammar#keywords) in generator functions, this would be a syntax error. Luckily, most unary operators have higher precedence than binary operators and do not suffer from this pitfall.
...
Because [`await`](/Web/JavaScript/Reference/Operators/await) has higher precedence than [`yield`](/Web/JavaScript/Reference/Operators/yield), this would become `(await yield) 1`, which is awaiting an identifier called `yield`, and a syntax error. Similarly, if you have `new !A;`, because `!` has lower precedence than `new`, this would become `(new !) A`, which is obviously invalid. (This code looks nonsensical to write anyway, since `!A` always produces a boolean, not a constructor function.)
...
The assignment counterparts of these operators ([`&&=`](/Web/JavaScript/Reference/Operators/Logical_AND_assignment), [`||=`](/Web/JavaScript/Reference/Operators/Logical_OR_assignment), [`??=`](/Web/JavaScript/Reference/Operators/Nullish_coalescing_assignment)) are short-circuited as well. They are short-circuited in a way that the assignment does not happen at all.
...
<table class="fullwidth-table">
  <tbody>
    ...
    <tr>
      <td>
        <a href="/Web/JavaScript/Guide/Functions">Function call</a><br><code>x(y)</code>
      </td>
    </tr>
    <tr>
    ...
    <tr>
      <td>right-to-left</td>
      <td>
        <a href="/Web/JavaScript/Reference/Operators/Conditional_operator">Conditional (ternary) operator</a><br><code>x ? y : z</code>
      </td>
      <td>[11]</td>
    </tr>
    <tr>
      <td>right-to-left</td>
      <td>
        <a href="/Web/JavaScript/Reference/Functions/Arrow_functions">Arrow</a><br><code>x => y</code>
      </td>
      <td>[12]</td>
    </tr>
    <tr>
    ...
    <tr>
      <td>
        <a href="/Web/JavaScript/Reference/Operators/Spread_syntax">Spread</a><br><code>...x</code>
      </td>
      <td>[13]</td>
    </tr>
    <tr>
...
7. The operand cannot be an identifier or a [private property](/Web/JavaScript/Reference/Classes/Private_properties) access.
