...
[Rest parameters](/Web/JavaScript/Reference/Functions/rest_parameters), [default parameters](/Web/JavaScript/Reference/Functions/Default_parameters), and [destructuring](/Web/JavaScript/Reference/Operators/Destructuring_assignment) within params are supported, and always require parentheses:
...
Arrow functions can be [`async`](/Web/JavaScript/Reference/Statements/async_function) by prefixing the expression with the `async` keyword.
...
This is because JavaScript only sees the arrow function as having an expression body if the token following the arrow is not a left brace, so the code inside braces ({}) is parsed as a sequence of statements, where `foo` is a [label](/Web/JavaScript/Reference/Statements/label), not a key in an object literal.
...
Because a [class](/Web/JavaScript/Reference/Classes)'s body has a `this` context, arrow functions as [class fields](/Web/JavaScript/Reference/Classes/Public_class_fields) close over the class's `this` context, and the `this` inside the arrow function's body will correctly point to the instance (or the class itself, for [static fields](/Web/JavaScript/Reference/Classes/static)). However, because it is a [closure](/Web/JavaScript/Closures), not the function's own binding, the value of `this` will not change based on the execution context.
...
For similar reasons, the [`call()`](/Web/JavaScript/Reference/Global_Objects/Function/call), [`apply()`](/Web/JavaScript/Reference/Global_Objects/Function/apply), and [`bind()`](/Web/JavaScript/Reference/Global_Objects/Function/bind) methods are not useful when called on arrow functions, because arrow functions establish `this` based on the scope the arrow function is defined within, and the `this` value does not change based on how the function is invoked.

### No binding of arguments

Arrow functions do not have their own [`arguments`](/Web/JavaScript/Reference/Functions/arguments) object. Thus, in this example, `arguments` is a reference to the arguments of the enclosing scope:
...
> [!NOTE]
> You cannot declare a variable called `arguments` in [strict mode](/Web/JavaScript/Reference/Strict_mode#making_eval_and_arguments_simpler), so the code above would be a syntax error. This makes the scoping effect of `arguments` much easier to comprehend.

In most cases, using [rest parameters](/Web/JavaScript/Reference/Functions/rest_parameters)
...
Arrow functions cannot be used as constructors and will throw an error when called with [`new`](/Web/JavaScript/Reference/Operators/new). They also do not have a [`prototype`](/Web/JavaScript/Reference/Global_Objects/Function/prototype) property.
...
The [`yield`](/Web/JavaScript/Reference/Operators/yield) keyword cannot be used in an arrow function's body (except when used within generator functions further nested within the arrow function). As a consequence, arrow functions cannot be used as generators.
...
Although the arrow in an arrow function is not an operator, arrow functions have special parsing rules that interact differently with [operator precedence](/Web/JavaScript/Reference/Operators/Operator_precedence) compared to regular functions.
...
The [`call()`](/Web/JavaScript/Reference/Global_Objects/Function/call), [`apply()`](/Web/JavaScript/Reference/Global_Objects/Function/apply), and [`bind()`](/Web/JavaScript/Reference/Global_Objects/Function/bind) methods work as expected with traditional functions, because we establish the scope for each of the methods:
