...
This happens within any scope (global, module, function, or block) when variables declared with [`let`](/Web/JavaScript/Reference/Statements/let) or [`const`](/Web/JavaScript/Reference/Statements/const) are accessed before the place where they are declared has been executed.
...
For more information, see the description of [Temporal Dead Zone](/Web/JavaScript/Reference/Statements/let#temporal_dead_zone_tdz).

This issue does not occur for variables declared using `var`, because they are initialized with a default value of `undefined` when they are [hoisted](/Glossary/Hoisting).

This error can also occur in [cyclic imports](/Web/JavaScript/Guide/Modules#cyclic_imports) when a module uses a variable that depends on the module itself being evaluated.
