...
When building [Progressive Web Apps (PWAs)](/Web/Progressive_web_apps), it's important to not only consider the appearance of your app's content, but also the way the app appears on the user's device once the app is installed.

One way to customize the window in which your app appears is by using the [`theme_color`](/Web/Manifest/theme_color) and [`background_color`](/Web/Manifest/background_color) [web app manifest](/Web/Manifest) members.
...
The [`background_color`](/Web/Manifest/background_color) manifest member defines the color that appears in the application window before your app's stylesheets have loaded.
...
The value can be any valid CSS [`<color>`](/Web/CSS/color_value). In this web app manifest file example, the application's background color is set to the [named color](/Web/CSS/named-color) `peachpuff`. :
...
The [`theme_color`](/Web/Manifest/theme_color) member in your PWA manifest defines the default color of operating system and browser UI elements used in the application.
...
- On desktop operating systems, the theme color is used to customize the title bar of your [standalone app window](/Web/Progressive_web_apps/How_to/Create_a_standalone_app).
...
As with `background_color`, any [`<color>`](/Web/CSS/color_value) value is valid. In this web app manifest file example, the `theme_color` is set to the `rgb(255 218 185)`, the [RGB](/Web/CSS/color_value/rgb) equivalent of `peachpuff`:
...
The [`theme-color`](/Web/HTML/Element/meta/name/theme-color) value for the [`name`](/Web/HTML/Element/meta/name) attribute of the {{htmlelement("meta")}} HTML element can be used to define a theme color per webpage. This is different from the `theme_color` manifest member which you define only once, globally, for your app.
