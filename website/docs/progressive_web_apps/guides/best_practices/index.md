...
[Progressive Web Apps](/Web/Progressive_web_apps) (PWAs) can be installed on devices and used as traditional websites in web browsers. This means that PWAs need to be able to adapt to different environments and to different user expectations.
...
Your PWA is based on Web technologies. This means that, on top of being installable on devices, PWAs can run in web browsers too. To ensure compatibility, it's essential to [test your app](/Learn/Tools_and_testing/Cross_browser_testing) across various browsers and operating systems.

Consider the diverse range of browsers your users may use and cater to a wide spectrum of potential users. Using [feature detection](/Learn/Tools_and_testing/Cross_browser_testing/Feature_detection) allows you to deliver a usable experience to the broadest audience.
...
Finally, use [semantic HTML elements](/Glossary/Semantics#semantics_in_html) rather than recreating your own buttons or form elements as semantic HTML elements support all user input methods right out of the box.
...
You can provide a custom offline page by using a [service worker](/Web/API/Service_Worker_API) to intercept network requests and respond with the custom offline page when the user is offline.
...
Learn more about [offline and background operations](/Web/Progressive_web_apps/Guides/Offline_and_background_operation).
...
There are tools, APIs, and best practices that help measure and improve performance. To learn more, see [web performance](/Web/Performance).
...
Learn how to make your app accessible in [accessibility](/Web/Accessibility).
...
- Use the [Notifications API](/Web/API/Notifications_API) to send notifications to the user's device.
- Handle files with the [`file_handlers`](/Web/Manifest/file_handlers) web app manifest member.
- [Display badges](/Web/Progressive_web_apps/How_to/Display_badge_on_app_icon) on the app icon.
- Enable [data sharing between apps](/Web/Progressive_web_apps/How_to/Share_data_between_apps).

Many of the [web app manifest members](/Web/Manifest#members) can be used to customize the way your app is displayed on the user's device and integrate more deeply within the operating system.
...
- Use a [standalone display mode](/Web/Progressive_web_apps/How_to/Create_a_standalone_app) to give your app its own dedicated window.
- [Define your app icon](/Web/Progressive_web_apps/How_to/Define_app_icons).
...
- [Customize your app's theme and background colors](/Web/Progressive_web_apps/How_to/Customize_your_app_colors) to provide a more polished experience that feels more like a platform-specific app.
