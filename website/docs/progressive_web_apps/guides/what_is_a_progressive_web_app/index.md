...
- [**PWAs can be installed on the device**](/Web/Progressive_web_apps/Guides/Making_PWAs_installable). This means:
...
- [**PWAs can operate in the background and offline**](/Web/Progressive_web_apps/Guides/Offline_and_background_operation): a typical website is only active while the page is loaded in the browser. A PWA can:

  - Work while the device does not have network connectivity.
  - Update content in the background.
  - Respond to [push messages](/Web/API/Push_API) from the server.
  - Display notifications using the OS [notifications](/Web/API/Notifications_API) system.
...
- A [web app manifest](/Web/Manifest) file, which, at a minimum, provides information that the browser needs to install the PWA, such as the app name and icon.
- Optionally, a [service worker](/Web/API/Service_Worker_API) to provide an offline experience.
