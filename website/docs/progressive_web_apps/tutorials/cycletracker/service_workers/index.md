...
- [`fetch`](/Web/API/ServiceWorkerGlobalScope/fetch_event),
- [`install`](/Web/API/ServiceWorkerGlobalScope/install_event), and
- [`activate`](/Web/API/ServiceWorkerGlobalScope/activate_event) events.
...
As mentioned, when an existing service worker is being replaced by a new one, the existing service worker is used as the PWA's service worker until the new service worker is activated. We use the `activate` event to delete old caches to avoid running out of space. We iterate over named {{domxref("Cache")}} objects, deleting all but the current one, and then set the service worker as the [`controller`](/Web/API/ServiceWorkerContainer/controller) for the PWA.

We listen for the current service worker's global scope [`activate`](/Web/API/ServiceWorkerGlobalScope/activate_event) event.
...
We start by checking that the browser supports the [Service Worker API](/Web/API/Service_Worker_API) by using [feature detection](/Learn/Tools_and_testing/Cross_browser_testing/Feature_detection#the_concept_of_feature_detection) for the presence of the [`serviceWorker`](/Web/API/ServiceWorker) property on the global [`navigator`](/Web/API/Navigator) object:
...
If the property is supported, we can then use the [`register()`](/Web/API/ServiceWorkerContainer/register) method of the service worker API's [`ServiceWorkerContainer`](/Web/API/ServiceWorkerContainer) interface.
...
At its core, a PWA is a web application that can be installed and that is progressively enhanced to work offline. We created a fully functional web application. We then added the two features - a manifest file and a service worker - required to convert it to a PWA. If you want to share your app with others, make it available via a secure connection. Alternatively, if you just want to use the cycle tracker yourself, [create a local development environment](/Web/Progressive_web_apps/Tutorials/CycleTracker/Secure_connection), [install the PWA](/Web/Progressive_web_apps/Guides/Installing), and enjoy! Once installed, you no longer need to run localhost.
