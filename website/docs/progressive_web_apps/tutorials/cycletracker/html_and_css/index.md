...
The first step in this introductory [PWA tutorial](/Web/Progressive_web_apps/Tutorials) is to write the HTML and CSS. The top section of the page is a form for the user to enter the start and end dates of each period. The bottom is a list of previous menstrual cycles.
...
To complete this tutorial, it is helpful to have a basic level of understanding of [HTML](/Learn/Getting_started_with_the_web/HTML_basics), [CSS](/Learn/Getting_started_with_the_web/CSS_basics), and [JavaScript](/Learn/Getting_started_with_the_web/JavaScript_basics). If you're not familiar with these, MDN is the home of [Getting Started](/Learn/Getting_started_with_the_web), an introduction to web development series.

In the next sections, we'll set up a [local development environment](/Web/Progressive_web_apps/Tutorials/CycleTracker/Secure_connection) and take a look at our progress before adding [JavaScript functionality](/Web/Progressive_web_apps/Tutorials/CycleTracker/JavaScript_functionality) to convert the static content created in this section into a functional web application. Once we have a functioning app we will have something that we can progressively enhance into a PWA that is installable and works offline.
...
The root {{HTMLelement("html")}} tags wrap all the content with the [`lang`](/Web/HTML/Global_attributes/lang) attribute defining the primary language of the page.
...
The HTML `<link>` element is used to specify a relationship between the current document and an external resource. There are more than 25 defined values for the [`rel`](/Web/HTML/Attributes/rel) attribute—and many more values that are not in any specification. The most common value, `rel="stylesheet"`, imports an external resource as a stylesheet.

We will revisit the `<link>` element and its `rel` attribute in a future section when we include the [link to the manifest](/Web/Progressive_web_apps/Tutorials/CycleTracker/Manifest_file#adding_the_manifest_to_the_app) file.

### Document body

The {{HTMLelement("body")}} element contains all the content we want displayed when users visit the site on the Internet.

Within the `<body>`, we include the name of the app as a level-1 heading using an [`<h1>`](/Web/HTML/Element/Heading_Elements) and a {{HTMLelement("form")}}.
...
The date pickers are  elements of type {{HTMLElement("input/date", "date")}}. We include the [`required`](/Web/HTML/Attributes/required) attribute to reduce user errors by preventing the user from accidentally submitting an incomplete form.

To associate a `<label>` with a form control, each `<input>` has an [`id`](/Web/HTML/Global_attributes/id) attribute matching the [`for`](/Web/HTML/Attributes/for) attribute of the associated {{HTMLelement("label")}}. The associated label provides each `<input>` with an {{glossary("accessible name")}}.
...
We encourage you to [learn more about making accessible web forms](/Learn/Forms).
...
Before closing the `</body>`, we include a link to the yet-to-be-written `app.js` JavaScript file. We include the [`defer`](/Web/HTML/Element/script#defer) attribute to defer the loading of this script and ensure the JavaScript is executed after the document's HTML has been parsed.
...
We use the [`:nth-of-type(even)`](/Web/CSS/:nth-of-type) pseudo-class [selector](/Web/CSS/CSS_selectors) to set every even-numbered list item to {{CSSXref("inherit")}} the background color from its parent; in this case, inheriting the `#fff` background color from the unordered list.
...
We add a little white space by setting the `body`'s {{CSSXref("margin")}} using the `vw` and `vh` [viewport units](/Web/CSS/length#relative_length_units_based_on_viewport), making white space on the outside of our app be proportional to the viewport's size. We also add a little padding to the `li` and `legend`. Finally, to improve, but not fix, the alignment of the past-periods data, we set the {{CSSXref("font-family")}} of the `ul` results section to be `monospace`, making each glyph have the same fixed width.
...
If any of the above CSS still looks unfamiliar to you, you can look up the [CSS properties](/Glossary/Property/CSS) and [selectors](/Web/CSS/CSS_selectors), or work through the [getting started with CSS](/Learn/CSS/First_steps/Getting_started) learning path.
...
Before moving on, [comment](/Learn/HTML/Introduction_to_HTML/Getting_started#html_comments) out or delete the fake past period data and header:
...
Before adding the [JavaScript functionality](/Web/Progressive_web_apps/Tutorials/CycleTracker/JavaScript_functionality) to convert this static content into a web app and then enhancing it into a progressive web app with a [manifest file](/Web/Progressive_web_apps/Tutorials/CycleTracker/Manifest_file) and [service worker](/Web/Progressive_web_apps/Tutorials/CycleTracker/Service_workers), we'll [create a local development environment](/Web/Progressive_web_apps/Tutorials/CycleTracker/Secure_connection) to view our progress.
