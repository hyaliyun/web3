...
While you can use [Web workers](/Web/API/Web_Workers_API/Using_web_workers) to run even very large, long-duration chunks of [JavaScript](/Web/JavaScript) code asynchronously, there's a huge caveat: Web workers can't directly manipulate the [DOM](/Web/API/Document_Object_Model) and have limited access to methods and properties of the [window](/Web/API/Window) object, including no access to [WebGL](/Web/API/WebGL_API). This all means that unless you can easily pull out the "pure calculation" chunks of your startup process into workers, you'll wind up having to run most or all of your startup code on the main thread.
...
- Use the [`defer`](/Web/HTML/Element/script#defer) or [`async`](/Web/HTML/Element/script#async) attribute on script tags needed by the Web application. This allows HTML parsers to continue processing the document, instead of having to wait until the scripts have been downloaded and executed before continuing.
...
- Don't include scripts or stylesheets that don't participate in the [critical rendering path](/Web/Performance/Critical_rendering_path) in your startup HTML file. Load them only when needed.
...
- Download time
  - : Keep in mind how long it will take the user to download your application's data. If your application is very popular, or has to re-download content frequently, you should try to have as fast a hosting server as possible. Always [compress](/Glossary/gzip_compression) your data to make it as small as you can.
- Data size
...
- Subjective factors
  - : Anything you can do to help keep the user engaged during the startup process will help make the time seem to go by faster. Displaying a mock splash screen can improve [perceived performance](/Learn/Performance/Perceived_performance). For heavy sites, anything you can do to help the user feel like your app is doing something instead of sitting there quietly helps.

## See also

- [Apps](/Web/Progressive_web_apps)
- [Games](/Games)
