...
VP8, which we [describe in general](/Web/Media/Formats/Video_codecs#vp8) in the main [guide to video codecs used on the web](/Web/Media/Formats/Video_codecs), has some specific requirements that must be followed when using it to encode or decode a video track on a WebRTC connection.
...
Our [overview of AVC](/Web/Media/Formats/Video_codecs#avc_h.264) and its features can be found in the main video codec guide.
...
<table class="standard-table">
  ...
  <tbody>
    <tr>
      <th scope="row">
        <a href="/Web/Media/Formats/Audio_codecs#opus">Opus</a>
      </th>
    ...
    <tr>
      <th scope="row">
        <a
          href="/Web/Media/Formats/Audio_codecs#g.711_pulse_code_modulation_of_voice_frequencies"
          >G.711 PCM (A-law)</a
        >
      </th>
...
The Opus format, defined by {{RFC(6716)}} is the primary format for audio in WebRTC. The RTP payload format for Opus is found in {{RFC(7587)}}. You can find more general information about Opus and its capabilities, and how other APIs can support Opus, in the [corresponding section](/Web/Media/Formats/Audio_codecs#opus) of our [guide to audio codecs used on the web](/Web/Media/Formats/Audio_codecs).
...
Now `codecList` is an array [`codec`](/Web/API/RTCRtpSender/getCapabilities_static#codecs) objects, each describing one codec configuration.
Also present in the list will be entries for [retransmission](/Web/API/RTCRtpSender/getCapabilities_static#rtx_retransmission) (RTX), [redundant coding](/Web/API/RTCRtpSender/getCapabilities_static#red_redundant_audio_data) (RED), and [forward error correction](/Web/API/RTCRtpSender/getCapabilities_static#fec_forward_error_correction) (FEC).
...
- [Introduction to WebRTC protocols](/Web/API/WebRTC_API/Protocols)
- [WebRTC connectivity](/Web/API/WebRTC_API/Connectivity)
