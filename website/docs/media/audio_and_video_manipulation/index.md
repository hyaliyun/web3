...
> [!NOTE]
> Due to potential security issues if your video is on a different domain than your code, you'll need to enable [CORS (Cross Origin Resource Sharing)](/Web/HTTP/CORS) on your video server.

### Video and WebGL

[WebGL](/Web/API/WebGL_API) is a powerful API that uses canvas to draw hardware-accelerated 3D or 2D scenes. You can combine WebGL and the {{htmlelement("video")}} element to create video textures, which means you can put video inside 3D scenes.
...
`playbackRate` aside, to manipulate audio you'll typically use the [Web Audio API](/Web/API/Web_Audio_API).
...
| An audio track from [WebRTC](/Web/API/WebRTC_API) (such as the microphone input you can get using {{domxref("MediaDevices.getUserMedia", "getUserMedia()")}}. | {{domxref("MediaStreamAudioSourceNode")}}  |
...
> [!NOTE]
> Unless you have [CORS](/Web/HTTP/CORS) enabled, to avoid security issues your video should be on the same domain as your code.
...
- [Using Video frames as a WebGL Texture](/Web/API/WebGL_API/Tutorial/Animating_textures_in_WebGL#using_the_video_frames_as_a_texture) (You can also the [THREE.js](https://threejs.org/) WebGL library (and others) to [achieve this effect](https://stemkoski.github.io/Three.js/Video.html))
- [Animating Textures in WebGL](/Web/API/WebGL_API/Tutorial/Animating_textures_in_WebGL)
...
- [AudioContext](/Web/API/AudioContext)
- More info on [Spatial Audio](/Web/API/BaseAudioContext/createPanner)
