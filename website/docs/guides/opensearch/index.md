...
> [!WARNING]
> OpenSearch plugins can't be uploaded anymore on [addons.mozilla.org](https://addons.mozilla.org/) (AMO). Search engine feature must use WebExtension API with [chrome settings](/Mozilla/Add-ons/WebExtensions/manifest.json/chrome_settings_overrides) in `manifest.json` file.
...
- InputEncoding
  - : The [character encoding](/Glossary/Character_encoding) to use when submitting input to the search engine.
- Image
  ...
  - : URL of an icon for the search engine. When possible, include a 16×16 image of type `image/x-icon` (such as `/favicon.ico`) and a 64×64 image of type `image/jpeg` or `image/png`.
    ...
    The URL may also use the [`data:` URL scheme](/Web/URI/Schemes/data). (You can generate a `data:` URL from an icon file at [The `data:` URL kitchen](https://software.hixie.ch/utilities/cgi/data/data).)
...
- Url
  ...
  - : Describes the URL or URLs to use for the search. The `template` attribute indicates the base URL for the search query.
    ...
    For search suggestions, the `application/x-suggestions+json` URL template is used to fetch a suggestion list in [JSON](/Glossary/JSON) format.
...
- You **must** include a `text/html` URL — search plugins including only Atom or [RSS](/Glossary/RSS) URL types (which is valid, but Firefox doesn't support) will also generate the "could not download the search plugin" error.
