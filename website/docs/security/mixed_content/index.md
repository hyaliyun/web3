...
Mixed content requests are insecure requests for resources from a [secure context](/Web/Security/Secure_Contexts):
...
For example, the following code defines an [`<a>`](/Web/HTML/Element/a#download) element that could be used to download the page at the insecure origin `http://example.com/`.
...
- [CSP: `upgrade-insecure-requests`](/Web/HTTP/Headers/Content-Security-Policy/upgrade-insecure-requests) upgrades all requests to HTTPS, including blockable mixed content
