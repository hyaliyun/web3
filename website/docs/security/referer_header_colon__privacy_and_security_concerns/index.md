...
There are privacy and security risks associated with the [Referer HTTP header](/Web/HTTP/Headers/Referer). This article describes them, and offers advice on mitigating those risks.
...
- The [`rel`](/Web/HTML/Attributes/rel) attribute set to [`noreferrer`](/Web/HTML/Attributes/rel/noreferrer) on HTML elements that are in danger of leaking such information (such as {{HTMLElement("form")}} and {{HTMLElement("a")}}).
- A {{HTMLElement("meta")}} element with a [name](/Web/HTML/Element/meta#name) of `referrer` and the content set to `no-referrer` to disable the Referer header for the whole document. See [Referrer-Policy Integration with HTML](/Web/HTTP/Headers/Referrer-Policy#integration_with_html).
