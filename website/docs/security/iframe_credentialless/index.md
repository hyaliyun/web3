...
An `<iframe>` is made credentialless by applying the [`credentialless`](/Web/HTML/Element/iframe#credentialless) attribute to it, or setting the equivalent DOM property — {{domxref("HTMLIFrameElement.credentialless")}} — to `true`.
...
- Pop-ups opened by credentialless iframes are opened with [`rel="noopener"`](/Web/HTML/Attributes/rel/noopener) set. This prevents OAuth pop-up flows from being used in credentialless iframes.
...
- [Cross-Origin Resource Sharing](/Web/HTTP/CORS)
- The `<iframe>` [`credentialless`](/Web/HTML/Element/iframe#credentialless) attribute
