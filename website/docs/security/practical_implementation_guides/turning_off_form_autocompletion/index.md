...
By default, browsers remember information that the user submits through  fields on websites. This enables browsers to offer autocompletion (that is, suggest possible completions for fields that the user has started typing in) or autofill (that is, pre-populate certain fields upon load).
...
- If a site sets `autocomplete="off"` for username and password  fields, the browser will still offer to remember this login. If the user agrees, the browser will autofill those fields the next time the user visits the page.
