...
Scripts executed from pages with an `about:blank` or [`javascript:` URL](/Web/URI/Schemes/javascript) inherit the origin of the document containing that URL, since these types of URLs do not contain information about an origin server.
...
The mechanism has some limitations. For example, it will throw a `SecurityError` [`DOMException`](/Web/API/DOMException) if the [`document-domain`](/Web/HTTP/Headers/Permissions-Policy/document-domain) [`Permissions-Policy`](/Web/HTTP/Headers/Permissions-Policy) is enabled or the document is in a sandboxed [`<iframe>`](/Web/HTML/Element/iframe), and changing the origin in this way does not affect the origin checks used by many Web APIs (e.g. [`localStorage`](/Web/API/Window/localStorage), [`indexedDB`](/Web/API/IndexedDB_API), [`BroadcastChannel`](/Web/API/BroadcastChannel), [`SharedWorker`](/Web/API/SharedWorker)). A more exhaustive list of failure cases can be found in [Document.domain > Failures](/Web/API/Document/domain#failures).
...
- Cross-origin _writes_ are typically allowed. Examples are links, redirects, and form submissions. Some HTTP requests require [preflight](/Web/HTTP/CORS#preflighted_requests).
...
Use [CORS](/Web/HTTP/CORS) to allow cross-origin access. CORS is a part of {{Glossary("HTTP")}} that lets servers specify any other hosts from which a browser should permit loading of content.
...
Access to data stored in the browser such as [Web Storage](/Web/API/Web_Storage_API) and [IndexedDB](/Web/API/IndexedDB_API) are separated by origin. Each origin gets its own separate storage, and JavaScript in one origin cannot read from or write to the storage belonging to another origin.
