...
**State Partitioning** is a broad effort by Mozilla to rework how Firefox manages client-side state (i.e., data stored in the browser) to mitigate the ability of websites to abuse state for cross-site tracking, i.e. via [third-party cookies](/Web/Privacy/Third-party_cookies).
...
Our current [default cookie policy](/Web/Privacy/Storage_Access_Policy) blocks access in a third-party context only for domains classified as trackers.
...
- [DOM Cache](/Web/API/Cache)
- [IndexedDB](/Web/API/IndexedDB_API)
- [Broadcast Channel](/Web/API/BroadcastChannel)
- [Shared Workers](/Web/API/SharedWorker)
...
- [DNS](/Glossary/DNS)
- HTTP Authentication
- [Alt-Svc](/Web/HTTP/Headers/Alt-Svc)
- Speculative Connections
- Fonts & Font Cache
- [HSTS](/Web/HTTP/Headers/Strict-Transport-Security)
- OCSP
...
- Prefetch
- Preconnect
- [CORS-preflight](/Glossary/Preflight_request) Cache
- WebRTC deviceID
...
- [Cookies](/Web/API/Document/cookie)
...
- When a partitioned third-party opens a pop-up window that has [opener access](/Web/API/Window/opener) to the originating document, the third-party is granted storage access to its embedder for 30 days.
...
[document.requestStorageAccess](/Web/API/Document/requestStorageAccess) to request unpartitioned access to cookie through the [Storage Access API](/Web/API/Storage_Access_API).
...
| Access to unpartitioned cookies is granted via the [StorageAccessAPI](/Web/API/Document/requestStorageAccess) | Storage access granted for origin "b.example" on "a.example".                                                                                         |
...
- Automatic storage access grants: [document.requestStorageAccess](/Web/API/Document/requestStorageAccess) will always prompt the user.
